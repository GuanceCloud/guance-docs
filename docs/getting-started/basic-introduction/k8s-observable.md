...
通过文档 [容器](https://www.yuque.com/dataflux/datakit/container) 可查看更多 K8S 数据采集配置。

#### 搭建 K8S 指标可视化仪表板

开启 contrainer 采集器以后，配置完指标采集image范围以后，就可以采集对应容器的 [kubelet_pod](https://www.yuque.com/dataflux/datakit/container#23ae0855)、[docker_containers](https://www.yuque.com/dataflux/datakit/container#440c5f51)、[kubernetes](https://www.yuque.com/dataflux/datakit/container#928858dc) 等指标，通过在观测云场景下搭建[仪表板](https://www.yuque.com/dataflux/doc/gqatnx)，可对指标进行可视化监控；通过在观测云监控下配置[告警](https://www.yuque.com/dataflux/doc/qxz5xz)，可通过短信、邮件、钉钉群、微信群等进行告警通知，帮助企业快速发现和定位K8S的故障问题。

1.通过使用 kubernates 指标搭建场景仪表板，支持通过观测云自带的内置视图的[系统视图](https://www.yuque.com/dataflux/doc/gyl9vv#GVo0x)一键创建。

![](../img/3.contrainer_1.png)

2.通过使用 docker_containers 指标搭建场景仪表板，支持通过观测云自带的内置视图的[系统视图](https://www.yuque.com/dataflux/doc/gyl9vv#GVo0x)一键创建。
...
通过 datakit yaml 中添加 Annotation，就可以采集 Pod 的[日志数据](https://www.yuque.com/dataflux/datakit/container#ecf264fa)，通过在观测云场景下搭建[日志自定义查看器](https://www.yuque.com/dataflux/doc/uynpbs)，可对日志进行可视化关联查询，帮助企业快速发现和定位K8S的故障问题。

![](../img/3.contrainer_3.png)

### 通过配置选举为多个集群设置不同的命名空间

当集群中只有一个被采集对象（如 Kubernetes ），但是在批量部署情况下，多个 DataKit 的配置完全相同，都开启了对该中心对象的采集，为了避免重复采集，我们可以开启 DataKit 的选举功能。假设有两个K8S集群，可以在开启选举的前提下，为两个集群设置不同的命名空间。更多可参考文档 [DataKit 选举支持](https://www.yuque.com/dataflux/datakit/election) 。

## 更多参考

更多关于容器和 K8S 的配置方法和指标说明可参考：
### [Kubernetes 环境下的配置](https://www.yuque.com/dataflux/datakit/k8s-config-how-to) 
### [Kubernetes 扩展指标采集](https://www.yuque.com/dataflux/datakit/kubernetes-x)
### [Kubernetes 集群中自定义 Exporter 指标采集](https://www.yuque.com/dataflux/datakit/kubernetes-prom)

更多关于K8S的最佳实践参考：
### [Nginx Ingress可观测最佳实践](https://www.yuque.com/dataflux/bp/ingress)
### [基于Istio实现微服务可观测最佳实践](https://www.yuque.com/dataflux/bp/istio)
