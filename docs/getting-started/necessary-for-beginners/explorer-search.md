...
点击快捷筛选项右上角的「设置」按钮，选择「查询值 TOP 5」，可查看当前筛选项排在前五名的字段属性值统计数量百分比。在排行榜右侧，支持点击「正向筛选」、「反向筛选」按钮，以 `key:value` 的形式对当前排名的字段属性值进行数据筛选查询。
...
在查看器显示列，支持输入字段值进行匹配搜索，默认选中搜索到的第一个字段值，支持通过键盘上下切换键（↑ ↓ ）选择添加显示列。

![](../img/8.showlist_2.2.png)

若输入的字段值不存在，提示“创建并添加”显示列，创建后作为预设字段，后续通过 Pipeline 切割字段并上报数据后可直接显示上报的数据。
