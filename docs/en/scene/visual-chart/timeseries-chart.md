...
| Stacked Display | Only supports bar chart, off by default, when on, "Mode" option appears on the right, including time series, percentage.<br /><li>Time sequence: each column is divided to show the percentage of subcategories under large categories, and statistics on the size of different metrics under each grouping<br /><li>Percentage: each layer of the column represents the percentage of the data of that category to the overall data of that grouping, and each series is stacked according to the percentage it represents |
...
| Legend | Used to visually display the name, quantity and percentage of each metric data. Currently supports selecting the bottom, right, and hidden legends (click on the metric row in the legend to hide/show the corresponding metric) |
| Show only grouping | Disable by default, when enabled, only "tag values" will be displayed in the legend, no more aggregate functions and metric names, etc. |
...
**Note**: A query statement in the time series chart returns a maximum of 10 timeseries, i.e., according to the results of the group by condition, data with more than 10 timeseries are displayed in order with only 10 timeseries.

![](../img/f4.png)

### 时Inter-axis function

Also, in analysis mode, Guance provides a timeseries function, which means you can not only preview the interactive changes of object data and time through the timeseries below the chart, but also drag and drop to select the time range for display. The timeseries range is a query period for the selected time range, fixed three query periods forward and up to one query period backward (up to the current point in time). Example.

- The current time point is 11:00, and the time range is selected as "Last 1 hour", then the timeseries range is 「8:00 - 11: 00」.
- The current time point is 11:00, and the time range is selected as 「10:00-10: 40」 (the query period is 40 minutes), then the timeseries range is 「8:40 - 11:00」.

![](../img/f3.png)

#### Similar Trend Metrics

In the analysis mode of the time series chart, check the trend line/bar of the time series chart to "View Similar Trend Metrics".

"View Similar Trend Metrics" is a search for similar metric trends in the space, using the time frame you selected as absolute time. You can
...
- Click on the search result to go to the "Similar Trend Metrics" detail page
...
- Number of similarities: the number of charts with similar trends under the corresponding "Measurement"
...
The legend function is used to visually display the name, quantity and percentage of each metric data, and currently supports three ways to select the hidden, bottom and right side.
