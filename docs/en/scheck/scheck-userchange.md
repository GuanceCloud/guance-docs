...
2. Create a manifest file `files.manifest` under the directory `/usr/local/scheck/custom.rules.d` (this directory is for user-defined scripts), with the following content:
...
3. In the same directory as the manifest file, create a script file `users.lua` with the following content:
```lua
...
4. When a user is added, it will be detected within the next 10 seconds and the `trigger` function will be invoked, sending the event to the file `/var/log/scheck/event.log`, adding a line of data, for example:
