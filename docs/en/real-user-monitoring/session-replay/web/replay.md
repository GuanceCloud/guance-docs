...
- Static resources are blocked by browsers due to CORS (usually web fonts).
  ...
  - Since playback occurs within the sandbox environment corresponding to `<<< custom_key.brand_main_domain >>>` if certain static resources have not been authorized for a specific domain, your browser will block the request;
