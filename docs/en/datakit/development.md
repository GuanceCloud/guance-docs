...
## Install, Upgrade Test {#install-upgrade-testing}
...
### Custom Directory Run DataKit {#customize-workdir}

By default, DataKit runs in the specified directory (/usr/local/DataKit under Linux) as ==service==, but in an additional way, you can customize the DataKit working directory to run in a non-service mode and read configuration and data from the specified directory, which is mainly used to debug the functions of DataKit during development.
