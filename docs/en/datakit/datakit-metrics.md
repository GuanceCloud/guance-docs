# Datakit Self-Monitoring Metrics

---

To facilitate the observability of Datakit itself, we have added many Prometheus metrics to relevant business modules during the development of Datakit. By exposing these metrics, we can easily troubleshoot some issues that occur during Datakit's operation.

## Metrics List {#metrics}

Since [version 1.5.9](changelog.md#cl-1.5.9), accessing `http://localhost:9529/metrics` retrieves the current metrics list. Different versions of Datakit may adjust certain metrics or add/remove some metrics.

These metrics are also used in the [Datakit Monitor](datakit-monitor.md) display, with optimizations made for better presentation. To view raw metrics (or those not displayed in the monitor), you can use a combination of `curl` and `watch` commands, such as checking the CPU usage of the Datakit process:

```shell
# Retrieve CPU usage every 3 seconds
...
Other metrics can be observed similarly. The following is a list of existing metrics (current version 1.68.1):

<!-- These metrics can be obtained by executing make metrics -->
...
| GAUGE   | `datakit_election_status`                                          | `elected_id,id,namespace,status`                                                                  | Datakit election status, if metric = 0, means not elected, or the elected time(unix timestamp second)                 |
...
Datakit also exposes Golang runtime metrics via its `/metrics` endpoint. Sample metrics include:
...
These metrics provide detailed insights into the Golang runtime performance and memory usage within Datakit, which can be useful for troubleshooting and optimizing the application.

---

By exposing these comprehensive metrics, Datakit ensures that operators and developers have the necessary tools to monitor its health and performance effectively. This helps in identifying and resolving issues promptly, ensuring optimal operation of the system.
