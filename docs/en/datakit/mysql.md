...
For specific fields, the actual collected data shall prevail, and some fields cannot be collected due to MySQL configuration, existing data and other reasons.
...
Record the number of executions of query statements, waiting time, locking time, and the number of record rows of query.
...
Select some SQL statements which are time-consuming to execute, obtain their execution plans, and collect various performance metrics in the actual execution process.
...
Record the wait event information of the current thread.
