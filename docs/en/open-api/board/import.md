...
## Overview
Reset the content of the dashboard corresponding to `dashboard_uuid` based on the content of `templateInfo`. After the operation is completed, the UUID of the view will change, and the charts and view variables within the view will be recreated. At this point, the shared resources in the original view will become invalid; the content of `templateInfo` can match the `content` field result from the [Export Dashboard API] response.
...
| specifyDashboardUUID  | string   |         | Specify the UUID for the newly created dashboard, must start with `dsbd_custom_` followed by 32 lowercase letters or digits<br>Example: dsbd_custom_xxxx32 <br>Can be empty: False <br>Can be empty string: False <br>$matchRegExp: ^dsbd_custom_[a-z0-9]{32}$ <br> |
