...
There are currently two instrumentation methods: **compile-time instrumentation** and **code-invasive manual instrumentation**.

Invasive instrumentation **requires modifying existing code**, but overall, common business code does not require many changes; simply replacing the relevant `import` packages is sufficient.

DDTrace requires installing [orchestrion](https://github.com/DataDog/orchestrion){:target="_blank"} for compile-time APM integration.

Below are the usage methods for these two approaches.
...
- Must use **Go Module** to manage the project.
...
If installation fails, try cloning the project locally and then compiling it.
...
Run the command in the root directory of your project to generate a go file locally.
...
After compilation, the executable program will trigger traces during runtime and upload them.

Modifying configurations via environment variables is the same as with other languages; refer to the documentation below for specific environment variable configurations.
...
Other libraries related to components depend on the situation, such as:
...
You can learn more about available tracing SDKs from the [Github plugin library](https://github.com/DataDog/dd-trace-go/tree/main/contrib){:target="_blank"} or the [Datadog support documentation](https://docs.datadoghq.com/tracing/trace_collection/compatibility/go/#integrations){:target="_blank"}.
...
The following environment variables can be specified when starting the program to configure some DDTrace settings. Their basic format is:
...
???+ attention
    ...
    These environment variables will be overridden by corresponding fields injected with `WithXXX()` in the code. Therefore, configurations injected in the code have higher priority, and these ENVs only take effect when the corresponding fields are not specified in the code.
<!-- markdownlint-enable -->
...
- **`DD_ENV`**
    ...
    Set the current environment of the application, e.g., `prod`, `pre-prod`

- **`DD_AGENT_HOST`**

    **Default value**: `localhost`

    Set the IP address of DataKit, where the generated trace data will be sent to DataKit

- **`DD_TRACE_AGENT_PORT`**

    Set the port that DataKit uses to receive trace data. You need to manually specify [DataKit's HTTP port][4] (usually 9529)

- **`DD_DOGSTATSD_PORT`**

    Default value: `8125`
    If you want to collect StatsD data produced by DDTrace, you need to manually enable the [StatsD collector] in DataKit[5]

- **`DD_TRACE_SAMPLING_RULES`**

    **Default value**: `nil`

    Use a JSON array to represent sampling settings (sampling rates apply in the order of the array). The `sample_rate` represents the sampling rate, with values ranging from `[0.0, 1.0]`.

    **Example One**: Set global sampling rate to 20%: `DD_TRACE_SAMPLING_RULES='[{"sample_rate": 0.2}]' ./my-app`

    **Example Two**: For services named `app1.*` and spans named `abc`, set the sampling rate to 10%; otherwise, set the sampling rate to 20%: `DD_TRACE_SAMPLING_RULES='[{"service": "app1.*", "name": "b", "sample_rate": 0.1}, {"sample_rate": 0.2}]' ./my-app`

- **`DD_TRACE_SAMPLE_RATE`**

    **Default value**: `nil`

    Enable the sampling rate switch above
...
- **`DD_TAGS`**
    ...
    **Default value**: `[]`
    ...
    Inject a set of global tags here. These tags will appear in every span and profile data. Multiple tags can be separated by spaces or commas, e.g., `layer:api,team:intake`, `layer:api team:intake`
...
- **`DD_SERVICE_MAPPING`**
    ...
    **Default value**: `null`
    Dynamically rename service names. Service name mappings can be separated by spaces or commas, such as `mysql:mysql-service-name,postgres:postgres-service-name`, `mysql:mysql-service-name postgres:postgres-service-name`
