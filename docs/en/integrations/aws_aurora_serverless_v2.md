...
monitor:
  - desc: 'AWS Aurora Serverless V2 监控器'
...
To synchronize the monitoring data of AWS Aurora Serverless V2, we install the corresponding collection script：「观测云集成（AWS-RDS-Aurora-postgresql 采集）」(ID：`guance_aws_rds_aurora_postgresql`)
...
## Metric {#metric}
Configure AWS Aurora Serverless V2 monitoring. The default indicator set is as follows. You can collect more indicators by configuring them [Aws Cloud Monitor Metrics Details](https://docs.amazonaws.cn/AmazonRDS/latest/AuroraUserGuide/Aurora.AuroraMonitoring.Metrics.html){:target="_blank"}
