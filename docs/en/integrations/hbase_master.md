...
dashboard :
  - desc  : 'Built-in views of HBase Master'
...
Add to the startup parameters of HBase Master:
...
Go to the `conf.d/prom` directory under the [DataKit installation directory](./datakit_dir.md), copy `prom.conf.sample` to `master.conf`.
...
HBase Master Metrics are located under the Hadoop Measurement Set, this section mainly introduces the relevant Metrics descriptions for HBase Master
...
|`hadoop_hbase_droppedpuball` |`Number of dropped PubAlls` | count |
|`hadoop_hbase_errorrollrequest` |`Number of error roll requests` | count |
|`hadoop_hbase_exceptions` |`Total number of exceptions` | count |
|`hadoop_hbase_exceptions_callqueuetoobig` |`Number of exceptions due to a too large call queue` | count |
...
|`hadoop_hbase_fswritetime_min` |`Minimum time of file system write` | ms |
|`hadoop_hbase_fswritetime_num_ops` |`Number of file system write operations` | count |