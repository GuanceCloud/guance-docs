...
summary: 'Alibaba Cloud MongoDB replica set indicators display, including CPU usage, memory usage, disk usage, data disk space, log disk space, statement execution times per second, requests, connections, network traffic, replication latency, QPS, etc.'
__int_icon: 'icon/aliyun_mongodb'
Alibaba Cloud MongoDB shard cluster indicators display, including CPU usage, memory usage, disk usage, data disk space, log disk space, statement execution times per second, requests, connections, network traffic, replication latency, QPS, 

Alibaba Cloud MongoDB single node example indicators display, including CPU usage, memory usage, disk usage, disk space occupied by data, statement execution times per second, requests, connections, network traffic, QPS, etc.
...
# Alibaba Cloud
<!-- markdownlint-enable -->
Alibaba Cloud MongoDB replica set indicators display, including CPU usage, memory usage, disk usage, data disk space, log disk space, statement execution times per second, requests, connections, network traffic, replication latency, QPS, etc.

Alibaba Cloud MongoDB shard cluster indicators display, including CPU usage, memory usage, disk usage, data disk space, log disk space, statement execution times per second, requests, connections, network traffic, replication latency, QPS, etc.
...
To synchronize the monitoring data of ECS cloud resources, we install the corresponding acquisition script: 「观测云集成（阿里云-MongoDB 采集）」 (ID: 'guance_aliyun_mongodb')
...
2. Check the presence of asset information in the Guance cloud platform, 「Infrastructure / Custom」

3. In the Guance cloud platform, 「Metrics」checks whether there is corresponding monitoring data.

## Metrics {#metric}

Ali Cloud-cloud monitoring is configured, the default collection indicators are as follows, and more indicators can be collected through configuration.

For more indicators, please refer to: [Aliyun Cloud monitoring indicators details](https://help.aliyun.com/document_detail/163515.html){:target="_blank"}
...
After the data is synchronized normally, the data can be viewed in the 「 logging 」 of the observation cloud.
