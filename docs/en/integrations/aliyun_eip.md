...
To synchronize the monitoring data of EIP cloud resources, we install the corresponding collection script：「观测云集成（阿里云-EIP采集）」(ID：`guance_aliyun_eip`)
...
Configure Ali Cloud - cloud monitoring. The default indicator set is as follows. You can collect more indicators by configuring them [Alibaba Cloud Monitor Metrics Details](https://cms.console.aliyun.com/metric-meta/acs_vpc_eip/eip){:target="_blank"}
...
The collected Alibaba Cloud EIP object data structure can see the object data from 「基础设施-自定义」
