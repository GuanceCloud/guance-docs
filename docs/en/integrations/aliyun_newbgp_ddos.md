...
> Tip: Before using this collector, you must install the 「观测云集成 Core 核心包」 and its supporting third-party dependencies.

To synchronize the monitoring data of DDoS New BGP High Defense cloud resources, we install the corresponding collection script：「观测云集成（阿里云-DDoS高防采集）」(ID：`guance_aliyun_ddoscoo`)
...
Configure Ali Cloud - cloud monitoring. The default indicator set is as follows. You can collect more indicators by configuring them [Alibaba Cloud Monitor Metrics Details](https://cms.console.aliyun.com/metric-meta/acs_newbgpddos/newbgpddos?){:target="_blank"}
...
The collected Alibaba Cloud DDoS New BGP High Defense object data structure can see the object data from 「基础设施-自定义」
