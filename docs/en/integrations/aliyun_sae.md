...
- Application container metrics are gathered using Alibaba Cloud's monitoring API and reported to <<< custom_key.brand_name >>> through the Function platform (DataFlux Func).
- After DataKit collects the corresponding data, it processes and reports it to <<< custom_key.brand_name >>>.
...
- Add image: pubrepo.<<< custom_key.brand_main_domain >>>/datakit/datakit:1.31.0
...
1. ENV_DATAWAY: Required, gateway address for reporting to <<< custom_key.brand_name >>>.
...
For more details, refer to [Alibaba Cloud SAE Application Engine Observability Best Practices](https://<<< custom_key.brand_main_domain >>>/learn/articles/SAE).
...
If deploying Func manually, refer to [Manually Deploy Func](https://<<< custom_key.func_domain >>>/doc/script-market-guance-integration/){:target="_blank"}.
...
To synchronize SAE monitoring data, we install the corresponding collection scripts: «<<< custom_key.brand_name >>> Integration (Alibaba Cloud-SAE-Application)» (ID: `guance_aliyun_sae_app`) and «<<< custom_key.brand_name >>> Integration (Alibaba Cloud-SAE-Application Instance)» (ID: `guance_aliyun_sae_app_instance`).
...
2. On the <<< custom_key.brand_name >>> platform, under «Infrastructure / Custom», check if there are any asset information entries.
3. On the <<< custom_key.brand_name >>> platform, under «Metrics», check if there are any corresponding monitoring data entries.
...
Alibaba Cloud SAE provides a way to output logs to the <<< custom_key.brand_name >>> platform via Kafka. The process is as follows:
...
For more detailed steps, refer to [Alibaba Cloud SAE Application Engine Observability Best Practices](https://<<< custom_key.brand_main_domain >>>/learn/articles/SAE).