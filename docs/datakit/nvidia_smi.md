...
进入 DataKit 安装目录下的 `conf.d/host` 目录，复制 `nvidia_smi.conf.sample` 并命名为 `nvidia_smi.conf`。示例如下：
...
以下所有数据采集，默认会追加名为 `host` 的全局 tag（tag 值为 DataKit 所在主机名），也可以在配置中通过 `[inputs.nvidia_smi.tags]` 指定其它标签：
