...
列表中展示的“发生时间”、“链路ID”、“服务名”、“资源名”以及“持续时间”，可通过「显示列」自定义添加、编辑、删除显示列。
...
具体执行的语句和执行时间详情可以参考[span列表](#7b4fc7cc)。
#### 执行时间占比计算说明
...
- 指标视图：可查看相关主机的CPU、内存等性能指标视图，通过绘制当前链路中最开始发生 span 的时间标识来定位当前链路相关主机的性能指标。点击「打开该视图」至内置视图，可通过克隆的方式对主机视图进行自定义修改，并作为用户视图保存，用户视图可通过绑定在链路详情页查看，更多配置详情，可参考[绑定内置视图](https://www.yuque.com/dataflux/doc/dns233/)。
