...
图表查询支持「简单查询」、「表达式查询」、「DQL查询」，图表查询条件详细说明请点击 [图表查询](https://www.yuque.com/dataflux/doc/cxlbps) 进行查看，默认添加简单查询，查询中默认内置「Top」/「Bottom」函数，预设5、10、20、100，4种排名数量，可根据选定指标进行升降排序。排行榜不支持添加别名「AS」。
