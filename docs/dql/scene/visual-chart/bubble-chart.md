...
图表查询条件详细说明请点击 [图表查询](https://www.yuque.com/dataflux/doc/cxlbps) 进行查看，默认添加「x 轴」、「y 轴」「大小」查询，没有「复制」和「删除」按钮；没有「添加查询」按钮。查询的指标集以及指标可以不同，但 by（分组）的Tag 必须一致，修改一个，另外两个自动同步修改。
