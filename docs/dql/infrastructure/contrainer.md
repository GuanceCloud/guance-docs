...
- 时间范围：容器对象列表支持查看**最近十分钟内**采集的Containers、Pods、Services、Deployments、Clusters、Nodes、Replica  Sets、Jobs、Cron Jobs 数据，通过「刷新」按钮可以刷新至当前时间范围，重新获取数据列表
- 搜索与筛选：通过列表上方的搜索和筛选栏，您可以基于标签、字段、文本（包含日志文本）进行关键词搜索、标签筛选、字段筛选、关联搜索，进而过滤出相关的容器对象
   - 输入框支持模糊匹配相关标签和字段
...
- 快捷筛选：您可以通过勾选左侧快捷筛选的字段快速筛选数据，支持自定义添加筛选字段，支持通过“反选”进行快捷筛选，支持通过“重置”快速清除快捷筛选条件。
- 排序：点击列表菜单，如：CPU使用率、MEM使用率，您可以基于选中的标签进行升降排序；
- 分组：通过分组对容器对象数据进行相关聚合，聚合的结果将显示分组排行榜Top10。分组支持多标签筛选。通过点击分组排行榜，该分组对应的字段将被做为筛选条件显示查询结果，即您可查看对应分组里的对象数据；点击分组右侧的"设置"按钮，可自定义添加分组标签，添加完成后，可选择按照添加分组标签进行分组聚合展示，自定义分组标签仅管理员可添加；
- 显示列：通过页面左上角的显示列，您可以向列表添加、删除显示列，便捷您的数据查看和访问；
- 设置：通过页面左上角的"设置"图标，你可以为容器对象[绑定内置视图](https://www.yuque.com/dataflux/doc/dns233)，导出当前对象列表数据到CSV文件或者场景视图。

![](img/WX20211019-174722.png)
...
- 通过「内置视图」的绑定功能，添加查看容器对象相关的内置视图。更多配置详情，可参考[绑定内置视图](https://www.yuque.com/dataflux/doc/dns233/)。
...
![](img/6.container_4.png)

#### 标签属性

容器的「Label 属性」是本身自带的，点击容器对象列表中的名称，即可划出详情页查看其对应的Label属性。

![](img/1.container_1.png)

容器标签添加后，可在容器对象列表通过添加的标签筛选展示相同标签的容器列表。

![](img/1.container_2.png)
...
![](img/9.contrainer_3.png)

- 指标视图：可查看默认24小时内，相关主机的CPU、内存等性能指标视图。点击「打开该视图」至内置视图，可通过克隆的方式对主机视图进行自定义修改，并作为用户视图保存，用户视图可通过绑定在容器详情页查看，更多配置详情，可参考[绑定内置视图](https://www.yuque.com/dataflux/doc/dns233/)。

![](img/9.contrainer_4.png)
...
- 通过「绑定视图」添加查看 Pod 对象相关的内置视图。更多配置详情，可参考[绑定内置视图](https://www.yuque.com/dataflux/doc/dns233/)。

![](img/7.pod_2.png)

另外，通过点击 pod 详情页的标签「主机」，您可以对该主机相关的日志、容器、进程、链路、巡检等数据进行查询。

- “添加到筛选”，即添加该标签至 pod 列表查看器，查看与该主机相关的全部 pod 数据
- “复制”，即复制该标签内容至剪贴板 
- “查看相关日志”，即查看与该主机相关全部日志
- “查看相关容器”，即查看与该主机相关全部容器
- “查看相关进程”，即查看与该主机相关全部进程
...
- 指标视图：可查看默认24小时内，相关主机的CPU、内存等性能指标视图。点击「打开该视图」至内置视图，可通过克隆的方式对主机视图进行自定义修改，并作为用户视图保存，用户视图可通过绑定在Pod详情页查看，更多配置详情，可参考[绑定内置视图](https://www.yuque.com/dataflux/doc/dns233/)。
...
- Pod 网络流量数据默认保存最近48小时，体验版默认保存最近24小时；
...
- “添加到筛选”，即添加该标签至 Services 列表查看器，查看与该主机相关的全部 Service 数据
- “复制”，即复制该标签内容至剪贴板 
- “查看相关日志”，即查看与该主机相关全部日志
- “查看相关容器”，即查看与该主机相关全部容器
- “查看相关进程”，即查看与该主机相关全部进程
...
点击 Deployments 列表中的 Deployment 名称即可划出详情页查看 Deployment 的详细信息，包括名称、基础信息、 Label 标签属性和其他字段属性。支持查看关联的日志、Replica Set 和 Pod 数据。
...
- “添加到筛选”，即添加该标签至 Deployments 列表查看器，查看与该主机相关的全部 Deployment 数据
- “复制”，即复制该标签内容至剪贴板 
- “查看相关日志”，即查看与该主机相关全部日志
- “查看相关容器”，即查看与该主机相关全部容器
- “查看相关进程”，即查看与该主机相关全部进程
...
- “添加到筛选”，即添加该标签至 Clusters 列表查看器，查看与该主机相关的全部 Clusters 数据
- “复制”，即复制该标签内容至剪贴板 
- “查看相关日志”，即查看与该主机相关全部日志
- “查看相关容器”，即查看与该主机相关全部容器
- “查看相关进程”，即查看与该主机相关全部进程
...
- “添加到筛选”，即添加该标签至 Nodes 列表查看器，查看与该主机相关的全部 Nodes 数据
- “复制”，即复制该标签内容至剪贴板 
- “查看相关日志”，即查看与该主机相关全部日志
- “查看相关容器”，即查看与该主机相关全部容器
- “查看相关进程”，即查看与该主机相关全部进程
...
- “添加到筛选”，即添加该标签至 Replica Sets 列表查看器，查看与该主机相关的全部 Replica Sets 数据
- “复制”，即复制该标签内容至剪贴板 
- “查看相关日志”，即查看与该主机相关全部日志
- “查看相关容器”，即查看与该主机相关全部容器
- “查看相关进程”，即查看与该主机相关全部进程
...
- “添加到筛选”，即添加该标签至 Jobs 列表查看器，查看与该主机相关的全部 Jobs 数据
- “复制”，即复制该标签内容至剪贴板 
- “查看相关日志”，即查看与该主机相关全部日志
- “查看相关容器”，即查看与该主机相关全部容器
- “查看相关进程”，即查看与该主机相关全部进程
...
- “添加到筛选”，即添加该标签至 Cron Jobs 列表查看器，查看与该主机相关的全部 Cron Jobs 数据
- “复制”，即复制该标签内容至剪贴板 
- “查看相关日志”，即查看与该主机相关全部日志
- “查看相关容器”，即查看与该主机相关全部容器
- “查看相关进程”，即查看与该主机相关全部进程
