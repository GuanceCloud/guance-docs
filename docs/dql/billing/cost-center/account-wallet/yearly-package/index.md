...
在观测云费用中心，一个账号可同时绑定多个工作空间进行包年套餐费用统计，超出套餐部分按照[按量付费](https://www.yuque.com/dataflux/doc/ateans)进行统计，仅支持[观测云企业账号结算](https://www.yuque.com/dataflux/doc/xcifgo)一种结算方式。
...
按照以上示例的使用量和数据存储策略，1天的超额费用=15+60+15+40+2=132元。<br />注意：超出套餐的部分，“按量付费”的方式基于默认数据保存策略的单价进行结算。
