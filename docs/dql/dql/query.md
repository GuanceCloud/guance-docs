...
DQL 查询遵循如下的语法范式，各个部分之间的相对顺序不能调换，从语法角度而言，`data-source` 是必须的（类似于 SQL 中的 `FROM` 子句），其它部分都是可选的。更多 DQL 语法介绍可参考文档 [DQL 定义](./20-DQL/02-DQL定义.md) 。
```
namespace::
	data-source
	target-clause
...
下面通过 SHOW 函数查询“对象”和“日志”的来源、字段等数据。更多函数介绍可参考文档 [DQL 函数](./20-DQL/03-DQL函数.md) 以及 [DQL 外层函数](./20-DQL/04-DQL外层函数.md) 。
