...
![](img/7.metrics_create_1.png)

**Step 1**: 数据筛选。筛选出当前空间已有的全部/单个日志数据来源，并基于此数据源开始生成新的数据。

- 来源：数据来源，即已开启采集的日志数据来源 。**注意：“ * ”表示为全部数据来源**
...
在「生成指标」页面，点击规则右侧的「查看指标」，将为您跳转至「指标」页面，打开对应的指标集。也可以在「指标」或者「指标字典」直接搜索生成的指标集。
