...
同步 腾讯云-Redis 的监控数据，我们安装对应的采集脚本：「<<< custom_key.brand_name >>>集成（腾讯云-Redis采集）」(ID：`guance_tencentcloud_redis`)
...
我们默认采集了一些配置, 具体见指标一栏 [配置自定义云对象指标](https://<<< custom_key.func_domain >>>/doc/script-market-guance-tencentcloud-redis/){:target="_blank"}
...
- 「<<< custom_key.brand_name >>>集成（腾讯云-Redis慢查询日志采集） 」(ID：`guance_tencentcloud_redis_slowlog`)
