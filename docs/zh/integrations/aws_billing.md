...
同步 AWS-账单采集监控数据，我们安装对应的采集脚本：「{{{ custom_key.brand_name }}}集成（AWS-账单采集-实例维度）」(ID：`guance_aws_billing_by_instance`)
...
2. 在{{{ custom_key.brand_name }}}平台，「云账单 / 账单分析」中查看是否存在对应的账单信息
