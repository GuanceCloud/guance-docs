...
同步 阿里云-OSS 的监控数据，我们安装对应的采集脚本：「<<< custom_key.brand_name >>>集成（阿里云-OSS采集）」(ID：`guance_aliyun_oss`)
...
[配置自定义云对象指标](https://<<< custom_key.func_domain >>>/doc/script-market-guance-aliyun-oss/){:target="_blank"}
