...
- cmd：external 采集脚本路径
- args：Oracle 访问参数列表
  --host：Oracle 实例地址(ip)<br />
  --port：Oracle 监听端口<br />
  --username：Oracle 数据库用户名(填写前置条件中创建的用户名) <br />
  --password：Oracle 数据库密码 (填写前置条件中创建的用户密码) <br />
  --service-name：Oracle 的服务名<br />
  --query：自定义查询语句，格式为<sql:metricName:tags><br />
- envs：Oracle 环境变量
...
1、 如需采集 Oracle 的日志，目前可通过 [日志数据采集](/datakit/logging) ：`/usr/local/datakit/conf.d/log/logging.conf` 中添加如下 logging 配置 来实现：
...
logfiles：日志文件路径 (通常填写 Oracle 实例运行日志) <br /> ignore：过滤 \*.log 中不想被采集的日志(默认全采) <br /> source：来源标签（便于在构建日志视图时筛选该日志文件）<br /> character_encoding：日志文件的字符集(默认 utf-8) <br /> match：该配置为多行日志采集规则配置（以下配置文件中为 Oracle 实例运行日志规则）
