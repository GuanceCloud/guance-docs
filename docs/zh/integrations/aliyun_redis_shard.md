...
同步阿里云 Redis 集群版的监控数据，我们安装对应的采集脚本：「{{{ custom_key.brand_name }}}集成（阿里云- Redis采集）」(ID：`guance_aliyun_redis`)
...
在「管理 / 脚本市场」中点击并安装对应的脚本包：「{{{ custom_key.brand_name }}}集成（阿里云- Redis 慢查询日志采集）」(ID：`guance_aliyun_redis_slowlog`)
