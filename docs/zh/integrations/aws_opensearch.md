...
同步 AWS OpenSearch 的监控数据，我们安装对应的采集脚本：「<<< custom_key.brand_name >>>集成（AWS-OpenSearch采集）」(ID：`guance_aws_open_search`)
...
我们默认采集了一些配置, 具体见指标一栏 [配置自定义云对象指标](https://<<< custom_key.func_domain >>>/doc/script-market-guance-aws-open-search/){:target="_blank"}
