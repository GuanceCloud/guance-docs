...
程序启动后，DDTrace 会定期（默认 1 分钟上报一次）收集数据并上报给 Datakit，稍等几分钟后就可以在{{{ custom_key.brand_name }}}空间[应用性能监测 -> Profile](https://console.guance.com/tracing/profile){:target="_blank"} 查看相应数据。
...
如果没有发生错误，稍等一两分钟后即可在{{{ custom_key.brand_name }}}平台 [应用性能监测 -> Profile](https://console.guance.com/tracing/profile){:target="_blank"} 页面查看具体的性能指标数据。
