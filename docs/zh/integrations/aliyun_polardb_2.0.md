...
同步 PolarDB 分布式2.0 的监控数据，我们安装对应的采集脚本：「<<< custom_key.brand_name >>>集成（阿里云-PolarDB-X 2.0采集）」(ID：`guance_aliyun_polardbx_2`)
...
我们默认采集了一些配置, 具体见指标一栏[配置自定义云对象指标](https://<<< custom_key.func_domain >>>/doc/script-market-guance-aliyun-polardbx-1/){:target="_blank"}
