...
同步 阿里云 PolarDB MySQL 的监控数据，我们安装对应的采集脚本：「<<< custom_key.brand_name >>>集成（阿里云-PolarDB采集）」(ID：`guance_aliyun_polardb`)
...
配置[云数据库 PolarDB 慢查询明细](https://<<< custom_key.func_domain >>>/doc/script-market-guance-aliyun-polardb-slowlog-record/){:target="_blank"}
