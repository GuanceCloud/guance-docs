...
> 提示：使用本采集器前，必须安装「<<< custom_key.brand_name >>>集成 Core 核心包」及其配套的第三方依赖包

同步 DDoS 云资源的监控数据，我们安装对应的采集脚本：「<<< custom_key.brand_name >>>集成（阿里云-DDoS高防采集）」(ID：`guance_aliyun_ddoscoo`)
