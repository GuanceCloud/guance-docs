...
:material-numeric-1-circle-outline: 启用 / 禁用默认配置：

- 启用：若默认配置为启用状态，且当前工作空间内有新增的自定义等级，则您在创建或修改异常追踪时列出的等级选项为其合集；

- 禁用：若默认配置为禁用状态，则您在创建/修改异常追踪时仅能选择自定义等级配置。

    - 注意：禁用此选项时，若工作空间内仍然存在使用默认等级的 Issue，则您需要将仍然处于【Open】【Pending】的 Issue 调整为自定义等级，关联的监控器配置也同样需要调整。

:material-numeric-2-circle-outline: 编辑：点击编辑按钮，即可修改当前自定义等级的颜色、名称及描述。

:material-numeric-3-circle-outline: 删除：点击即刻删除当前等级。