...
3. 详细输入当前问题的描述，可艾特当前工作空间内的某个成员或输入 `#` 关联频道，快速查看该故障；
...
=== "监控"
    ...
    当您在配置监控器时，可通过**配置频道**的方式自动创建 Issue。以[配置阀值检测](../monitoring/monitor/threshold-detection.md#steps)为例:

    1、输入事件标题，默认为 Issue 标题；

    2、输入事件内容，默认为 Issue 描述；

    **注意**：如需在事件描述中 `@成员`，您可以在内容框上方点击 `@` 按钮、在内容框内输入 `@` 或直接在下方 `@` 选择下拉框点击选择通知成员。选定后，当事件创建完成，会自动向该成员发送邮箱通知。

    ![](img/issue-monitor-1.png)

    3、在**事件通知 > 频道**，点击频道下拉框，即可**多选当前频道**或**新建频道**。选定后，当事件创建完成，会将一同自动创建的 Issue 投递到此时您选定的频道显示。

    注意：**默认频道**即为**异常追踪**里的**全部**频道。

    ![](img/issue-monitor.png)

    4、由事件触发创建的 Issue，**来源**显示为当前监控器。**状态**默认 Open、**等级**默认 P2。

    5、当此监控器的事件通知规则配置完成后，Issue 将同步创建异常追踪，您可以前往**异常追踪 > 您选定的频道**进行查看。
