...
`last_over_time(series_selector[d])` 是一个聚合函数，它返回给定 `d` 时间窗口上给定 [series_selector](https://docs.victoriametrics.com/keyConcepts.html#filtering) 返回的每个时间序列上最后一个原始样本值。
...
> 该函数由 PromQL 支持。另请参见 [range_linear_regression](https://docs.victoriametrics.com/MetricsQL.html#range_linear_regression)。
...
`resets(series_selector[d])` 是一个聚合函数，它返回给定 `d` 时间窗口上给定 [series_selector](https://docs.victoriametrics.com/keyConcepts.html#filtering) 返回的每个时间序列上的计数器重置次数。预期 `series_selector` 返回计数器类型的时间序列。
...
`tlast_over_time(series_selector[d])` 是一个[ Rollup 函数](https://docs.victoriametrics.com/MetricsQL.html#rollup-functions)，它是`timestamp`的一个别名。
...
`ceil(q)` 是一个[转换函数](https://docs.victoriametrics.com/MetricsQL.html#transform-functions)，它将由 `q` 返回的每个时间序列的每个点四舍五入到最接近的整数。
...
`log10(q)` 是一个 [转换函数](https://docs.victoriametrics.com/MetricsQL.html#transform-functions)，它为 `q` 返回的每个时间序列的每个点计算`log10(v)`。
...
**标签操作函数**用于对选定的[聚合结果](https://docs.victoriametrics.com/MetricsQL.html#rollup-functions)执行标签操作。

更多细节：

- 如果标签操作函数被直接应用于 [序列选择器](https://docs.victoriametrics.com/keyConcepts.html#filtering)，则在执行标签转换之前会自动应用 [default_rollup()](https://docs.victoriametrics.com/MetricsQL.html#default_rollup) 函数。例如，`alias(temperature, "foo")` 会隐式转换为 `alias(default_rollup(temperature[1i]), "foo")`。
...
`any(q) by (group_labels)` 是 [聚合函数](https://docs.victoriametrics.com/MetricsQL.html#aggregate-functions)，它从 `q` 返回的时间序列中每个 `group_labels` 返回一个序列。

> 另请参见[group](https://docs.victoriametrics.com/MetricsQL.html#group)。

#### avg

`avg(q) by (group_labels)` 是 [聚合函数](https://docs.victoriametrics.com/MetricsQL.html#aggregate-functions)，它返回 `q` 的时间序列每个 `group_labels` 的平均值。聚合将针对具有相同时间戳的点的每个组单独计算。

> 此函数由 PromQL 支持。

#### bottomk

`bottomk(k, q)` 是 [聚合函数](https://docs.victoriametrics.com/MetricsQL.html#aggregate-functions)，它返回 `q` 的所有时间序列中具有最小值的 `k` 个点。聚合将针对具有相同时间戳的点的每个组单独计算。

> 此函数由 PromQL 支持。另请参见 [topk](https://docs.victoriametrics.com/MetricsQL.html#topk)。

#### bottomk_avg

`bottomk_avg(k, q, "other_label=other_value")` 是 [聚合函数](https://docs.victoriametrics.com/MetricsQL.html#aggregate-functions)，它返回 `q` 中平均值最小的 `k` 个时间序列。如果设置了可选的 `other_label=other_value` 参数，则返回其余时间序列的总和和所给标签。例如，`bottomk_avg(3, sum(process_resident_memory_bytes) by (job), "job=other")` 将返回平均值最小的3个时间序列和具有任何余数的 `{job="other"}` 标签的时间序列。

> 另请参见 [topk_avg](https://docs.victoriametrics.com/MetricsQL.html#topk_avg)。

#### bottomk_last

`bottomk_last(k, q, "other_label=other_value")` 是 [聚合函数](https://docs.victoriametrics.com/MetricsQL.html#aggregate-functions)，它返回 `q` 中最后值最小的 `k` 个时间序列。如果设置了可选的 `other_label=other_value` 参数，则返回剩余时间序列的总和和所给标签。例如，`bottomk_max(3, sum(process_resident_memory_bytes) by (job), "job=other")` 将返回最大值最小的3个时间序列和具有任何余数的 `{job="other"}` 标签的时间序列。

> 另请参见 [topk_last](https://docs.victoriametrics.com/MetricsQL.html#topk_last)。

#### bottomk_max

`bottomk_max(k, q, "other_label=other_value")` 是 [聚合函数](https://docs.victoriametrics.com/MetricsQL.html#aggregate-functions)，它返回 `q` 中最大值最小的 `k` 个时间序列。如果设置了可选的 `other_label=other_value` 参数，则返回剩余时间序列的总和和所给标签。例如，`bottomk_max(3, sum(process_resident_memory_bytes) by (job), "job=other")` 将返回最大值最小的3个时间序列和具有任何余数的 `{job="other"}` 标签的时间序列。

> 另请参见 [topk_max](https://docs.victoriametrics.com/MetricsQL.html#topk_max)。

#### bottomk_median

`bottomk_median(k, q, "other_label=other_value")` 是 [聚合函数](https://docs.victoriametrics.com/MetricsQL.html#aggregate-functions)，它返回 `q` 中中位数最小的 `k` 个时间序列。如果设置了可选的 `other_label=other_value` 参数，则返回剩余时间序列的总和和所给标签。例如，`bottomk_median(3, sum(process_resident_memory_bytes) by (job), "job=other")` 将返回中位数最小的3个时间序列和具有任何余数的 `{job="other"}` 标签的时间序列。

> 另请参见 [topk_median](https://docs.victoriametrics.com/MetricsQL.html#topk_median)。

#### bottomk_min

`bottomk_min(k, q, "other_label=other_value")` 是 [聚合函数](https://docs.victoriametrics.com/MetricsQL.html#aggregate-functions)，它返回 `q` 中最小值最小的 `k` 个时间序列。如果设置了可选的 `other_label=other_value` 参数，则返回剩余时间序列的总和和所给标签。例如，`bottomk_min(3, sum(process_resident_memory_bytes) by (job), "job=other")` 将返回最小值最小的3个时间序列和具有任何余数的 `{job="other"}` 标签的时间序列。

> 另请参见 [topk_min](https://docs.victoriametrics.com/MetricsQL.html#topk_min)。

#### count

`count(q) by (group_labels)` 是 [聚合函数](https://docs.victoriametrics.com/MetricsQL.html#aggregate-functions)，它返回 `q` 的非空点每个 `group_labels` 的数量。聚合将针对具有相同时间戳的点的每个组单独计算。

> 此函数由 PromQL 支持。

#### count_values

`count_values("label", q)` 是 [聚合函数](https://docs.victoriametrics.com/MetricsQL.html#aggregate-functions)，它计算具有相同值的点的数量，并将计数存储在具有每个初始值的附加 `label` 的时间序列中。聚合将针对具有相同时间戳的点的每个组单独计算。

> 此函数由 PromQL 支持。

#### distinct

`distinct(q)` 是 [聚合函数](https://docs.victoriametrics.com/MetricsQL.html#aggregate-functions)，它计算具有相同时间戳的每个组的唯一值的数量。

#### geomean

`geomean(q)` 是 [聚合函数](https://docs.victoriametrics.com/MetricsQL.html#aggregate-functions)，它计算具有相同时间戳的每个组的几何平均值。

#### group

`group(q) by (group_labels)` 是 [聚合函数](https://docs.victoriametrics.com/MetricsQL.html#aggregate-functions)，它为 `q` 返回的时间序列中每个 `group_labels` 返回 `1`。
...
- 所有未包装在 Rollup 函数内的系列选择器都会自动包装到 `default_rollup` 函数中。例如：

    - `foo` 会转换为 `default_rollup(foo[1i])`
    - `foo + bar` 会转换为 `default_rollup(foo[1i]) + default_rollup(bar[1i])`
    - `count(up)` 会转换为 `count(default_rollup(up[1i]))`，因为 [count](https://docs.victoriametrics.com/MetricsQL.html#count) 不是 [rollup 函数](https://docs.victoriametrics.com/MetricsQL.html#rollup-functions) ，而是 [aggregate 函数](https://docs.victoriametrics.com/MetricsQL.html#aggregate-functions)
    - `abs(temperature)` 会转换为 `abs(default_rollup(temperature[1i]))`，因为 [abs](https://docs.victoriametrics.com/MetricsQL.html#abs) 不是 [rollup 函数](https://docs.victoriametrics.com/MetricsQL.html#rollup-functions)，而是 [transform 函数](https://docs.victoriametrics.com/MetricsQL.html#transform-functions)
