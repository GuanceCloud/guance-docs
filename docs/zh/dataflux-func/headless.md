...
**注意**：HTTP 服务地址：即 DataKit 的 HTTP 服务地址，通过该地址接收外部数据，在接入应用时需要填写：
...
1. 点击**修改**；
2. 获取并填入邮箱验证码；
3. 点击确定，完成**身份验证**，即可修改当前 HTTP 服务地址。
...
若您之前开通过 RUM Headless，应用到期后，所有数据会为您保留 7 天，到期释放。在这 7 天内，若需要再次开通应用，您可以选择**恢复所有数据**或者**不恢复数据**：
