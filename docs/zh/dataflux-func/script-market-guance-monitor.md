...
![](script-market-guance-monitor/self-hosted-monitor-list.png)
...
![](script-market-guance-monitor/self-hosted-monitor-edit.png)

## 4. 在自建 DataFlux Func 中配置自动触发配置

完成代码，并发布后，需要前往自建 DataFlux Func「管理 / 自动触发配置」，为函数创建自动触发配置后，函数才会实际运行。

![](script-market-guance-monitor/self-hosted-monitor-cron-config.png)

*注意：自建巡检的参数在观测云中配置，「自动触发配置」中的参数指定不起作用*

执行一段时间后，即可在观测云查看生成的事件。

![](script-market-guance-monitor/self-hosted-monitor-event.png)
...
![](script-market-guance-monitor/self-hosted-monitor-disabled.png)
