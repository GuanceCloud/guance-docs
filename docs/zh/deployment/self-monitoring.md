...
```yaml
   - name: ENV_DATAWAY
     value: https://openway.<<< custom_key.brand_main_domain >>>?token=tkn_a624xxxxxxxxxxxxxxxxxxxxxxxx74 ## 此处填上 dataway 真实地址
   ...
   image: pubrepo.<<< custom_key.brand_main_domain >>>/datakit/datakit:1.65.2     ## 修改成最新镜像版本
```
