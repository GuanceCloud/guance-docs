...
**数据保存策略**：即当前工作空间的数据保留时长。包含**指标、日志、备份日志、事件、用户性能、安全巡检、应用访问**七大功能模块。可按需选择。


### 存储类型

此处提供日志易、SLS 存储及 Elasticsearch 存储三种非指标类型数据存储引擎。按需选中某一数据引擎后，
日志、备份日志、应用性能、安全巡、事件数据将保存到当前选中的存储引擎内。



若您不选择以上数据存储引擎，则数据会保存到主存储引擎内。
...
**自定义索引**：默认关闭，开启后，允许该工作空间创建多个索引策略用于保存日志；
