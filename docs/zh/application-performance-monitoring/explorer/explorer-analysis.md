...
在链路查看器的图表统计中，您可以查看所选时间范围内链路不同状态的以下关键指标：

- 请求数 
- Error 请求数  
- 响应时间  

这些指标可通过筛选功能同步展示在图表中。具体统计方式如下：

- 请求数/Error 请求数：根据所选时间范围，系统将其划分为 60 个时间点，并通过柱状图展示链路的请求数和 Error 请求数。  
- 响应时间：同样根据所选时间范围划分为 60 个时间点，通过折线图展示四个响应时间指标，包括每秒平均响应时间、P75 响应时间、P90 响应时间和 P99 响应时间。

### 导出 {#export}

筛选出所需链路数据后，您可以将数据导出为以下格式进行查看和分析：
