...
在观测云控制台，您可以在**应用性能监测**的**链路**，查看全部链路数据。您可以搜索、筛选、导出链路数据，查看链路详情并通过火焰图、Span 列表等对链路性能进行全量分析，无论是同步或异步的调用，都能够清晰追踪每一个链路性能的数据细节。
...
在链路查看器[分析栏](../getting-started/function-details/explorer-search.md#analysis)，支持基于 <u>1-3 个标签</u>进行多维度分析统计，并支持多种数据图表分析方式。
...
在链路查看器图表统计，您可以查看所选时间范围内链路不同状态的**请求数**、**Error 请求数**、**响应时间**，支持通过筛选同步展示图表统计。

- 请求数/Error 请求数：根据所选时间范围划分为 60 个时间点，用柱状图统计展示链路的**请求数**以及 **Error 请求数**。
- 响应时间：根据所选时间范围划分为 60 个时间点，用折线图统计展示四个响应指标，分别为“每秒平均响应时间”、**P75 响应时间**、“P90响应时间”以及**P99 响应时间**
...
观测云支持**查看历史快照**直接保存当前查看器的快照数据，通过[快照](../management/snapshot.md)功能，您可以快速复现即时拷贝的数据副本，将数据恢复到某一时间点和某一数据展示逻辑。
...
在链路详情页，你可以在**扩展属性**查看当前链路的相关字段属性：
...
=== "关联日志"
    ...
    在链路详情页，您可以通过**日志**查看基于当前链路关联的日志（关联字段：`trace_id`）。您可以对日志进行关键字搜索和多标签筛选，日志内容默认按照日志查看器的**最大显示行数**、**显示列**的配置展示，您可以自定义显示列，若您需要查看更详细的日志内容，可以点击日志内容跳转到日志详情页面，或点击跳转按钮到日志页打开。
...
=== "关联主机"
    ...
    ???+ attention
        ...
        观测云默认保存主机对象最近 48 小时的历史数据。未找到当前链路时间对应的主机历史数据时，您将无法查看关联主机的属性视图。
...
=== "关联网络"
    ...
    观测云支持您通过详情页下方的**网络**，查看 48 小时内的[网络](../infrastructure/network.md)数据连接情况。包括 Host 、Pod 、Deployment 和 Service。
