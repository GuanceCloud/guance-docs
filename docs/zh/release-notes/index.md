...
观测云调整变更数据存储的策略，您可以在一天内重复多次变更指标、日志、备份日志、事件、应用性能、用户访问的数据存储时长，观测云会保存您的修改记录，其中第一次修改当天生效，其他将以最后一次修改记录在次日生效。关于如何变更，可参考文档 [数据存储策略](../billing/billing-method/data-storage.md) 。

#### 图表链接设置优化

观测云优化图表链接的配置方式，您可以通过选择预置的关联链接，直接在对话框中选择需要添加的参数，并填入参数值，即可简单快速的配置图表的关联链接地址。更多详情可参考文档 [自定义链接](../scene/visual-chart/chart-link.md#custom-link) 。

![](img/6.link_5.1.png)

#### 新增支持创建重名的仪表板、笔记、自定义查看器

- 在观测云「场景」，您可以通过新建、导入、复制等操作创建重名的仪表板、笔记、自定义查看器；
- 在观测云「管理」-「设置」-「配置迁移」，您可以一键导入重名的仪表板、自定义查看器和监控器，观测云会提供重名提示，您可以根据实际的需求来选择是否“跳过”、“仍然创建”和“取消”。更多详情可参考文档 [配置迁移](../management/index.md#export-import) 。

![](img/5.input_rename_1.png)

##### DQL 查询时间范围优化

观测云新增支持以 [xx:xx:xx] 的格式添加数据查询的时间范围，您可以在 DQL 查询工具以及图表、指标、视图变量的 DQL 查询模式中添加时间范围来查询相应的数据。
...
在观测云日志查看器，显示列的显示行数新增“全部”选项，您可以在显示行数选择“全部”来一键展开查看超长的日志 Message 数据。
...
在配置监控器事件通知时，新增无数据通知配置，您可以自定义无数据通知的内容，若没有配置，则自动使用官方默认的通知模版。更多详情可参考文档 [监控器配置](../monitoring/monitor/threshold-detection.md#notification) 。

##### 事件通知内容配置新增预置的跳转链接

在配置监控器的事件通知内容时，新增预置的跳转链接，您可以根据需求配置跳转链接来查看当前告警事件相关的日志、链路、错误追踪、Profile、容器、Pod、进程数据以及关联的仪表板。更多详情可参考文档 [监控器配置](../monitoring/monitor/threshold-detection.md#notification) 。

#### SSO 单点登录优化

- SSO 新增登录用户 [账号管理](../management/sso/index.md#account)；
- SSO 登录映射规则配置优化，在配置 “Email” 字段映射规则时兼容大小写；
- SSO 登录链接获取范围调整，对于 SSO 已存在的用户，会先判断用户是否存在，然后获取用户所在且域名匹配的工作空间登录链接列表。
...
- [注册观测云商业版](../billing/commercial-register.md) 时，开通方式不再受站点限制，您可以任意选择 “[观测云直接开通](../billing/billing-account/enterprise-account.md)”、“[阿里云市场开通](../billing/billing-account/aliyun-account.md)“和“[亚马逊云市场开通](../billing/billing-account/aws-account.md)“三种方式；
- 在注册观测云或创建工作空间时，新建的工作空间默认不再支持左 * 查询，您可以联系客户经理进行调整；
