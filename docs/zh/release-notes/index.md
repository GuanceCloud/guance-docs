...
- 创建工作空间时新增[菜单风格选择](../management/index.md#create)，支持选择不同的工作空间风格属性，从而定制化个人菜单目录。
- 优化日志数据访问[权限相关规则](../logs/logdata-access.md#config)适配，进一步明确多角色数据查询权限及权限控制对应关系。
- 事件详情页下的[基础属性](../events/event-details.md#attribute)新增关联查询，支持查看全部相关检测维度内容。
- 告警策略管理支持[不同级别告警到不同的通知对象、同一个告警不同级别能到不同的通知对象](../monitoring/alert-setting.md#create)。
- 成员管理新增添加[昵称备注](../management/member-management.md#batch)功能，规范工作空间内的成员用户名，支持通过昵称备注搜索成员。
