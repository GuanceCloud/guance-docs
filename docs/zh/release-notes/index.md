...
基于新增的角色权限管理，调整管理员提权的触发条件，当用户角色被赋予 Token 的查看、操作权限，即触发费用中心的提权审核。关于如何提升普通成员为管理员，可参考文档 [提权为管理员](../management/role-management.md#upgrade) 。
...
- 无操作登录会话保持时间：支持设置范围 30 ～ 1440 分钟；
- 登录会话最大保持时间：支持设置范围 0 ～ 7 天，其中 0 表示永不超时。
...
在「监控」 - 「监控器」/「智能巡检」，选择一个已经存在的监控器/智能巡检，点击右侧的「编辑」按钮，即可订阅监控器/智能巡检的告警通知。更多详情可参考文档 [订阅管理](../monitoring/subscribe.md) 。
...
- 工作空间锁定倒计时优化：14天内倒计时，用户可以关闭倒计时的弹出框，继续使用观测云；14天倒计时结束后，用户的数据将被一键清除，用户无法继续使用观测云，支持用户通过费用中心进行解锁后恢复使用；
- 用户访问监测增加 Session、View、Resource、Action、Long Task、Error 不同查看器的颜色标识。
