...
用户访问监测 Session、View、Error 查看器新增会话重放功能，可通过视频的方式查看用户的操作行为，帮助用户演示单击、鼠标移动、页面滚动等操作内容，重现发生的用户访问问题。更多详情可参考文档 [会话重放](../real-user-monitoring/session-replay.md) 。

![](img/.png)

##### 新增“浏览器插件”自动化追踪

RUM 追踪新增 “浏览器插件”的实现方式，使用浏览器记录用户访问行为，创建无代码的端到端测试，更多详情可参考文档 [自动化追踪](../real-user-monitoring/self-tracking.md#auto-tracking) 。

![](img/.png)

##### 调整 RUM 的查看器/视图的显示逻辑

- 用户访问监测应用列表显示调整，支持在应用列表跳转查看当前应用的“分析看板”和“查看器”内容详情；
- 用户访问监测顶部导航菜单“查看器”支持查看所有应用的用户访问数据，您可以通过筛选 “应用ID” 来筛选需要查看器的数据；
- 用户访问监测顶部导航菜单“分析看板“支持切换查看 Web 端、移动端、小程序的场景分析视图。

![](img/.png)

##### 新增 CDN 厂商信息采集

新增 CDN 厂商信息采集，支持通过创建统计 CDN 数据的图表，帮助用户分析不同厂商 CDN 的加速效果，更多采集配置，可参考 [Rum 采集器配置](../../datakit/rum.md) 。
...
在场景查看器，新增自定义配置查看器导航及展示内容，包括事件、基础设施、指标、日志、应用性能监测、用户访问监测、可用性监测、安全巡检、CI 可视化等查看器。

在查看器列表，点击右侧「操作」菜单下的「编辑」，选择「添加至菜单」，即可添加当前查看器至对应的查看器进行内容编辑展示。

![](img/.png)

##### 优化场景视图变量级联功能

场景视图变量优化级联功能，新增支持 `match（re）` 、`not match（re）` 、`wildcard` 、`not wildcard` 多种逻辑运算符。更多详情可参考文档 [视图变量](../scene/view-variable.md) 。

![](img/.png)

##### 优化饼图，新增合并配置选项

饼图新增 “合并” 的配置选项，支持用户配置占比比例大小判断是否合并到 “其他” 显示。若饼图的数据点较多，可使用合并来聚合数据点到“其他”的切片，提高饼图的可读性，配置合并以后：
- 饼图切片增加 “其他” 的切片，表示被合并的数据汇总显示
- 图例增加 “其他” 的图例项，表示被合并的数据汇总数值及占比显示

![](img/.png)

#### 其他功能优化
- 优化注册流程，支持绑定已有观测云费用中心账号，更多详情可参考文档 [注册商业版](../billing/commercial-register.md) 
- 去掉配置监控器时，检测指标查询中「检测维度」的必选逻辑，提高查询性能
