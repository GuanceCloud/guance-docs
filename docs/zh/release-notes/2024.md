...
2. 查看器[自动刷新](../getting-started/function-details/explorer-analysis.md#refresh)逻辑调整：在时间组件中去除“暂停”按钮，刷新频率选项里新增 “off”，用户可按需关闭自动刷新功能，操作更简洁直观。同时取消个人设置中的“查看器自动刷新”开关，避免功能冗余，优化用户设置体验。
...
10. 解决了自研时序数据库升级到最新版本后 DQL 不支持 tag 计算的问题。
...
整合了帮助中心页面，允许用户通过统一入口访问所有[集成列表](<<< homepage >>>/integrations/integration-index/)，并支持搜索及标签过滤功能，以提高查找效率。
...
- 监控
  ...
  - [静默管理](../monitoring/mute-management.md)：
...
- 查看器 > [快捷筛选](../getting-started/function-details/explorer-filter.md#quick-filter)：
...
- 监控：
  ...
  - 通知对象管理：新增[简单 HTTP 通知类型](../monitoring/notify-target.md#http)，直接通过 Webhook 地址接收告警通知；
- 场景：
