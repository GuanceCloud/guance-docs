...
| `view.loading_time`             | number(ns)   | 页面已经 ready，并且没有任何网络请求和 DOM 变动。<br/>> 参见 [页面 Loading Time](./page-performance.md)。                                                                                                                                                 |
| `view.largest_contentful_paint` | number(ns)   | 最大内容绘制，测量加载性能。为了提供良好的用户体验，LCP 应在页面首次开始加载后的 2.5 秒内发生。                                                                                                                                                           |
...
| `view.first_contentful_paint`   | number(ns)   | 首次内容绘制 (FCP) 指标测量页面从开始加载到页面内容的任何部分在屏幕上完成渲染的时间。对于该指标，"内容"指的是文本、图像（包括背景图像）、`<svg>` 元素或非白色的 `<canvas>` 元素。<br/>> 可参考 [w3c](https://www.w3.org/TR/paint-timing/#sec-terminology) |
