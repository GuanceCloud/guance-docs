...
通过采集 Web、Android、iOS、小程序和第三方框架的用户访问数据，观测云用户访问监测帮助您快速监测用户的使用行为和遇到的问题。通过对用户访问数据的实时查看和分析，您可以快速了解用户访问环境、回溯用户的操作路径、分解用户操作的响应时间以及查看用户操作导致的一系列调用链的应用性能指标情况。

![](img/rum-arch_1.png)

## 前置条件

要开启用户访问监测功能，<u>首先需要部署一个公网 DataKit 作为 Agent</u>，客户端的用户访问数据通过这个 Agent，将数据上报到观测云工作空间。
