...
- **DQL 查询：**支持用户直接手写 DQL 语句，查询返回对应的数据值，支持在查询中添加时间范围，支持配置级联查询；
- **UI 查询：**支持选择指标、基础对象、自定义对象、日志、应用性能、用户访问、安全巡检数据来源，通过直接在 UI 上点击选择查询变量，查询返回对应的数据值，不支持添加时间范围和配置级联查询；
- **自定义：**支持直接定义一组数值用于视图变量使用，无需通过查询语句获取相关值，不支持添加时间范围和配置级联查询；

#### DQL 查询

在 DQL 查询过程中，支持用户直接手写 DQL 语句，查询返回对应的数据值，您可以在查询中添加时间范围、配置级联查询。([了解更多DQL函数说明](../dql/funcs.md))
...
| show_tag_value(from=['指标集名'], keyin=['标签名']) LIMIT 50 | 返回指定指标集下某个标签的 50 个值，该函数不支持添加时间筛选 |
...
> 指标查询可用 [Show 函数](#show) 来查询数据，不支持添加时间范围筛选。
...
> 基础设施查询可用 [DQL 语句查询](#dql) 来查询数据，支持在查询中添加时间范围筛选，跟随仪表板时间控件联动。例如：`O::kubernetes_nodes:(DISTINCT_BY_COLLAPSE(namespace)) LIMIT 50` 表示选定时间内基础对象数据中某个标签的 50 个值。
...
> 自定义对象查询可用 [DQL 语句查询](#dql) 来查询数据，支持在查询中添加时间范围筛选，跟随仪表板时间控件联动。例如：`CO::cdsmnl:(DISTINCT_BY_COLLAPSE(cds)) LIMIT 50` 表示选定时间内自定义对象数据中某个标签的 50 个值 。
...
> 日志查询可用 [DQL 语句查询](#dql) 来查询数据，支持在查询中添加时间范围筛选，跟随仪表板时间控件联动。例如：`L::RE(.*):(DISTINCT_BY_COLLAPSE(trace_id)) LIMIT 50` 表示选定时间内日志数据中某个标签的 50 个值。
...
> 应用性能查询可用 [DQL 语句查询](#dql) 来查询数据，支持在查询中添加时间范围筛选，跟随仪表板时间控件联动。例如：`T::RE(.*):(DISTINCT_BY_COLLAPSE(resource)) LIMIT 50` 表示选定时间内链路数据中某个标签的 50 个值。
...
> 用户访问查询可用 [DQL 语句查询](#dql) 来查询数据，支持在查询中添加时间范围筛选，跟随仪表板时间控件联动。例如：`R::error:(DISTINCT_BY_COLLAPSE(province)) LIMIT 50` 表示选定时间内用户访问数据中某个标签的 50 个值。
...
> 安全巡检查询可用 [DQL 语句查询](#dql) 来查询数据，支持在查询中添加时间范围筛选，跟随仪表板时间控件联动。例如：`S::RE(.*):(DISTINCT_BY_COLLAPSE(rule)) LIMIT 50` 表示选定时间内安全巡检数据中某个标签的 50 个值。
...
若工作空间无数据上报，在配置变量查询时无法选择数据，可在查询中预设字段。
