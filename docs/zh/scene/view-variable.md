...
1. 数据来源：当前视图变量可选的数据范围，包括 DQL、指标、基础对象、自定义对象、日志、应用性能、用户访问、安全巡检和自定义。 

2. [变量查询](#query)：当前视图变量使用的查询方式，包括 DQL 查询、PromQL 查询、UI 查询和自定义。 

3. 默认值：当前视图变量在仪表板中的初始变量。支持预览当前视图变量的全部变量，并选择在仪表板默认查看的初始变量，若默认值为空，则在仪表板显示最新的字段数据。支持选择 **All 所有值**选项，将列表所有获取到的值当作变量传参。

    **注意**：

    - 默认值列表最多显示 50 个变量，支持搜索查找更多。

    - 如果列表值内存在 `\ "` 等特殊字符情况下，自动追加 `\` 转义，保证实际查询结果。

4. 变量名：当前视图变量集合所属的变量名称，在仪表板中添加图表时，需要在图表查询中引用，格式为：#{变量名}。 

5. 显示名：当前视图变量集合在仪表板中显示的名称，例如：变量名为 `host`，显示名为 `主机名`。 
