...
观测云的矩形树图用于显示不同分组下指标数据的占比分布情况。例如您可以查看不同主机的 CPU 使用率情况。

## 图表查询

图表查询支持**简单查询**、**表达式查询**、**DQL 查询**。您可以添加多个查询，但 by（分组）的 Tag 必须一致，修改一个，另外的自动同步修改。查询结果预设 5 种返回数量，包括 5、10、20、50、100，默认返回 20 条数据。支持手动输入，最多包含 100 条数据。
...
| 选项 | 说明 |
| --- | --- |
| 指标 | 颜色表示分组的标签（Tag），面积表示指标数据。当添加了多个指标查询，可选择主要展示的指标，面积大小根据选择的指标数据结果、Top/Bottom 以及数量进行展示。 |
...
| 选项 | 说明 |
| --- | --- |
| 时间分片 | 开启时间分片后，会先对原始数据按照一定的时间间隔进行分段聚合，再对聚合后数据集进行第二次聚合得到结果值，默认关闭。<br />详情可参考 [时间分片说明](chart-query.md#time-slicing)。 |
