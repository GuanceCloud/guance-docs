...
图表查询支持**简单查询**、**表达式查询**、**DQL 查询**，图表查询条件详细说明请点击 [图表查询](chart-query.md) 进行查看，默认添加简单查询。可以添加多个查询，但 by（分组）的 Tag 必须一致，修改一个，另外的自动同步修改。查询结果预设 5 种返回数量，包括 5、10、20、50、100 ，默认返回 20 条数据，支持手动输入，最多为 100 条数据。
