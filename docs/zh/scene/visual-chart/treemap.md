...
用于显示不同分组下指标数据的占比分布情况，可展示不同类别的占比情况，有效利用空间，相比饼图可以显示更多数据。
...
支持**简单查询**、**表达式查询**、**DQL 查询**、**PromQL 查询**和**数据源查询**。

您可以添加多个查询，但 by（分组）的 Tag 必须一致，修改一个，另外的自动同步修改。查询结果预设 5 种返回数量，包括 5、10、20、50、100，默认返回 20 条数据。支持手动输入，最多包含 100 条数据。
