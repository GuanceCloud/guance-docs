...
基于<<< custom_key.brand_name >>>配备的大模型自然语言生成能力，可使用“自动生成”功能，快速生成贴合语义的标题和描述。
...
=== "全局"
    ...
    包含不同维度的通用单位格式，如数据大小、时间间隔、时间戳等。可在下拉框中输入自定义单位。

    ---

=== "自定义"

    选择当前查询条件下包含的指标，输入自定义单位。
...
系统自动列出当前图表查询下的所有指标与分组。添加别名后，图表的分组部分将根据别名值动态更新。别名支持使用模板变量，实现一键替换。例如：

- `{{tags}}`：替换为所有“标签名”和“标签值”；    
...
???+ warning "注意"
    ...
    - 指标部分的别名由查询语句右侧的 `AS` 决定；
    
    <img src="../../img/as_alias.png" width="60%" > 
    
    - 当您的查询中存在 by 条件，则显示返回的所有图例序列；
    - 若在别名中同时设置指标部分和分组部分，此时设置优先级为：分组别名 > 指标别名。

#### 场景示例

如下图，分别查询主机 CPU 的总使用率（`usage_total`）和用户态使用率（`usage_user`）。基于前者，别名的设置会出现以下场景：
...
支持选择隐藏、底部、右侧图例，点击图例中的指标行可隐藏/显示对应指标。

##### 显示值

选择要在图例中显示的值或计算值。

时序图图例值会根据图表查询结果进行再次聚合计算，支持选择 `Last（最后一个值）`、`First（第一个值）`、`Avg（平均值）`、`Min（最小值）`、`Max（最大值）`、`Sum（求和）`、`Count（返回点数）`。
...
???+ warning "注意"
    ...
    `Avg`、`Sum`、`Count` 根据选择的时间间隔（原始间隔和自动对齐）不同，显示不同的结果。

### 数据格式 {#data_format}

1. 小数位数：可选择 0、1、2、3 位小数或全精度。

2. 千分位分隔符：默认开启；关闭后将显示原始值，无分隔符。
...
???+ warning "注意"
    ...
    - 该设置仅针对相对时间生效，若查询时间区间为“绝对时间范围”，时间偏移不生效。
    - 针对有时间间隔的图表，如时序图，设定时间间隔超出 1min 则时间偏移不生效，<= 1m 的情况下才偏移生效。针对没有时间间隔的图表，如概览图、柱状图等，时间偏移保持生效。
