...
| 显示列 | <li>仅时序表格图支持；<br/><li>支持用户设置查询数据显示列，同时支持用户自定义手输列名；查询结果值支持作为[值变量](chart-link.md#z-variate)做链接跳转使用；<br/><li>支持用户在编辑模式下手动调整列宽，保存后可根据列宽展示列表信息；支持**拖拽**调整显示列顺序，列表根据配置顺序依次展示。 |
