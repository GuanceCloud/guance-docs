...
1）`&tags=#{V}`  ，链接中传递所有视图变量的条件；

2）`&tags={"version":"{V.version}"}`，链接中仅传视图变量 version 的条件；

3）`&tags={"host":"#{V.host}"}`，链接中传递标签 host 的条件，host 值即为当前视图变量 host 的选值。
...
<font color=coral>**注意：**</font>内置链接**不支持存在筛选条件或视图变量的查询**。当您的图表查询存在筛选条件或视图变量时，您可以通过「链接」自定义链接。
