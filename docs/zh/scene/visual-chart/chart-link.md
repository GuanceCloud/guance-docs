...
在「仪表板」编辑模式下，选择「图表」-「链接」，即可查看可使用的模板变量。

1）在网址链接后面输入变量模板，如<br />`https://console.guance.cn/logndi/log?time=#{TR}&tags={"version":"#{V.version}"}`

- `https://console.guance.com/logIndi/log/?`为网址链接
- `time=#{TR}&tags={"version"="#{V.version}"}`为模板变量的内容。
...
**步骤三：按照图表链接模板变量修改链接**<br />根据链接的模板变量规则修改链接中的变量值，包括时间变量、标签变量和视图变量。打开方式选择“划出详情页”，别名填入“主机监控视图”。
...
**步骤三：按照图表链接模板变量修改链接**<br />根据链接的模板变量规则修改链接中的变量值，包括时间变量和视图变量（这里也可以使用标签变量`#{T.host}`）。打开方式选择“划出详情页”，别名填入“主机基础设施”。
