...
{{{ custom_key.brand_name }}}支持图表内置关联链接和自定义关联链接，可以帮助您实现从当前图表跳转至目标页面，并通过模板变量中对应的变量值传送数据信息，实现数据联动。

## 变量说明

{{{ custom_key.brand_name }}}支持 4 种模板变量：
...
| #{startTime} | 若未锁定时间，表示当前图表右上角时间控件中的开始时间。<br />若已锁定时间，表示锁定时间设置中的开始时间。  |
| #{endTime} | 若未锁定时间，表示当前图表右上角时间控件中的结束时间。<br />若已锁定时间，表示锁定时间设置中的结束时间。  |

**注意**：在进行实际查询时，可利用时间变量 `#{startTime}`和 `#{endTime}` 以及视图变量作为占位符。在图表查询实际执行时，系统将依据全局设置的这些变量值做最终替换。
...
即{{{ custom_key.brand_name }}}默认为图表提供的关联链接，主要基于当前查询的时间范围和分组标签，帮助您查看对应的日志、进程、容器、链路、主机监控视图。
...
在添加图表链接时，{{{ custom_key.brand_name }}}提供预设链接，帮助您简单快速配置链接地址。
...
{{{ custom_key.brand_name }}}支持三种链接打开方式，分别为**新页面**、**当前页面**和**划出详情页**。
...
{{{ custom_key.brand_name }}}支持对图表链接进行以下操作：

1. 开启/关闭显示：用于控制是否在图表上显示关联链接。   
2. 编辑：支持对已添加的链接进行修改。      
3. 删除：用于删除当前自定义的链接，链接删除后将无法恢复，请谨慎操作。       
4. 恢复为默认：支持对修改过的内置链接还原到初始默认状态。 
...
**前提**：已经在{{{ custom_key.brand_name }}}仪表板下完成图表创建，现在需要为图表添加链接。
...
=== "步骤一：添加图表链接"
    ...
    在图表链接中，输入名称“基础设施主机查看器”，粘贴在{{{ custom_key.brand_name }}}基础设施主机复制的链接地址，并根据需要展示的效果添加或修改模版变量参数。选择打开方式为**侧划页**。
