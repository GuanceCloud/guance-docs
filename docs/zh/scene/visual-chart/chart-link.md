...
| 标签变量 | #{T} | 当前图表查询的所有分组标签，是将当前图表查询的全部分组标签作为查询条件。<br />假设当前图表查询为：<br />`M::'datakit':(LAST('cpu_usage')) { 'vserion' = '1.0.0-rc1' } BY 'host', 'vserion', 'os'`<br />假设当前标签变量为："host"="abc"、"version"="V1.7.0" 和 "os"="linux"<br />模板变量：`&tags=#{T} `同为 `&tags={"host":"abc","version":"V1.7.0","os":"linux"}` |
...
???+ attention
    ...
    - 内置链接**不支持存在视图变量的查询**。当您的图表查询存在视图变量时，您可以通过「链接」自定义链接

## 自定义链接

观测云支持为图表添加自定义链接，通过配置模板变量筛选数据，帮助您实现从当前图表跳转至目标页面进行数据关联查询。自定义链接添加以后默认开启显示，可直接在图表预览中显示相关链接。
...
在链接地址，支持添加模版变量，包括时间变量、标签变量和视图变量。
...
| tags         | 标签筛选，一般用于查看器中数据过滤。<br/>链接格式：`&tags={host:[guance01,"guance02],service:kodo}` |
| variable     | 视图变量查询，一般用于仪表板视图中。<br/>链接格式：`&variable={"host":"guance","service":"kodo"}` |
| search       | 文本查询，一般用于查看器中的关键字搜索。<br/>链接格式：`&search=kodo*` |
...
| w            | 工作空间ID，当跨工作空间跳转时需要指定。<br/>链接格式：`&w=wksp_40a73c6c2b024301a0b1d139e1234567` |
...
| 仪表版ID   | `dashboard_id=dsbd_e4313axxxxxxxxxxxxxc4198775e` |
| 仪表板名称 | `name=CPU 监控视图`                              |
| 工作空间ID | `w=wksp_ed134a648xxxxxxxxxxxxx9a9c6115`          |
| 时间变量   | `time=#{TR}`                                     |
...
观测云支持对图表链接进行「开启/关闭显示」以及「编辑」、「删除」、「恢复为默认」等操作。
...
前提条件：已经在观测云仪表板下完成图表创建，现在需要为图表添加链接。

### 链接到其他视图

=== "步骤一：添加图表链接"

    在图表链接中，输入名称“主机 CPU 监控”，根据预设的链接和参数添加链接地址，选择打开方式为“划出详情页”。
...
=== "步骤二：在图表中打开链接"
    ...
    点击配置的“主机 CPU 监控”链接，即可侧滑打开链接的视图。可以看到，视图变量和时间范围都和图表一致。
...
=== "步骤一：添加图表链接"
    ...
    在图表链接中，输入名称“基础设施主机查看器”，粘贴在观测云基础设施主机复制的链接地址，并根据需要展示的效果添加或修改模版变量参数。选择打开方式为“划出详情页”。
...
=== "步骤一：添加图表链接"
     ...
     在图表链接中，名称输入“链接帮助文档”，在链接地址粘贴复制帮助手册的链接。打开方式选择“划出详情页”。
