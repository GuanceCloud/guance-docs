...
观测云的蜂窝图用于显示不同分组下的的数据情况。例如您可以查看不同主机的 CPU 使用率。
...
| 显示图例 | 在图表右下角显示颜色等级范围。 |
| 颜色 | 1、渐变色系：色块的渐变色。选择颜色后，系统将以所选颜色为基准，生成所选等级数量的颜色色块；<br>2、渐变区间：<br><li>自动：默认按照当前数据的最大值、最小值平分为 5 个区间，支持自定义最大值和最小值；<br><li>自定义：支持自定义渐变色系等级，即蜂窝图区域范围的等级设置。系统默认将所选指标的最大值和最小值平分为 5 个渐变等级，支持自定义等级数量（最大 10 个）、等级范围以及显示颜色。 |
