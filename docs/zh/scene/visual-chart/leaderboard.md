...
对某一相关同类事物的客观实力反映，简洁地展示出 Top N 或者 Bottom N 的升降序排行。

![](../img/top.png)

## 应用场景

可用于显示某个指标数据的升降序排行，查看分组数据的排名情况。


## 图表查询

支持**简单查询**、**表达式查询**、**DQL 查询**和 **PromQL 查询**。

查询中默认内置【Top】/【Bottom】函数，预设 4 种排名数量：5、10、20、100。也可以手动输入条数，最多为 100 条数据，可根据选定指标进行升降排序。

**注意**：

1. 单图表查询仅支持一条查询语句，默认为简单查询，点击**转换为表达式查询**切换为表达式查询，并将简单查询作为“查询 A”，支持相互切换。点击 :fontawesome-solid-code: 可切换至**DQL 查询** & **PromQL 查询**；     
2. 若简单查询中使用了表达式查询不支持的转换函数，切换后，不会将函数带入到表达式查询。
