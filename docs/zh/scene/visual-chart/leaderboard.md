...
图表查询支持**简单查询**、**表达式查询**、**DQL 查询**，图表查询条件详细说明请点击 [图表查询](chart-query.md) 进行查看，默认添加简单查询，查询中默认内置「Top」/「Bottom」函数，预设 5、10、20、100，4种排名数量，支持手动输入，最多为 100 条数据，可根据选定指标进行升降排序。排行榜不支持添加别名「AS」。
