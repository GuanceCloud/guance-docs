...
组合图表功能允许用户既可添加单个图表，也可整合多个图表。同时，还可选取当前仪表板中的视图变量作用于该组合图表，在组合对比的基础上灵活筛选数据。
...
{{{ custom_key.brand_name }}}会按照图表查询时间生成嵌入代码，如图表查询时间为最近 15 分钟，即嵌入其他平台后，该图表按照最近 15 分钟显示查询结果。
...
{{{ custom_key.brand_name }}}在场景视图分享的图表统一存储在**管理 > 分享管理 > [分享图表](../../management/share-management.md)**中。



### 导出图表 {#download}

点击 :material-tray-arrow-down: 即可将图表导出为 PNG 图片。

<img src="../img/upload.png" width="70%" >

其中，时序图、饼图、排行榜、柱状图、矩形树图还可将图表导出为 CSV 文件。
...
在仪表板中，点击图表上的**分析**按钮或双击图表空白处，可以放大图表进行查看和分析。您可以通过鼠标 hover 查看图表的数据信息；或通过选择[时间轴](timeseries-chart.md#timeline)查看特定时间范围的指标趋势。
...
为了挖掘深入事件的根本原因，实现更精准的数据分析与排查，您可以对所有指标数据进行下钻分析。

点击进入**下钻分析**，框选目标时间区域并点击**开始检索**，即可呈现下钻分析结果。在查询结果中，您可以查看该时间区域内的下钻一级标签及其对应的时序图分析结果。
...
为了精准定位业务逻辑的根因问题，您可以使用根因分析功能快速确定故障源头。目前，根因分析支持针对 `磁盘使用率` 以及 `内存使用率` 两种关键指标进行分析。
