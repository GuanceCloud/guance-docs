...
图表查询支持**简单查询**、**表达式查询**、**DQL 查询**，图表查询条件详细说明请点击 [图表查询](chart-query.md) 进行查看，默认添加简单查询。查询的指标集以及指标可以不同，但 by（分组）的Tag 必须一致，修改一个，其他查询自动同步修改。
