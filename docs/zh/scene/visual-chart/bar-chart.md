...
观测云的柱状图用于显示一段时间内的数据变化和各变量间的对比情况。例如您可以查询最近 15 分钟不同主机的 CPU 使用率。

## 图表查询

图表查询支持**简单查询**、**表达式查询**和 **PromQL 查询**；默认添加简单查询；您可以添加多个查询，但 by（分组）的 Tag 必须一致，修改一个，另外的自动同步修改。
...
| 堆叠显示 | 支持**指标堆叠**和**分组堆叠**，默认关闭，开启后，默认选中**指标堆叠**。右侧出现“方式”选项，包含默认、百分比：<br /><li> 默认：将每根柱子进行分割，以显示大类目下的细分类目占比情况，统计每个分组下不同指标的大小；<br /><li> 百分比：柱子的各个层代表的是该类别数据占该分组总体数据的百分比，每个系列按照所占的百分比进行堆积。<br /> |
