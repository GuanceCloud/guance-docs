...
???+ attention
    ...
    简单查询和 DQL 查询可通过 ![](../img/buttom-1.png) 按钮相互切换。切换后，若无法解析或者解析不完整：
...
???+ attention
    ...
    若在**日志 > 索引**设置索引以后，在图表查询的数据源选择**日志**时，支持选择不同的索引对应的日志内容，默认为索引 `default`。
...
> 筛选条件内容中的 `#{host}` 为视图变量，详情参考文档 [视图变量](../view-variable.md)。
...
UI 模式下支持选择如下聚合方式返回结果值，在 DQL 模式下支持更多的聚合函数，可参考 [DQL 聚合函数](../../dql/funcs.md)。
...
这里的转换函数又称为外层函数，UI 模式下支持选择的函数如下所示，在 DQL 模式中支持更多的外部函数，可参考 [DQL 外层函数](../../dql/out-funcs.md)。
