...
数据源包括来自指标、日志、基础对象、资源目录、事件、应用性能、用户访问、安全巡检、网络、Profile 的一系列数据组合。

| 选项 | 说明 |
| --- | --- |
| 指标 | 需要选择**指标集**和**指标**，一个指标集可包含多个指标，基础函数默认选中 **Last by**。结合图表类型，选择想要展示的指标进行查询。 |
| 日志/基础对象/资源目录/事件/应用性能/用户访问/安全巡检/网络/ Profile | 系统对象、资源目录和安全巡检需要选择**分类**和**属性/标签**，日志/事件/应用性能/用户访问需要选择**来源**和**属性/标签**，基础函数默认选中 **Count by**。 |

???+ warning "当日志作为数据源"

    若在**日志 > 索引**设置索引，在图表查询的数据源选择**日志**，支持选择不同的索引对应的日志内容，默认为索引 `default`。
    
    > 更多详情可参考文档 [日志索引](../../logs/multi-index/index.md)。
