...
观测云的饼图用于表现数据分组的对比情况。例如您可以查看磁盘已使用空间和剩余空间的对比情况。
...
| 图表类型 | <li>饼图：显示数据分组的对比情况，更多用于样本指标较少的场景；<br /><li>环形图：更多适用于反映多个样本指标各部分所占比例；<br /><li>玫瑰图：圆弧半径的大小表示数据的大小，适用于反映分类过多的场景，和数值大小相似的占比及对比场景。 |
| 合并     | 支持用户配置占比比例大小判断是否合并到 “其他” 显示，若饼图中数据占比比例过小的切片太多，可使用合并来聚合数据到“其他”的切片上，查看占比优先级更高的切片，提高饼图的可读性，配置合并以后：<br/><li>饼图切片增加 “其他” 的切片，表示被合并的数据汇总显示；<br/><li>图例增加 “其他” 的图例项，表示被合并的数据汇总数值及占比显示。 |
...
| 图例 | 用于直观显示各指标数据的名称、数量及占比情况。目前支持选择底部、右侧、环绕、隐藏图例（点击图例中的指标行，可隐藏/显示对应指标） |
...
![](../img/8.scene_pie_1.png)

- 环形图：

![](../img/8.scene_pie_2.png)

- 玫瑰图：

![](../img/8.scene_pie_3.png)
