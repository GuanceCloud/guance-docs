...
热力图是一种数据可视化工具，它通过颜色的变化来表示数据的密度或强度，常用于展示数据在特定场景中的分布情况。<<< custom_key.brand_name >>>将热力图应用于多种监控和分析场景，可视化展示聚合数据。
...
:material-numeric-1-circle: 自动：
 
- 可选择包含从 2～100 的等级数量，系统默认为 60；
- 包含 4 种渐变色系；
- 支持自定义最大值和最小值，如不设置，自动获取返回数据的最大最小值计算。

:material-numeric-2-circle: 自定义：可选择包含从 2～5 的等级数量，系统默认为 5。


### 时间

:material-numeric-1-circle: 锁定时间：即固定当前图表查询数据的时间范围，不受仪表板或其他全局时间组件的限制。设置成功后，当前图表直接展示您设定的时间，如【xx分钟】、【xx小时】、【xx天】。

:material-numeric-2-circle: 时间间隔：即图表查询数据的计算间隔（`interval`）。
...
:material-numeric-3-circle: 最大返回点数：即每个序列的最大数据点数，可任意输入 2～1000 之间的任意整数，若您没有自定义，默认限制最大点数为 720。
