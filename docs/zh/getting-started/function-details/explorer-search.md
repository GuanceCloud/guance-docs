...
观测云的查看器包括基础设施、事件、日志、应用性能监测、用户访问监测、CI可视化、可用性监测、安全巡检等。查看器提供多种搜索和筛选方式，自定义选择时间范围，数据显示列添加、升降序查看、导出等功能，帮助您快速、精准的检索数据，定位故障问题。
...
>前提： 支持站点“中国区1（杭州）”、“中国区3（张家口）”、以及“中国区4（广州）”，工作空间需要在 `2022年6月23日` 后创建，JSON 搜索功能目前仅支持日志查看器使用。
...
- 收起筛选历史：点击关闭按钮 `x` 或者使用 `esc` 按键可收起筛选历史。<br/>  
...
考虑到有时工作空间的数据基于一个时区上报，而当前工作空间的成员可能所处不同国家或地区，为保证各成员能在同一时区分析数据、排查故障，观测云支持在**时间控件**设置当前显示时区，从而切换到对应的工作空间时区查看数据。
...
- 默认显示“浏览器时间”；    
- 支持选择“工作空间时区”；可前往**管理 > 高级配置 > 工作空间时区**进行配置。
...
您也可以在 **[账号管理](../../management/account-management.md#zone)** 进行修改，默认显示**跟随浏览器时间**，支持选择**工作空间时区**。
