...
观测云的查看器包括基础设施、事件、日志、应用性能监测、用户访问监测、CI 可视化、可用性监测、安全巡检等。查看器提供多种搜索和筛选方式，自定义选择时间范围，数据显示列添加、升降序查看、导出等功能，帮助您快速、精准的检索数据，定位故障问题。
...
<font color=coral>**前提**</font>：支持站点 `中国区 1（杭州）`、`中国区 3（张家口）`、以及 `中国区 4（广州）`，工作空间需要在 `2022年6月23日` 后创建，JSON 搜索功能目前<u>仅支持日志查看器</u>使用。
...
<font color=coral>**前提**</font>：DQL 搜索功能目前仅支持日志查看器使用。

在日志查看器，您可以通过点击搜索框内的切换小图标 :fontawesome-solid-code: ，切换到 DQL 手动输入查询模式，支持自定义输入筛选条件和搜索条件。
...
=== "示例 1"
    ...
    在本示例中，对日志的来源和状态进行筛选，需要注意的是<u>筛选的字段值 `value` 要区分大小写</u>，返回的结果如下：
...
=== "示例 2"
    ...
    ```json
    essage = query_string("debug OR 200" )
    ```

    在本示例中，对日志的内容进行进行查询搜索，支持对日志字段（key）进行 `AND` / `OR` 多种条件组合查询。

    > 更多说明可参考 [DQL 函数 query_string()](../../dql/funcs.md#query_string) 。
...
在查看器中，可针对 `标签/属性` 进行值的筛选，支持六种筛选方式，分别为：<u>正向筛选、反向筛选、模糊匹配、反向模糊匹配、存在和不存在</u>。
...
=== "模糊匹配"
    ...
    <font color=coral>**注意：**</font>模糊匹配查询中若存在针对值 左* 匹配时容易导致查询超时，请谨慎使用。
...
=== "反向模糊匹配"
    ...
    通过 `-*key:value` 的形式进行反向模糊匹配（Not Wildcard Matching）搜索，`value` 中可书写通配符进行值匹配。例如：在日志查看器中搜索 `-*service:k8s*` ，即返回所有服务不以 `k8s` 开头日志数据。您可以手动输入标签内容进行反向模糊匹配。
...
=== "编辑筛选"
    ...
    **1、单击标签：**
    ...
    <font color=coral>**注意：**</font> label 标签不支持弹窗编辑。
...
在查看器分析栏，支持基于 <u>1-3 个标签 </u>进行多维度分析统计，以反映出数据在不同的维度下，不同时间的分布特征与趋势。观测云支持多种数据图表分析方式，包括时序图、排行榜、饼图和矩形树图。
...
=== "空间级筛选项"
    ...
    <font color=coral>**注意：**</font>空间级筛选项，工作空间所有成员可以查看，但普通成员和标准成员不支持编辑、删除、移动位置。
...
=== "个人级筛选项"
    ...
    <font color=coral>**注意：**</font>个人级筛选项，仅当前个人可查看，工作空间其他成员无法查看。
...
<font color=coral>**注意：**</font>筛选历史仅支持在本地浏览器查看当前用户的筛选、搜索条件。
...
<font color=coral>**注意：**</font>查看器自动刷新仅对本地浏览器生效。
...
在查看器列表，当鼠标放在显示列上时，可点击显示列的 :fontawesome-solid-gear: ，您可以对显示列进行升序、降序、向左移动列、向右移动列、向左添加列、向右添加列、替换列、添加到快捷筛选、添加到分组、移除列等操作。
...
在指标、日志、事件、应用性能监测、用户访问监测、云拨测、安全巡检等查看器，您可以对当前展示的数据进行搜索和筛选、选择时间范围、增加查看列等操作，然后点击查看器左上角的快照小图标，点击**保存快照**即可保存当前查看器所展示的数据内容。

> 更多快照使用详情，可参考文档 [快照](../../management/snapshot.md) 。
