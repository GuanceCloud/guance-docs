...
> 注意：筛选历史仅支持在本地浏览器查看当前用户的搜索条件。


### 操作说明

在查看器筛选历史最多查看 100 条搜索条件，支持通过键盘上下切换键（↑ ↓ ），切换选择搜索条件，点击键盘 `enter` 可添加到筛选。
...
- 在不同查看器应用筛选历史：当您在**日志 > 查看器**浏览了 `-source: default` 筛选历史（如上图），可在链路等其他查看器的搜索历史直接使用。
