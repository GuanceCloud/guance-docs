...
观测云的查看器包括基础设施、事件、日志、应用性能监测、用户访问监测、CI 可视化、可用性监测、安全巡检等。作为数据可观测的重要工具之一，查看器提供多种搜索和筛选方式，并支持以组合的方式搭配使用获取最终数据结果。本文将详细介绍查看器功能，帮助您快速、精准地检索数据，定位故障问题。

## 搜索 {#search}

搜索一般由<u>术语</u>和<u>运算符</u>两部分组成。支持通配符查询，`?` 表示匹配任意字符， `*` 匹配 0 或多个字符；若要将多个术语组合到一个复杂查询中，可以使用[布尔运算符](#bool)连接。
...
在查看器中，可针对 `标签/属性` 的格式进行筛选，按照 `字段 运算符 值` 顺序拼接可得。

<font color=coral>**注意：**</font>筛选跟搜索的最大区别是输入内容中是否存在 <u>: 冒号间隔符</u>，若存在则判断作为筛选过滤条件，若不存在即为搜索条件（搜索内容中如存在 : 可以使用双引号将搜索内容变更为短语）。

### 字段

您可在 **[管理 > 字段管理](../../management/field-management.md)** 页面查看观测云默认提供的官方字段信息。

### 运算符

观测云支持针对<u>字符串 OR 数值类型字段</u>做筛选过滤。
...
| `wildcard`      | [模糊匹配](#wildcard)，示例：`key:value*`，反向筛选则通过叠加 `≠` 实现。                 |
...
| `<=`      | 小于等于，示例：`key:<=value`。反向筛选则通过叠加 `≠` 实现。                  |
| `[xx TO xx]`      | 区间，示例：`key:[1 TO 100]`。反向筛选则通过叠加 `≠` 实现。                  |
...
# ? * 叠加使用
key:gua?ce*   // ? 匹配 n ,* 匹配 yun

# * 混合使用
key:gua*e*   // 第一个 * 匹配 nc ,第二个 * 匹配 yun
```
