...
<<< custom_key.brand_name >>>的查看器可以在基础设施、事件、日志、应用性能监测、用户访问监测、CI 可视化、可用性监测、安全巡检等功能模块内使用。作为数据可观测的重要工具之一，查看器提供多种搜索和筛选方式，并支持以组合的方式搭配使用获取最终数据结果。本文将详细介绍查看器功能，帮助您快速、精准地检索数据，定位故障问题。
...
???+ warning "注意"
    ...
    当搜索内容包含 `.`（如 `trace.id`），直接使用 `@key1.key2:value` 格式输入会导致系统将 `trace` 和 `id` 识别为两个独立的键，而非一个整体。为避免此问题，请改用 `@\"key1.xxx\":value` 的格式输入，例如 `@\"trace.id\":value`，这样系统会正确识别完整的键名。
...
???+ warning "注意"
    ...
    筛选跟搜索的最大区别是输入内容中是否存在 <u>: 冒号间隔符</u>，若存在则判断作为筛选过滤条件，若不存在即为搜索条件。
...
在查看器分析栏，支持基于数据进行多维度聚合查询分析统计，以反映出数据在不同的维度下，不同时间的分布特征与趋势。

点击**分析**，即可进入数据图表分析方式，包括时序图、排行榜、饼图和矩形树图。还可选择不同的时间间隔显示数据。
...
> 图表分析模式下，可参考 [操作](../../logs/explorer.md#charts) 来管理图表。
...
=== "空间级筛选项"
    ...
    由管理员/拥有者进行配置，点击快捷筛选旁的 :octicons-gear-24:，选择配置空间级筛选项，支持新增字段、编辑字段别名、调整字段顺序、删除字段。
...
- 点击下图图标，即可快速添加该字段至数据列表，再次点击即可移除。
...
=== "查询值 TOP 5"
    ...
    点击快捷筛选项右上角的 :octicons-gear-24: ，选择**查询值 TOP 5**，可查看当前筛选项排在前五名的字段属性值统计数量百分比。在排行榜，支持通过鼠标悬停查看数量统计，支持点击**正向筛选**、**反向筛选**按钮，以 `key:value` 的形式对当前排名的字段属性值进行数据筛选查询。

    ![](../img/8.explorer_1.png)

=== "维度分析"

    点击后，当前查看器将切换为分析模式，该字段自动代入“分析维度”中查询：
...
<<< custom_key.brand_name >>>支持在**筛选历史**查看筛选、搜索历史，且能应用于当前工作空间不同的查看器。
...
<<< custom_key.brand_name >>>支持通过时间控件控制当前查看器的数据展示范围，用户可以手动输入时间范围，或快速选择当前查看器的内置时间范围，或通过自定义设置时间范围。
...
<<< custom_key.brand_name >>>支持在时间控件通过点击 :octicons-pin-24: 锁定图标，设置固定的查询时间范围，设置完成后，所有的查看器/仪表板均默认显示当前的时间范围。
...
<<< custom_key.brand_name >>>支持在**时间控件**设置当前显示时区，从而切换到对应的工作空间时区查看数据。
...
帮助快速获得实时的查看器数据。可选频率：5s、10s、30s、1m、5m、30m、1h。

如不需要自动刷新，选择 Off（关闭）即可。

![](../img/refresh-1.png)

???+ abstract "时间控件对刷新频率的影响"
    ...
    此时，如果您重新选择时间范围小于1小时的相对时间，刷新频率将自动恢复为上次的配置。
...
查看器支持选择显示行数来展开查看日志数据：

- 可自定义添加、编辑、删除、拖动显示列； 
- 可通过键盘上下切换键（↑ ↓）选择添加显示列； 
- 可关键字搜索字段； 
- 可自行定义显示列作为预设字段，后续通过 Pipeline 切割字段并上报数据后可直接显示上报的数据。

![](../img/7.log_column_4.png)

### 时间列 {#time-column}

若查看器存在时间列，可在显示列配置下直接勾选，选择是否在列表中显示。
...
在添加显示列下拉框，选择字段添加到显示列。或直接搜索定位目标字段。

如果在下拉框中未找到目标字段，您可以在“添加显示列”中直接输入字段名称，按回车键即可完成添加。
...
在查看器列表的[标准模式](../../logs/manag-explorer.md#mode)下，hover 在显示列上时，点击设置按钮，可对显示列进行以下操作。
...
| 向左/右移动列      | 基于当前显示列，往左或往右移动。                          |
| 向左/右添加列      | 基于当前列，往左或往右添加新的显示列。                          |
| 替换列      | 在当前位置，替换当前显示列。                          |
| 添加到筛选      | 若左侧快捷筛选无当前显示列，点击即可添加该列作为新的快捷筛选项。                          |
| 进入[分析模式](#analysis)      | 即将该字段直接代入到分析模式下的筛选中。           |
| 移除列      | 移除当前显示列。                 |
...
在查看器，您可以对当前展示的数据进行搜索和筛选、选择时间范围、增加查看列等操作，然后点击查看器右侧的 :octicons-gear-24: ，导出当前查看器所展示的数据内容。您可以**导出到 CSV 文件**、**导出到仪表板**以及**导出到笔记**进行数据查看和分析。
