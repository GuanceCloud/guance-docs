...
| <div style="width: 100px">字段</div>      | 描述                          |
| ----------- | ------------------------------------ |
...
您可以针对字段值内容进行脱敏配置，配置规则为正则语法。正则匹配到的内容在当前快照将以 `*` 显示。
...
<font color=coral>**注意：**</font>在日志查看器，获得被分享[快照查看](../../logs/logdata-access.md#snapshot)权限的用户可在快照页面上方搜索栏查看此条快照下数据列表的筛选条件，且支持添加搜索条件。
