...
Datakit 是运行在用户本地机器上的一种基础数据采集工具，主要用于采集系统运行的各种指标、日志等数据，将它们汇总给 [{{{ custom_key.brand_name }}}](https://guance.com){:target="_blank"}，在{{{ custom_key.brand_name }}}中，用户可以查看并分析自己的各种指标、日志等数据。
...
<figure markdown>
  ![](https://{{{ custom_key.static_domain }}}/images/datakit/dk-network-arch.png){ width="800" }
...
<figure markdown>
  ![](https://{{{ custom_key.static_domain }}}/images/datakit/dk-internal-arch.png){ width="800" }
