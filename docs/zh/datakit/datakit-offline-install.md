...
> 如果是离线安装 lite 版本的 Datakit，需指定带 `_lite` 后缀的安装包，比如 `datakit_lite-linux-amd64-1.68.1.tar.gz`。
...
> 如果是离线升级 lite 版本的 Datakit，需指定带 `_lite` 后缀的安装包，比如 `datakit_lite-linux-amd64-1.68.1.tar.gz`。
...
???+ Attention
    ...
    Windows 下的 `Installer` 程序的下载链接需添加 **.exe** 后缀，如 [*https://static.guance.com/datakit/installer-windows-386-1.68.1.exe*](https://static.guance.com/datakit/installer-windows-386-1.68.1.exe) 和
