...
=== "Windows"
    ...
    ```powershell
    .\installer-windows-amd64.exe --offline --dataway "https://openway.guance.com?token=<YOUR-TOKEN>" --srcs .\datakit-windows-amd64-1.10.0.tar.gz,.\data.tar.gz
    ```
...
=== "Linux"
    ...
    ./installer-linux-amd64 --offline --dataway "https://openway.guance.com?token=<YOUR-TOKEN>" --srcs datakit-linux-amd64-1.10.0.tar.gz,data.tar.gz
    ```
<!-- markdownlint-enable -->
...
# 查看镜像架构是否正确
docker image inspect pubrepo.jiagouyun.com/datakit/datakit:1.10.0 |grep Architecture
