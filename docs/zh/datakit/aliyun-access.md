...
订阅完成后，对您的 Datakit 接入{{{ custom_key.brand_name }}}完全透明，无须修改 Datakit 配置，已自动建立私网连接。可以登录云主机执行以下 `ping openway.guance.com` 命令，查看 ping 出来的 IP，如果是内网 IP 地址，说明已经成功与{{{ custom_key.brand_name }}}数据网关建立了私网连接：
