...
在观测云工作空间，点击**日志 > 黑名单 > 新建黑名单**。

![](img/4.log_blacklist_1.png)

选择**来源**，添加一条或多条日志筛选过滤规则，点击**确定**即默认开启该日志过滤规则。您可以通过**黑名单**，查看全部日志过滤规则。
...
在观测云工作空间**日志 > 黑名单**，点击**批量操作**，即可**批量导出**或**批量删除**黑名单。
...
在观测云工作空间**日志 > 黑名单**中支持导入/导出黑名单，即通过导入/导出 JSON 文件的方式创建黑名单。
