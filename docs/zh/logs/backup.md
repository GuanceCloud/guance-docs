...
为提供更加全面的备份日志存储方式，观测云支持以下四种存储路径：
...
???+ attention "配置须知"
    ...
    备份到 AWS S3 的备份日志<u>暂时还不支持在观测云的查看器中查看，您需前往对应的 AWS S3 中查看备份的历史日志数据</u>。为避免产生额外的流量开销，当前仅支持<u>与观测云站点同 Region 的外部存储数据</u>写入：
...
=== "角色授权"
    ...
    [在 AWS 中配置观测云 IAM 角色](./role-auth.md)后，填写存档信息，填入 AWS 账号 ID、AWS 角色名称及 Bucket 名称。
...
<font color=coral>**注意：**</font>若存档类型信息有变动，将弹出确认框，进行创建规则的二次确定。

<img src="../img/back-7.png" width="70%" >

#### 华为云 OBS

???+ attention "配置须知"

    - 仅 “华为云” 站点-中国区4（广州）支持配置备份到 OBS 外部存储，暂不支持在备份日志查看器查看到被匹配到的日志数据（仅支持同 Region 的数据传输配置）；
...
???+ attention "配置须知"
    ...
    备份到外部存储（OSS）的数据目前暂不支持在备份日志查看器中查询分析。为避免产生额外的流量开销，当前仅支持与观测云站点同 Region 的外部存储数据写入。
...
=== "角色授权"
    ...
    [在阿里云配置台配置授权角色](./aliyun-account.md)后，填写存档信息，填入阿里云账号 ID、OSS 角色名称及 Bucket 名称。
...
=== "Access Keys"
    ...
    您需在阿里云中[配置观测云 RAM 策略](./aliyun-ram.md)。配置完成后，填写账号信息，输入阿里云账号 ID、阿里云 AK & SK 及 Bucket 名称。
...
<font color=coral>**注意：**</font>该存储类型全站点开放。
...
您可前往**管理 > 设置**，在**变更数据存储策略**，点击**更换**，可查看备份日志存储时长：180 天/ 360 天/ 720 天。点击**编辑**即可变更存储策略。
