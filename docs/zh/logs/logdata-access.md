...
1. 定义当前规则的名称；
2. 输入当前规则的描述；
3. 选择[**索引**](../logs/multi-index/index.md)。可选范围包含当前工作空间内日志索引（包含日志默认索引、自定义索引、绑定的外部索引）以及[被授权的所有可查看索引](./cross-workspace-index.md)；

4. 定义当前规则下，日志数据的[访问范围](#scope)；

5. 添加需要脱敏的单个或多个字段；

6. 利用正则表达式针对字段内容中的敏感信息进行脱敏；

7. 选定当前访问规则可应用到的单个或多个成员角色，包含系统内默认角色及自建角色；
8. 可点击右下角预览，查看当前数据访问规则在查看器的可视化展示效果；
9. 点击保存。
...
1. 默认角色未配置数据访问规则时，拥有所有日志的数据查询权限；
2. 基于**用户角色存在日志数据查询权限**的基础，日志数据访问的规则才能生效；
...
3. 启用/禁用规则：修改数据访问规则状态。规则被禁用后，不同角色对日志数据的访问查询范围不受限制，规则启用后恢复。
