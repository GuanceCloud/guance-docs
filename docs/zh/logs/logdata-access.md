...
- 点击**新建规则**按钮，即可开始创建新的数据访问规则；

- 点击 :octicons-copy-24: 图标，可以克隆已有规则创建新的规则。

在弹出的新建页面中，选择**索引**，设置筛选条件，并决定授权的角色对象。

> 有关索引更多相关操作，可参考 [日志索引](../logs/multi-index.md)。   

![](img/data-2.png)

1、索引：多选；当前工作空间内日志索引，包含日志默认索引、自定义索引、绑定的外部索引；

2、数据范围：不同字段之间的逻辑关系可以自定义选择 **任意(OR)** 或 **所有(AND)**；
...
5、角色：多选；包含系统内默认角色及自建角色；选中后仅针对配置的角色做查看脱敏。
