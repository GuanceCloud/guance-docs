...
<<< custom_key.brand_name >>>会根据选择的时间范围自动划分若干时间点，通过堆积柱状图展示不同日志状态的数量，帮助进行统计分析。若对日志进行了筛选过滤，柱状图将同步展示筛选后结果。

- 可 hover 后导出图表，最终导出到仪表板、笔记或粘贴板；
- 可自定义选择时间间隔。
...
<<< custom_key.brand_name >>>查看器默认展示最近 15 分钟的日志数据。您可以自定义数据展示的[时间范围](../getting-started/function-details/explorer-search.md#time)。
...
日志查看器默认显示 `time` 和 `message` 字段。可对显示列进行升序、降序、向左移动列、向右移动列、向左添加列、向右添加列、替换列、添加到快捷筛选、添加到分组、移除列等操作。
...
<<< custom_key.brand_name >>> DQL 查询支持从日志数据的 JSON 字段中提取内嵌的值，通过在 DQL 查询语句中添加带有 `@` 符号的字段，系统会识别该配置并将其作为独立字段在查询结果中展示。如：
...
在进行日志数据筛选时，若您希望对系统过滤出的数据实施进一步的告警监控，可以通过一键设置监控器来实现。<<< custom_key.brand_name >>>将自动应用您选定的索引、数据来源和搜索条件，简化配置流程。
...
<<< custom_key.brand_name >>>为状态值已经设置了系统默认颜色。如您需要修改不同的状态下，对应数据在查看器中显示的颜色，可通过点击**设置状态颜色**进行修改。
...
为了让您能更快的的获取日志的重点数据信息，<<< custom_key.brand_name >>>采用不同的颜色高亮日志的不同内容，分成浅色和深色两种主题颜色。
...
<<< custom_key.brand_name >>>的日志数据列表默认为您展示日志的触发时间和内容。您可以在查看器**显示列**中选择日志显示 “1 行”、“3 行”、“10 行”和全部来查看完整的日志内容。
