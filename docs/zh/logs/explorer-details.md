...
日志上报到<<< custom_key.brand_name >>>时，若数据超过 1M 大小，会按照 1M 标准切分，即一条日志 2.5M，会被分割为 3 条（1M/1M/0.5M）。切割后的日志您可以根据以下字段查看相关完整性：
...
日志服务的上下文查询功能帮助您利用时间线索，追溯到某条异常日志发生前后的日志记录。通过<<< custom_key.brand_name >>>对于日志上下文信息的可视化呈现，有效帮助您故障排查和问题定位，从而及时作出响应。
...
=== "主机"
    ...
    **注意**：<<< custom_key.brand_name >>>默认保存主机对象最近 48 小时的历史数据。未找到当前日志时间对应的主机历史数据时，您将无法查看关联主机的属性视图。
...
=== "网络"
    ...
    **注意**：
    ...
    1. 若同时查询到 Host、Pod、Deployment、Service 的关联字段，进入详情页时则按照此顺序显示网络数据；  
    2. 若未查询到关联字段，则排在末端显示为灰色，点击提示**未匹配到网络视图**。

</div>

## 绑定内置视图

<<< custom_key.brand_name >>>支持设置绑定或者删除内置视图（用户视图）到日志详情页面。点击绑定内置视图，即可为当前日志详情页添加新的视图。
