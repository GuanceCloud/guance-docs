...
在观测云**日志 > 索引**，点击**绑定外部索引**。在弹出的对话框选择 **SLS Logstore**。您可以选择 <u>RAM 子账号授权和第三方快捷授权</u>两种授权方式：
...
3、[字段映射](./multi-index.md#mapping)：非必填；     
...
4、[字段映射](./multi-index.md#mapping)：非必填；       
...
???+ warning "注意"
    ...
    - 操作跨账号角色授权需**使用阿里云主账号</u>或<u>授权了 RAM 访问控制 GetRole、GetPolicy、CreatePolicy、CreatePolicyVersion、CreateRole、UpdateRole、AttachPolicyToRole 权限**的子账号；
