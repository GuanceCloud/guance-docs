...
观测云商业版和专属版工作空间都支持绑定外部 SLS 的索引数据，绑定成功后，您可以在观测云工作空间对来自 SLS 的日志数据进行查询和分析。
...
由于观测云和 SLS 日志服务的标准字段不一致，为了能够更好的让用户在观测云查看和分析存储在 SLS 上的日志，观测云提供字段映射的功能，您可以在绑定 SLS 索引时直接为 SLS 日志的字段进行映射（见上图）。

- `time` ：日志的上报时间，默认映射 SLS  `date` 字段为 `time` ；
- `_docid` ：日志的唯一 ID ，映射后您可以查看绑定的 SLS 日志详情，例如：您可以将原字段 `logid` 映射为 `_docid` 。
- `message` ：日志的内容，映射后您可以查看绑定的 SLS 日志内容，并且通过 `message` 字段帮助您聚类分析您的 SLS 日志数据，例如：您可以将原字段 `content` 映射为 `message` 。更多详情可参考文档 [日志查看器聚类分析](explorer.md) 。
