...
#### [自定义显示列](../getting-started/function-details/explorer-search.md#columns)
...
观测云支持自定义[绑定内置视图](https://preprod-docs.cloudcare.cn/management/built-in-view/bind-view/)到查看器。通过**内置视图**的绑定功能，您可以自定义进程对象的相关内容，并创建绑定关系。
...
观测云支持在进程详情页查看网络连接数据，包括源IP/端口、目标IP/端口、发送字节数、接收字节数、TCP延时、TCP重传次数等。
