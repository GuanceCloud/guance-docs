...
在「基础设施」-「网络」，选择「服务」即可查看服务网络分布情况。在「服务网络 Map」，您能够可视化查询当前工作空间服务之间的每秒请求数、错误率、平均响应时间。

- 时间控件：默认获取最近 15 分钟的数据且不支持自动刷新，需手动点击刷新获取新的数据；
- 搜索和筛选：你可以快速基于关键词模糊匹配搜索服务名称；或基于筛选标签显示服务及其关联关系。
- 填充：你可以通过「填充」自定义填充服务节点，填充值的大小及自定义区间将决定填充的服务节点颜色。支持选择每秒请求数、错误率、平均响应时间作为填充指标。
- 自定义区间：你可以开启「自定义区间」为选择的填充指标自定义图例颜色区间范围。图例的颜色将依据图例的最大值和最小值等分为 5 个区间，每个区间将自动对应五个不同的颜色，不在数据区间范围内的连线和节点置灰显示。	
- 鼠标悬停：悬停鼠标至服务网络节点，可查看每秒请求数、错误率、平均响应时间。
...
在「网络」-「服务」，点击查看服务网络详情，支持查看基于 IP/端口查看源 IP 到目标 IP 之间的网络流量和数据连接情况，点击“查看网络流数据”即可查看其对应的网络流数据。

##### 参数说明

| 参数         | 说明                                                         | 统计方式 |
| ------------ | ------------------------------------------------------------ | -------- |
| 请求数       | 时间范围内，当前服务的总请求数                               | 总和     |
| 平均响应时间 | 时间范围内，当前服务响应时间                                 | 平均值   |
| 错误数       | 时间范围内，当前服务的请求错误数，即 status_code 字段值为 4xx,5xx 的总和 | 总和     |
| 错误率       | 时间范围内，当前服务的 “请求错误数 / 总请求数” 的值          | 百分比   |
...
在「网络」-「服务」，点击查看服务详情，即可查看服务所属的主机、IP 地址、扩展属性等信息。
...
您可以点击主机/Pod/Deployment/服务 图标进行关联查询，支持查看上下游、网络详情、 主机/Pod/Deployment/服务详情、关联日志、关联链路以及关联事件。

### 查看上下游

在基础设施网络，点击主机/Pod/Deployment/服务 图标，点击“查看上下游”，即可查看当前节点的上下游节点关联。
