...
<<< custom_key.brand_name >>>支持对每一个基础设施对象进行关联分析，在容器对象的详情页，除了容器的基本信息，您还可以一站式地了解关联对应容器的的指标、主机、Pods、日志、进程等，更快更全面地监测容器运行情况。
...
=== "日志"
    ...
    **注意**：为了更流畅的用户查询体验，<<< custom_key.brand_name >>>默认即时保存用户在**日志**的浏览设置（包括“最大显示行数”、“显示列”），以使**关联日志**与**日志**保持一致。然而，在**关联日志**进行的自定义调整，在退出页面后不做保存。
...
<<< custom_key.brand_name >>>支持对每一个基础设施对象进行关联分析，在 Pods 对象的详情页，您不仅能够了解 Pods 的基本信息，还可以一站式关联对应 Pods 的指标、容器、日志、网络、主机等，更快更全面的监测 Pods 运行情况。
...
=== "网络"
    ...
    Deployments 网络数据采集成功后会上报到<<< custom_key.brand_name >>>控制台，您可以在**基础设施 > 容器 > Pods**详情页中的**[网络](network.md)**，查看当前 Deployments 的网络性能监测数据信息。
