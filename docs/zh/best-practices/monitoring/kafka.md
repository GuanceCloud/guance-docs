...
在开始使用观测云观测 Kafka 之前，您需要先注册一个 [观测云账号](https://auth.guance.com/register?channel=)，注册完成后登录到观测云工作空间。然后按照 [Kafka 集成文档](https://docs.guance.com/integrations/middleware/kafka/)来实现 Kafka 的可观测。
