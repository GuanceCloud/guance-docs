...
2. 按需输入针对该策略的描述；
3. 选择通知时区；
4. 选择基于[等级](#notice-config)或基于[成员](#member)配置触发通知的告警策略；
5. 按需选择重复告警（原告警沉默）的时间范围；
6. 选择[告警聚合](#pattern)的模式，决定告警通知的最终聚合形式；
...
如果监控器在短时间内频繁检测到同等级异常，可能意味着存在持续性问题。为减少重复通知，您可以在基于[等级](#simple)或[成员](#member)的简单通知配置上，增设升级通知规则。这样，当异常持续出现时，系统会自动将其提升为紧急通知，并发送给指定接收者，确保问题能被及时关注和处理。
