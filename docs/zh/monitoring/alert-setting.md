...
当[监控器](./monitor/index.md)检测到异常时，会自动创建异常事件记录。通过将监控器与告警策略关联，您可以确保相关告警通知能够及时发送给指定对象。

告警策略的配置不仅提供了名称、描述、时区和操作权限等基础功能，还支持从**告警等级**和**通知对象**两个维度灵活定义通知方式。此外，您还可以为告警策略配置升级通知功能，以应对紧急情况。同时，告警策略允许您自定义通知发送的时间，以满足不同场景的需求。
...
???+ warning "配置须知"
    ...
    - 恢复通知：当历史发送的异常告警事件恢复后，系统会发送恢复通知到对应通知对象。例如：给某个群发送过相关事件的`紧急`通知，当这一状态开始恢复时，会向此群发送恢复通知。  
...
#### :material-numeric-1-circle: 基于等级的通知配置 {#simple}
...
| 短信      | 短信通知；一个短信组可以添加多个空间成员，可前往监控 > 通知对象管理查看。<br />[体验版](../plans/trail.md)无短信通知，其他版本短信通知 0.1 元 / 条，按天计费，没有免费额度。      |
| 自定义外部邮箱      | 输入邮箱回车即可；仅支持商业版和部署版用户。      |


#### :material-numeric-2-circle: 基于成员的通知配置 {#member}
...
2. 选定需要通知到的成员和团队；
3. 添加过滤条件，实现标签匹配；
4. 针对过滤后的事件数据，您可为不同的事件等级设定对应的通知对象；
5. 按需开启[自定义通知时间范围](#custom)的配置。

**注意**：如果您配置了多组自定义通知时间范围，系统将按照从上到下的顺序进行匹配，并仅使用第一个匹配到的时间范围内的通知规则来发送告警。
...
点击过滤按钮后，系统会自动获取当前工作空间的字段，并以 `key:value` 的形式设置过滤条件。您可以选择以下匹配方式：等于、不等于、通配、通配取反和正则匹配。相同 `key` 字段的多个过滤条件之间为 OR 关系，不同 `key` 字段的过滤条件之间为 AND 关系。

您可以通过以下两种方式配置过滤条件：

- 直接在页面上选择字段并设置条件。 
- 编写正则表达式以实现更复杂的筛选逻辑，满足精细化配置需求。

**注意**：每条告警规则下仅能添加一组过滤条件，一组条件中可包含一条或多条过滤规则，系统会结合所有规则进行条件筛选。
...
- 当相同级别的告警持续产生时，系统会检查时间间隔，以确定是否发送首次升级通知；
- 发送首次升级通知后，系统会根据第二条升级通知配置的时间间隔，判断是否需要发送第二次升级通知。

**注意**：
- 每条通知规则最多支持配置两个升级通知。
- 每个升级通知仅触发一次，不会出现重复告警的情况。
...
2. 依据天、周、月及自定义这四个维度来划分事件所在的周期；
    ...
    - 若选择自定义，需上传 CSV 文件，系统会根据文件内的填写日期自动填充。文件内日期格式需为 `年/月/日` & `YYYY/MM/DD`；文件内日期数量最多不超过 365 个。
...
**注意**：若选择【永久】这一重复告警选项，系统仅发送首次告警通知，后续不再重复发送。
...
:material-numeric-4-circle-outline: AI 聚合：使用<<< custom_key.brand_name >>>的大模型合并监控告警，减少冗余，避免短时间内产生大量重复告警。
