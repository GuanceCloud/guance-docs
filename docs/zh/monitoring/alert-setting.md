...
<img src="../img/notice-config.png" width="80%" >
...
:material-numeric-1-circle-outline: [事件等级](monitor/event-level-description.md)：包括<u>紧急、重要、警告、无数据、信息</u>。基于选定的事件等级，观测云支持联动[告警聚合](#pattern)进行告警通知。
...
针对每条通知规则可配置发送升级通知，若某段时间内持续产生相同等级的事件，可发送升级通知给某一对象。若一个通知规则配置了两个升级，则：

- 会根据实际触发情况判断是否需要触发对应等级的告警通知；
- 相同等级判断若一直持续产生，观测云会判断时间间隔是否符合发送升级通知；
- 发送完首次升级通知后，观测云会同样根据第二次配置的时间间隔判断是否符合再次发送升级通知内容。
