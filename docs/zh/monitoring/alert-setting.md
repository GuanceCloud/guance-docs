...
此处的告警策略，{{{ custom_key.brand_name }}}除了为其配备名称、描述、时区、操作权限等基础功能外，还可以**从等级和成员两个维度**来定义告警策略对外发送通知的方式，并为这些策略添加升级通知功能。{{{ custom_key.brand_name }}}还允许您自定义通知发送的时间。
...
???+ warning "配置须知"
    ...
    - 恢复通知：当历史发送的异常告警事件恢复后，{{{ custom_key.brand_name }}}会发送恢复通知到对应通知对象。例如：给某个群发送过相关事件的`紧急`通知，当这一状态开始恢复时，会向此群发送恢复通知。  
...
| 短信      | 短信通知；一个短信组可以添加多个空间成员，可前往监控 > 通知对象管理查看。<br />{{{ custom_key.brand_name }}}体验版无短信通知，其他版本短信通知 0.1 元 / 条，按天计费，没有免费额度。      |
...
**注意**：若您配置了多组自定义通知时间范围，{{{ custom_key.brand_name }}}会按照从上到下的顺序匹配，最终只会取第一个匹配到的时间范围内通知规则做告警发送。
...
点击过滤按钮后，{{{ custom_key.brand_name }}}自动获取当前工作空间的字段，以 `key:value` 的形式通过**等于、不等于、通配、通配取反和正则匹配**的方式设置过滤条件。相同 `key` 字段的多个过滤条件之间为 OR 的关系，不同 `key` 字段的过滤条件之间为 `AND` 的关系。
...
- 如果相同级别的告警持续产生，{{{ custom_key.brand_name }}}会检查时间间隔，确定是否需要发送升级通知；
- 发送完首次升级通知后，{{{ custom_key.brand_name }}}会根据第二条升级通知配置的时间间隔，判断是否需要再次发送升级通知。
...
2. 依据天、周、月及自定义这四个维度来划分事件所在的周期；
    ...
    - 若选择自定义，需上传 CSV 文件，{{{ custom_key.brand_name }}}会根据文件内的填写日期自动填充。文件内日期格式需为 `年/月/日` & `YYYY/MM/DD`；文件内日期数量最多不超过 365 个。
...
**注意**：若选择【永久】这一重复告警选项，{{{ custom_key.brand_name }}}仅发送首次告警通知，后续不再重复发送。
