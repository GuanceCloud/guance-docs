...
1. 定义当前告警策略的名称；
2. 按需输入针对该策略的描述；
3. 选择通知时区，默认显示当前的[工作空间时区](../management/index.md#workspace)。若拥有者或管理员未配置，则默认填充 `东八区` 时区；
4. 选择基于[等级](#notice-config)配置触发通知的告警策略；
5. 选择重复告警（原告警沉默）。即，若同一个事件不是非常紧急，但是告警通知频率高，可以通过设置重复告警通知的时间间隔这一方式来减少告警通知频率；
6. 选择[告警聚合](#pattern)，将决定告警通知的最终聚合形式；
7. 为策略规则添加[操作权限](#permission)；
8. 点击保存，即可创建成功。
...
基于成员配置通知规则，可以实现精准的点对点告警通知，从而有助于迅速响应异常事件。在这一配置项下，在用户的层级上帮助更精细化地管理告警通知和问题边界。

1. 选定需要通知到的成员和团队；
2. 添加过滤条件，实现标签匹配；
3. 针对过滤后的事件数据，您可为不同的事件等级设定对应的通知对象。如需必要，还可针对该等级[配置升级通知](#upgrade)；
