...
2. 打开 DataFlux Func 的[脚本市场](https://func.guance.com/doc/script-market-basic-usage/)并添加官方脚本市场
...
在自建的 DataFlux Func 中，通过「脚本市场」安装「观测云自建巡检 Core 核心包」「观测云算法库」并前往 PIP 工具安装相关依赖，安装「 观测云自建巡检（磁盘使用率）」并根据提示配置观测云 API Key 完成开启。
