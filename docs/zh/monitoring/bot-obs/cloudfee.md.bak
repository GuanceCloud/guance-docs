...
5. 在脚本市场中安装开启[「观测云集成（华为云-账单采集）」](https://func.guance.com/doc/script-market-guance-huaweicloud-billing/)、[「观测云集成（阿里云-账单采集）」](https://func.guance.com/doc/script-market-guance-aliyun-billing/)、[「观测云集成（腾讯云-账单采集）」](https://func.guance.com/doc/script-market-guance-tencentcloud-billing/) 并且收集数据天数超过 15 天
...
在自建 DataFlux Func 创建新的脚本集开启云账户账单巡检配置，新建脚本集之后，在创建巡检脚本时选择对应的脚本模板保存，在生成的新脚本文件中根据需要更改即可。
