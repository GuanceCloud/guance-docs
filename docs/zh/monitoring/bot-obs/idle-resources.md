...
4. 在自建的 DataFlux Func 中，通过「脚本市场」安装「观测云自建巡检 Core 核心包」「观测云算法库」「 观测云自建巡检（闲置资源巡检）」
...
在自建 DataFlux Func 创建新的脚本集开启闲置资源巡检配置，新建脚本集之后，在创建巡检脚本时选择对应的脚本模板保存，在生成的新脚本文件中根据需要更改即可。
...
#### 启用/禁用
闲置资源巡检默认是「开启」状态，可手动「关闭」，开启后，将对配置好的 闲置资源巡检配置列表进行巡检。

#### 导出
智能巡检支持“导出 JSON 配置”。在智能巡检列表右侧的操作菜单下，点击「导出」按钮，即可导出当前巡检的 JSON 代码，导出文件名格式：智能巡检名称.json 。

#### 编辑
智能巡检「 闲置资源巡检 」支持用户手动添加筛选条件，在智能巡检列表右侧的操作菜单下，点击「编辑」按钮，即可对巡检模版进行编辑。
...
## 查看事件
观测云会根据当前的工作空间的主机状态进行巡检当发现有主机出现闲置时，智能巡检会生成相应的事件，在智能巡检列表右侧的操作菜单下，点击「查看相关事件」按钮，即可查看对应异常事件。
...
闲置资源巡检会检测云主机的运行状态发现其出现闲置情况时生成对应的事件报告。
