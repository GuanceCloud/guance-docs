...
**用户访问智能检测**基于智能检测算法，智能检测用户访问网站/APP 的性能异常和错误突增情况。通过异常检测指标，自动进行异常分析，定义异常等级并触发告警。

- 性能检测指标判定阈值： LCP(>= 2.5 s)、INP (>= 200 ms)、FID (>= 100 ms)、CLS (> 0.01)
...
**注意**：用户访问智能检测触发的事件等级为【紧急】【重要】【警告】。告警级别阈值：Critical > 76.2 、 Warning > 47.4 、 error > 59.8 .
...
* 异常总结：显示查看用户访问异常服务标签、异常分析报告详情、异常值分布情况统计

* 异常分析：关联用户访问异常页面的性能指标，分析页面 LCP、FCP、页面错误率等异常趋势
