...
选择钉钉机器人、企业微信机器人、飞书机器人、Webhook 自定义、短信组和简单 HTTP 请求这 6 种渠道作为通知对象，配置发送事件通知的目标 [Webhook 地址](#webhook)。此外，您还可以[定义相关通知权限](#permission)，以控制哪些用户或角色可以操作这些通知对象规则。
...
2. 可选择批量操作；
3. 若通知对象规则未赋予您权限，则无法启用/禁用、编辑或删除。
