...
默认情况下，<<< custom_key.brand_name >>>会使用[事件内容](#content)作为告警通知内容。如果您需要自定义实际对外发送的通知，可以选择在此处启用开关，填入通知信息。
...
<<< custom_key.brand_name >>>支持查看已有监控器的状态、最后更新时间、创建时间以及创建人，支持通过恢复监控器来查看监控器的历史配置，帮助您快速和团队其他成员沟通协作来更新监控器。
