...
除选定系统提供的以上特定选项，您还可以输入[**自定义 crontab 任务**](./detection-frequency.md)，基于分钟、小时、天、月、周等周期配置定时任务执行情况。
...
| 检测维度 | 配置数据里对应的字符串类型（`keyword`）字段都可以作为检测维度进行选择，目前检测维度最多支持选择三个字段。通过多个检测维度的字段组合，可以确定一个确定的检测对象，系统会判断某个检测对象对应的统计指标是否满足触发条件的阈值，若满足条件则产生事件。<br />*（例如选择检测维度 `host` 与 `host_ip`，则检测对象可以为 {host: host1, host_ip: 127.0.0.1}）。当检测对象为“日志”，默认以 `status`，`host`，`service`，`source`，`filename` 为检测维度。*  |
...
### 恢复条件 {#recover-conditions}

启用配置恢复条件后，为当前查看器配置恢复条件及严重程度，当查询结果为多个值时，任意一值满足触发条件则产生恢复事件。


- 告警恢复逻辑：每个告警级别对应一个恢复阈值，触发告警后发送对应恢复事件。

- 恢复事件发送逻辑：     
    - 告警级别从高到低变化时，发送对应级别的恢复事件和下一个级别的告警事件；           
    - 告警级别从低到高变化时，发送对应级别的告警事件；     
    - 告警恢复正常时，发送正常恢复事件。   
