...
除选定{{{ custom_key.brand_name }}}提供的以上特定选项，您还可以输入[**自定义 crontab 任务**](./detection-frequency.md)，基于分钟、小时、天、月、周等周期配置定时任务执行情况。
...
| 别名 | 自定义检测指标名称。 |
| [查询方式](../../scene/visual-chart/chart-query.md) | 支持简单查询、表达式查询和 PromQL 查询。<br />此处的 PromQL 查询规则为 Instant Query，从而能查询一个范围内的精准结果值。 |
