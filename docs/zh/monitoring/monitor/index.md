...
工作空间的全部监控器可通过观测云平台的**监控**进行查看和管理。
...
| <div style="width: 100px"> 操作 </div> | 说明 |
| --- | --- |
| 启动/禁用 | 观测云支持启用/禁用已有的监控器。新建的监控器将默认启动。<br>:warning: 禁用的监控器将不再生效；被禁用的监控器可重新开启。 |
| 编辑 | 观测云支持通过点击监控器名称或**编辑**对已有的监控器进行重新编辑。 |
...
- 为监控器配置告警策略时，为保证检测时间范围内的数据不会因为网络、落库等延迟影响，监控器的异常事件检测配置了 2 分钟的等待处理；  
