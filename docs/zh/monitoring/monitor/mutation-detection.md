...
- 触发前置条件配置：默认开启；当检测值满足触发前置条件所设阈值时（运算符支持 >、>=、<、<= ，默认选中 >），再继续进行突变向上（数据升高）、向下（数据下降）、向上或向下 3 种形式的数据比较，进行突变检测规则的判断；

![](../img/muta_01.png)

- 您可选择关闭该配置，只进行突变向上（数据升高）、向下（数据下降）、向上或向下 3 种形式的数据比较，进行突变检测规则的判断。
