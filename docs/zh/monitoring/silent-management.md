...
静默指的是满足条件的事件不发送告警，但事件依然产生。观测云提供**静默管理**功能，您可以对当前工作空间下不同的监控器、智能巡检、自建巡检、SLO、告警策略的全部静默规则进行管理，设置静默后，静默对象在静默时间内不向任一告警通知对象发送告警通知。
...
- 静默范围是在静默任务中设置**停止发送告警通知的监控对象**，主要包含四个维度：
...
**注意**：调整[时区](../management/index.md#zone)不影响设置静默规则的静默时间。
