...
在某些情况下，检测配置中指定的`by`或`维度`字段可能会包含中横线`-`，如：`host-name`。

由于语法问题，模板引擎会将`host-name`解析为「`host` 减去 `name`」，导致无法正常渲染。

此时，可以使用`df_event['host-name']`来替代`host-name`，如：`{{ df_event['host-name'] }}`
