...
观测云计费周期为**天**，即根据工作空间当天统计的使用量在次日零点结算，生成每日账单同步到观测云[费用中心](../billing-center/index.md)，最终根据实际绑定结算方式从对应账中扣除消费金额。
...
> 观测云不同类型数据的数据保存策略支持自定义选择，详细信息请参考 [数据存储策略](../../billing-method/data-storage.md)。
-->
...
- [<font color="coral"> :fontawesome-solid-arrow-right-long: &nbsp; 观测云自研时序数据库</font>](./gauncedb.md)
