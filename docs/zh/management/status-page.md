...
**注意**：观测云将对站点下工作空间的各功能模块以 1 分钟/次的频率进行检测，5 分钟进行一次检测汇总。5 分钟内，存在一次异常即判定此次汇总为异常。若连续 6 次（即 30 分钟）为连续的异常事件，则会触发告警通知邮件。第一次异常告警通知后，若第一次检测该功能模块（即 5 分钟一次的检测汇总）为正常，则判定异常事件已恢复正常。
