...
观测云支持为工作空间的登录账号，统一设置登录会话保持时间，包括无操作登录会话保持时间和登录会话最大保持时间，设置以后，超时登录会话会失效。

- 无操作登录会话保持时间：支持设置范围 30 ～ 1440 分钟，默认为 30 分钟；
- 登录会话最大保持时间：支持设置范围 0 ～ 7 天，其中 0 表示永不超时，默认为 7 天。

在工作空间，点击左下角「账号」-「账号管理」，在登录保持时间，点击「修改」，即可修改默认的登录会话保持时间。

> 若是 SSO 登录账号，默认使用 SSO 配置的登录会话保持时间，支持在「账号管理」设置新的登录会话保持时间。更多详情可参考文档 [SSO 设置登录会话保持时间](../management/sso/index.md#login-hold-time)
