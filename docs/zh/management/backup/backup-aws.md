...
1. 存档类型选择 **AWS S3**，即表示将匹配到的日志数据保存到该对象存储中；   
2. 选择[访问类型](#type)；
3. 点击确定，即可创建成功。
...
1. 须使用<<< custom_key.brand_name >>>默认生成的外部 ID 配置 AWS 资源第三方访问权。

2. [在 AWS 中配置<<< custom_key.brand_name >>> IAM 角色](../role-auth.md)后，填写存档信息，填入 AWS 账号 ID、AWS 角色名称、地区及存储桶名称。
...
1. 点击下载 AWS 资源授权模板，[前往 AWS 中配置<<< custom_key.brand_name >>> IAM 策略](../ak-auth.md)。
...
1. AWS 提供跨账号授权能力，您需使用<<< custom_key.brand_name >>>专属的账号 ID 并[依据配置说明](../aws-cross-account.md)添加跨账号访问授权策略。
