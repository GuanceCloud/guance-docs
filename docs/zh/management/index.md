...
在以任何方式加入工作空间之前，需要先[注册<<< custom_key.brand_name >>>账号](https://<<< custom_key.studio_main_site_auth >>>/businessRegister)。[注册完成](../plans/commercial-huaweiyun.md)后，您可以通过[<<< custom_key.brand_name >>>官网](https://<<< custom_key.brand_main_domain >>>/)，点击**登录**，选择注册站点，输入登录账号和密码，登录到<<< custom_key.brand_name >>>控制台。系统会默认为您创建一个工作空间，并赋予**拥有者**权限。
...
???+ warning "注意"
    ...
    如果在管理后台选择隐藏头像，此处关于查看器自动刷新、高性能模式等配置项可前往**管理 > 个人设置**进行修改。
