...
<<< custom_key.brand_name >>>支持基于企业域名为员工设置单点登录，只要符合企业统一身份认证的员工都可以通过和企业域名相同后缀的邮箱单点登录到<<< custom_key.brand_name >>>，访问权限可选择只读成员或标准成员。

在<<< custom_key.brand_name >>>工作空间**管理 > 成员管理 > SSO 管理 > 用户 SSO**，按需选择 [SAML](configuration-faq.md#saml) 或 [OIDC](#oidc)，即可开始为员工设置 SSO 单点登录。

- <<< custom_key.brand_name >>>支持工作空间创建多个 SSO 的 IDP 配置，每个工作空间 SSO 配置最多不超过 10 个；
- 若多个工作空间同时配置了相同的身份提供商 SSO 点单登录，用户通过 SSO 单点登录到工作空间后，可以点击<<< custom_key.brand_name >>>左上角的工作空间选项，切换不同的工作空间查看数据。
...
| 访问限制    | 用于校验单点登录时输入的邮箱后缀是否和域名匹配，相同域名后缀的邮箱才有权限获取此身份提供商单点登录访问链接。用户可以在首次登录时动态创建<<< custom_key.brand_name >>>成员账号，无需提前在工作空间内创建。 |
...
| 字段      | 描述                  |
| ----------- | ------------------- |
| 登录地址       | 基于用户上传的元数据文档生成的<<< custom_key.brand_name >>> SSO 的登录地址，一个登录地址有且只能访问一个工作空间。  |
| Metadata      | 基于用户上传的元数据文档生成的<<< custom_key.brand_name >>> SSO 的元数据文档。                  |
| Entity ID      | 基于用户上传的元数据文档生成的<<< custom_key.brand_name >>> SSO 登录的响应实体 ID，用于在身份提供商标识服务提供商（SP），如<<< custom_key.brand_name >>>。                  |
| 断言地址      | 基于用户上传的元数据文档生成的<<< custom_key.brand_name >>> SSO 登录的响应目标地址，用于在身份提供商调用进行单点登录。                  |
...
???+ abstract "举例说明"
    ...
    - 配置 SSO 单点登录时，无操作会话过期时间为 30 分钟，成员 A 此时登录了<<< custom_key.brand_name >>>，即其 SSO 登录账号无操作会话过期时间为 30 分钟；
    - 此后，管理员更新无操作会话过期时间为 60 分钟，成员 A 的 SSO 的登录账号无操作会话过期时间还是为 30 分钟；若在此之后有成员 B 登录<<< custom_key.brand_name >>>，则其 SSO 登录账号无操作会话过期时间为 60 分钟，以此类推。
...
要实现 SSO 登录，需将身份提供商的账号信息与<<< custom_key.brand_name >>>账号信息进行字段映射，此处信息主要包含如下字段：
...
???- abstract "用户侧配置 OIDC 相关注意事项"
    ...
    1. 授权模式：<<< custom_key.brand_name >>>只支持 `authorization_code` 授权模式； 其返回类型必须是 `code`；
...
| 发起登录 URL      | 用于从<<< custom_key.brand_name >>>端进入 OIDC 协议流程，是提供商发起登录的 URL。                 |
...
- 启用后，SSO 登录的用户账号将被剥夺其当前所在工作空间中的角色，并根据身份提供商提供的**属性字段**和**属性值**，匹配角色映射规则动态分配角色。若未匹配到角色映射规则，则用户账号将被剥夺所有角色，且不允许登录访问<<< custom_key.brand_name >>>工作空间；
...
启用 SSO 以后，若企业成员通过 SSO 单点登录到<<< custom_key.brand_name >>>，即可在 **SSO 登录**查看所有登录过的成员数量，点击成员数字，可查看具体被授权单点登录成员名单。
...
<<< custom_key.brand_name >>>支持基于配置角色映射关系，为企业提供更精细的单点登录方案，开启角色映射后，支持为企业员工动态的分配访问权限，员工可根据被分配的角色权限来访问<<< custom_key.brand_name >>>。

### 配置角色映射

在<<< custom_key.brand_name >>>工作空间**管理 > 成员管理 > SSO 管理 > 角色映射**，点击**添加映射**，即可创建一个新的映射关系。
...
- 角色授权：<<< custom_key.brand_name >>>支持四种默认成员角色 Owner、Administrator、Standard 和 Read-only；或在[角色管理](../role-management.md)创建的自定义角色。
...
- 编辑：支持重新修改已配置的映射角色，此时使用 SAML 登录没有映射到<<< custom_key.brand_name >>>角色的用户将被剥夺所有角色，并且不允许登录到<<< custom_key.brand_name >>>控制台；
- 删除：支持直接删除或选择批量删除已配置的映射角色，此时使用 SAML 登录没有映射到<<< custom_key.brand_name >>>角色的用户将被剥夺所有角色，并且不允许登录到<<< custom_key.brand_name >>>控制台。

![](../img/5.sso_mapping_10.1.png)


## 登录<<< custom_key.brand_name >>>控制台 {#login}

SSO 配置完成后，在[<<< custom_key.brand_name >>>官网](https://www.guance.com/)登录页面选择**单点登录**。
...
输入企业通用邮箱和密码可直接登录到<<< custom_key.brand_name >>>对应的工作空间：
...
在<<< custom_key.brand_name >>>工作空间，点击**管理 > 设置 > 安全 > 操作审计**，点击**查看**，即可查看当前工作空间所有的审计事件。
...
将外部平台与<<< custom_key.brand_name >>>进行集成，从而实现外部平台的帐户自动登录（单一登录）到<<< custom_key.brand_name >>>平台并访问对应工作空间资源，不必为企业/团队单独创建<<< custom_key.brand_name >>>账号。
