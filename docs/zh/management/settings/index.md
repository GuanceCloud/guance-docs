...
观测云支持为当前工作空间设置备注，备注仅自己可见，将和工作空间名称同时显示，帮助更清晰地识别工作空间信息。
...
观测云支持当前空间拥有者和管理员复制/变更空间内的 Token，并自定义当前 Token 的失效时间。进入**管理 > 空间设置 > 更换 Token**，选择失效时间并确认**更换**后，观测云会自动生成新的 Token，旧的 Token 会在指定时间内失效。
...
观测云支持拥有者和管理员删除指定资源目录分类以及所有资源目录。
