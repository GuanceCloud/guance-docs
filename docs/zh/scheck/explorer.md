...
同时，您可以通过「跳转」按钮，跳转至概览页面对应的内置视图页面进行查看，并对该视图进行克隆并修改。更多可参考 [内置视图](../scene/built-in-view/index.md)。
...
观测云会依据选择的时间范围统计该时间点上出现不同状态巡检事件的数量。您可以通过堆积柱状图，查看不同时间点的巡检事件数量，支持选择不同的时间间隔查看巡检事件数量统计，支持导出到仪表板、笔记和复制到剪切板。
...
观测云支持创建可快捷访问的数据副本，通过快照功能，您可以快速复现即时拷贝的数据副本信息，将数据恢复到某一时间点和某一数据展示逻辑。

您可以对当前展示的数据进行搜索和筛选、选择时间范围、增加查看列等操作，然后点击查看器左上角的「快照」小图标，点击「保存快照」即可保存当前查看器所展示的数据内容，支持在历史快照中对保存的快照进行“分享快照”、“复制快照链接”和“删除快照”等操作。更多详细介绍可参考文档 [快照](../management/snapshot.md) 。
