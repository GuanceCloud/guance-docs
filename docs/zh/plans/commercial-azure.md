# 微软云市场开通{{{ custom_key.brand_name }}}商业版
---

本文将介绍<u>在微软云市场订阅{{{ custom_key.brand_name }}}服务以后，如何开通{{{ custom_key.brand_name }}}商业版并使用微软账号进行费用结算</u>。

**注意**：通过微软云账号进行结算，仅支持美元这一币种。

## 订阅{{{ custom_key.brand_name }}} {#subscribe}

1. 登录进入[微软云市场](https://azuremarketplace.microsoft.com/en-us/marketplace/apps/ceinformationtechnologycompanylimited1715838939322.guance_saas?tab=Overview)的{{{ custom_key.brand_name }}}商品页；
2. 点击 **Get It Now**；
...
## 开通{{{ custom_key.brand_name }}}商业版

点击后自动跳转回{{{ custom_key.brand_name }}}的开通页面。如果当前微软云账号未绑定{{{ custom_key.brand_name }}}费用中心账号，会出现两种情况：
...
#### :material-numeric-2-circle: 绑定{{{ custom_key.brand_name }}}工作空间
...
由于{{{ custom_key.brand_name }}}控制台和费用中心是两个独立的平台，此时需要确认您是否注册过{{{ custom_key.brand_name }}}、拥有{{{ custom_key.brand_name }}}账号。

<img src="../img/azure-8.png" width="70%" >

:material-numeric-1-circle-outline: 有{{{ custom_key.brand_name }}}账号
...
:material-numeric-1-circle-outline: 没有{{{ custom_key.brand_name }}}账号
