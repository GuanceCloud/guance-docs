...
:material-numeric-1-circle: 基于当前工作空间不同的数据转发外部存档类型统计汇总转发的流量大小，并根据数据对应计费出账。（每 GB / 天）。
...
:material-numeric-2-circle: 基于当前工作空间数据转发到观测云内部存储类型统计汇总转发的流量大小，并根据数据对应计费出账。（每 GB / 天）。
