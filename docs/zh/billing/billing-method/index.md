...
采用 [默认出账逻辑](#default) ，每天的费用为 39.9 元，具体明细如下：

| 计费项   | 日计费数量 | 计费单价        | 计费逻辑                                                     | 日计费费用 |
| -------- | ---------- | --------------- | ------------------------------------------------------------ | ---------- |
| 主机     | 10 台      | 3 元 / 台       | 10*3                                                         | 30 元      |
...
| PV       | 2 万个     | 0.7 元 / 万个   | (实际统计数量 / 计费单元) * 单价<br/>即 （2 万 / 1 万） * 0.7 元 | 1.5 元     |
...
采用 [仅时间线+数据出账逻辑](#time-data) ，每天的费用为 11.4 元，具体明细如下：
