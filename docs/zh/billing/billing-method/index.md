...
若在当前指标集下新增了一个 "角色权限" 字段用来表示，因为每个邮箱都可以赋予不同的角色权限，所以每个邮箱的不同状态下都对应存在 "只读成员"、"标准成员"、"管理员" 3 种角色权限。那么可以得到当前 "测试" 指标集下的时间线数量为 18 （3 * 2 * 3 =18）。
...
当前空间下，统计一天内所有页面浏览产生的 PV 数量，默认计费单位为“万个”。
...
用于计算当前工作空间各个项目的使用情况。用户可计算一天、一个月和一年的使用情况。点击 [价格计算器 ](https://www.guance.com/billing)可自动计算价格。
