...
???+ abstract "计算示例"
    ...
    假设执行一次【突变检测】，则计算为 5 次任务调用。

    2.超检测区间情况下的计算示例

    若检测区间为 30 分钟，则超出的部分按照每 15 分钟向上加 1。例如，执行一次【离群检测】，计算为 6 次任务调用。

    3.检测类型算多次且超检测区间情况下的计算示例

    执行两次【区间检测】，叠加检测区间为 60 分钟，则计算为 13 次任务调用（2 次检测 * 5 + 3 次超出检测区间）。


    :material-numeric-2-circle-outline: 智能监控调用次数计算示例：

    假设执行一次【主机智能监控】，则计算为 100 次任务调用。
