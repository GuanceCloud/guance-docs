...
观测云支持商业版拥有者变更空间内的数据存储策略，在观测云工作空间「管理」-「设置」，点击「更换」后，选择所需的数据存储时长，点击「确定」即可变更当前工作空间内的数据存储时长。


???+ attention

    1. 只允许商业版工作空间拥有者进行此操作
    2. 商业版拥有者可按照需求多次变更指标、日志、备份日志、事件、应用性能、用户访问的数据存储时长，每日仅第一次修改当天生效，二次及以上修改，将以最后一次修改的结果为准，在第二天生效
    3. 指标数据存储策略变更后，旧的存储策略里面的数据将会被删除，请谨慎选择。支持为指标集[自定义设置](../../metrics/dictionary.md#storage) 数据存储策略
