...
- 设置高消费预警：设置预警后，当计费项日账单大于预警阈值时，会向 Owner 和 Administrator 发送邮件通知。

    选择[计费项](./billing-method/index.md#item)，并选定**预警阈值**，点击**确定**，添加完成后，您将收到**观测云高消费预警通知**的邮件通知。
...
在**账单列表**中：可查看**累计消费金额**以及各计费项的的消费金额，同时可按不同月份查看每天的费用统计。
...
在消费分析中，可查看昨日支出、累计支出，您也可以根据所选时间范围查看统计消费分析。
