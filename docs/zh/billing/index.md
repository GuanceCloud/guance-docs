...
- 总阈值预警：即为当前工作空间设定一个总体消费阈值，并指定专属通知成员。一旦工作空间内所有计费项的总消费金额**超过**这个阈值，观测云将自动向该成员发送邮件预警。

- 单个计费项预警：即针对单个计费项设置预警。同样支持设置专属的阈值预警通知成员。当某个计费项的**日账单金额超过设定的预警阈值**时，观测云将自动向该成员发送邮件预警。

<img src="img/billing.png" width="50%" >

1. 按需填写总预警阈值并为其设置通知成员；
2. 选择[计费项](../billing-method/billing-item.md#item)并输入**预警阈值**；
3. 为单个计费项设置通知成员；
4. 选择通知成员，无论总阈值还是单一计费项的实际账单金额超出设定的预警阈值时，都会向成员发送预警邮件；
5. 点击**确定**。

**注意**：Owner 角色不受通知对象选择的限制。即无论是否选择 Owner 为通知对象，观测云都会在触发预警条件时向 Owner 发送邮件通知。
...
观测云商业版采用**按量付费**的[计费方式](../billing-method/index.md)，通过计费周期、计费项、计费价格、计费模式等不同的纬度，综合计算出当前的费用情况。
