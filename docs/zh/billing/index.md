...
观测云现有[体验版](../plans/commercial.md)、[商业版](../plans/trail.md)、[私有云部署版](../deployment/deployment-description.md#_4)三种版本。
...
- 所有成员角色：在观测云控制台**管理 > 空间设置 > 基本信息 > 当前版本**中查看；

- 拥有者、管理员：在观测云控制台[**付费计划与账单**](#billing)中查看。



## 付费计划与账单 {#billing}

观测云公有云版秉承按需购买、按量付费的原则。
...
<font size=2>**注意**：仅 Owner 和 Administrator 进行该操作。</font>

为避免日消费超出预算，您可以为当前账户设置总阈值。当工作空间内所有计费项的总消费金额超过阈值，观测云将向 Owner、Administrator 发出邮件预警。

或是直接针对单个计费项设置预警。即当计费项**日账单大于预警阈值**时，会向 Owner 和 Administrator 发送邮件通知。

![](img/billing.png)

1. 按需填写总预警阈值；
2. 选择[计费项](./billing-method/index.md#item)；
3. 并选定**预警阈值**；
4. 点击**确定**。

添加完成后，您将收到**观测云高消费预警通知**的邮件通知；计费项预警通知不会重复发送。
...
- 若转发规则保存到观测云的备份日志，则显示对应的数据保存策略，若保存到外部存储，则展示为 `-`。
...
观测云现有两套独立运行、统计数据量相关连的账号体系，共同为**商业版用户**实现用量计费与费用结算流程：
...
观测云商业版采用<font color=coral>**按量付费**</font>的[计费方式](./billing-method/index.md)，通过计费周期、计费项、计费价格、计费模式等不同的纬度，综合计算出当前的费用情况。



## 费用结算方式

观测云计算出当前工作空间的账单详情后，会同步推送到所绑定的观测云费用中心账号，进行后续费用结算流程。

现支持观测云费用中心账号、云账号等多种[结算方式](./billing-account/index.md)，后者包括阿里云、AWS、华为云、微软云账号结算。在云账号结算模式下支持多个站点的云账单合并到一个云账号下进行结算。
