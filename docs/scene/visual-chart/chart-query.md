...
数据源包括来自指标、日志、基础对象、自定义对象、事件、应用性能、用户访问、安全巡检、网络、Profile、backup_log 的一系列数据组合。

| 选项 | 说明 |
| --- | --- |
| 指标 | 需要选择「指标集」和「指标」，一个指标集可包含多个指标，基础函数默认选中「Last by」。结合图表类型，选择想要展示的指标进行查询。 |
| 日志/基础对象/自定义对象/事件/应用性能/用户访问/安全巡检/网络/ Profile/backup_log | 系统对象、自定义对象和安全巡检需要选择「分类」和「属性/标签」，日志/事件/应用性能/用户访问需要选择「来源」和「属性/标签」，基础函数默认选中「Count by」。 |
