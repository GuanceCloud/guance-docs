...
通过“添加筛选条件”以匹配时序图查询对象与预设异常事件，从而达到时序数据跟事件关联显示的目的。这个功能可以帮助用户在查看趋势的同时，感知数据波动期间是否有相关事件产生，以帮助用户从另一个视角定位问题。
...
事件关联功能支持基于“添加筛选条件”匹配时序图查询对象与预设异常事件（假设：时序图的查询对象为“主机 CPU IOwait”，预设的异常事件监控器为“主机 CPU IOwait 过高”，通过字段“host”可实现时序数据与事件数据关联）。  

- 示例 1: 时序图-图表查询

  ![](../img/11.chart_1.1.png)

- 示例2 : 监控器-主机 CPU IOwait 过高

  ![](../img/11.chart_1.2.png)

完成事件关联配置的时序图，在时间轴上若存在事件记录那么就会用阴影色块标注显示；在分析模式下，点击高亮的阴影色块，即可从新页打开相关事件的查看器。

![](../img/11.chart_1.0.png)
