...
2）**事件通知等级：**包括紧急、重要、警告、无数据、恢复、无数据恢复、无数据视为恢复。默认发送所有异常事件告警通知，如有需要可通过列表自定义事件通知等级。
...
| 紧急 | critical | 致命 |
| 无数据 | nodata | 无数据 |
| 恢复 | ok | 事件从 warning、error、critical 等故障状态恢复 |
| 无数据恢复 | ok | 事件从 nodata 无数据状态恢复 |
| 无数据视为恢复 | ok | 监控器中配置了「无数据时产生恢复事件」后，事件从 warning、error、critical 等故障状态进入 nodata 状态，从而认为恢复 |
