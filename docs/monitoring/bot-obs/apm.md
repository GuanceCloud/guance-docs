...
![](/Users/wendy/dataflux-doc/docs/monitoring/img/1.bot_obs_1.png)
...
说明：按 1 分钟时间间隔聚合数据， 6 小时为取数周期，1 小时为运行周期，查看区间时间内延迟、错误率是否出现异常，根据出现的异常的 `resource` 来进行根因分析，并根据 `resource` 的类型来确定可能因该异常影响的到的用户群体。
