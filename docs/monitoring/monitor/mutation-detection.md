...
突变检测是通过比较两个不同时间段内同一个指标的绝对或相对（%）变化值来判断是否产生异常情况。多应用于追踪某个指标的峰值或者数据变化，当出现异常情况时可以更精准的产生事件留做记录。
...
1）**检测指标：**监控的指标数据，目前仅支持 "指标" 数据。支持比较两个时间段内该指标的差值或者差值百分比。
