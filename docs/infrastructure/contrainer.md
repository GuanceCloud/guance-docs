...
“观测云” 支持您通过分析功能，基于**1-3个标签**对容器对象数据进行分组聚合分析。分组聚合的结果将依据数值大小（自大到小）排行，默认展示前十名（Top10）。

- 点击分组，该分组对应的字段将被做为筛选条件显示查询结果，即您可查看对应分组里的对象数据；
- 点击分组右侧的" :material-cog: 设置"按钮，可自定义添加分组标签，添加完成后，可选择按照添加分组标签进行分组聚合展示。**注意：自定义分组标签仅管理员及以上权限可添加。**
