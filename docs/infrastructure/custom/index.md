...
**注意：**绑定内置视图前，需要确认所绑定内置视图中的视图变量是否存在与该对象分类相关的字段，如`host`。更多配置详情，可参考[绑定内置视图](../../management/built-in-view/bind-view.md)。
...
“观测云” 支持您通过分析功能，基于**1-3个标签**对数据进行分组聚合分析。分组聚合的结果将依据数值大小（自大到小）排行，默认展示前十名（Top10）。
