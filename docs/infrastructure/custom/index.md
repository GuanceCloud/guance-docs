...
添加完自定义对象分类以后，即可通过API的方式进行自定义数据上报。进行自定义对象数据上报时，需要先安装并连通 DataKit 和 DataFlux Function，再通过 DataFlux Function 上报数据到 DataKit，最终通过 DataKit 上报数据到 “观测云” 工作空间。具体操作过程可参考文档 [自定义对象数据上报](custom/data-reporting.md) 。
