...
观测云支持「默认存储」和「 SLS 存储」两种数据存储方案，若您在开通时选择“SLS 存储”方案，那么观测云还支持您在该工作空间绑定查看所属阿里云账号下的其他 SLS 数据，关于如何开通 SLS 存储方案，可参考文档 [选择 SLS 存储](../billing/commercial-version.md#sls) 。
