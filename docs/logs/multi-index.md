...
**仅 SLS 存储方式的工作空间有「绑定索引」功能**。（观测云支持用户注册 SLS 工作空间，默认创建 guance 前缀的 project 以及 logstore，由于部分用户过去就在使用 sls 日志服务，并且需要在观测云中查看这些非 guance 前缀的 logstore，所以观测云新增「绑定索引」功能。
