...
观测云支持「默认存储」和「 SLS 存储」两种数据存储方案，若您在开通观测云商业版时选择“阿里云账号结算”且选择“SLS 存储”，则可以在观测云日志索引中绑定您在阿里云账号中的 SLS 数据索引，关于如何开通 SLS 存储方式，可参考文档 [选择 SLS 存储](../billing/version-description.md#sls) 。

在观测云「日志」-「索引」，点击「绑定索引」。

![](img/log01.png)

在「绑定索引」页面，选择 Project 和与之对应的索引列表，点击确认后，绑定的索引统一在「其他索引」中显示。
