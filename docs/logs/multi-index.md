...
观测云支持「默认存储」和「 SLS 存储」两种数据存储方案。若您在开通时选择「 SLS 存储 」方案，那么观测云还支持您在该工作空间绑定查看所属阿里云账号下的其他 SLS 数据，关于如何开通 SLS 存储方案，可参考文档 [选择 SLS 存储](../billing/commercial-register.md#sls) 。

在观测云「日志」-「索引」，点击「绑定索引」。

![](img/log01.png)

在「绑定索引」页面，填写 AccessKey ID 与 AccessKey Secret 后，自动获取 Project 和 Logstore。索引名称默认与 Logstore 的名称保持一致，您也可以自定义编辑名称。

???+ attention

    - 索引名称在工作空间内需具有唯一性；
    - 观测云商业版工作空间支持绑定外部 SLS 的索引数据，已默认开通 SLS 存储的工作空间支持绑定其他已授权开通 SLS 权限的索引数据，绑定完成后，您可以在观测云工作空间对来自 SLS 的日志数据进行查询和分析。

![](img/1-index-1129.png)

点击「确定」后，即可完成索引绑定，您可以在「查看器」、 「备份日志」通过切换索引进行查看。
...
日志索引支持对绑定的外部索引进行字段映射。在「绑定索引」及「其他索引」页面，原字段 date 默认映射为 time；默认填充 __docid 和 message 两个字段。例如：您可以将原字段 logid 映射为 docid、原字段 content 映射为 message。

在「其他索引」界面，点击编辑，您可以修改索引，编辑字段映射。

![](img/2-index-1129.png)

注意：

- 每个索引的映射规则不相通，各自独立保存；

- 若某条日志存在 _docid 字段，又映射了一个相同字段，则原日志中的 __docid 不生效；

- 若在某部分日志中，您将 xxx 映射给 __docid ，但在这部分日志中， xxx 的 value 不唯一，则只取其中一条展示。
