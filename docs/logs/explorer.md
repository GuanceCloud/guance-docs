...
![](img/17.explorer_2.png)

在日志详情页查看上下文打开新的日志查看器时，会以 “host”、“source”、“service”、“filename” 为筛选条件，以当前日志时间做为开始时间、 当前日志时间 + 30分钟做为结束时间带到时间控件上进行上下文日志筛选。

注意：若当前日志时间距离当前实际系统时间小于 30 分钟，则筛选当前实际系统时间最近 30 分钟的数据。如当前日志时间为 `2022/08/04 18:32:05` （见上图），当前实际系统时间为 `2022/08/04 18:56:40`  ，则查看上下文筛选的数据的时间范围是当前实际系统时间最近 30 分钟的数据（见下图）。

![](img/17.explorer_3.1.png)
...
观测云的日志数据列表默认为您展示日志的触发「时间」和日志的「内容」。您可以在查看器「显示列」中选择日志显示“3行”或“10行”来查看完整的日志内容。
