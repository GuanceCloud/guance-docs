...
| Parameter Name        | Type     | Required   | Description              |
|:------------------|:-------|:-----|:----------------|
| name | string |  | Name (Added in iteration on 2024-09-04, default name: creator_creation_time)<br>Allow null: False <br>Allow empty string: False <br>Maximum length: 64 <br> |
...
| maskFields | string |  | Sensitive fields, multiple fields separated by commas<br>Example: message,host <br>Allow null: False <br>Allow empty string: True <br> |
...
2. If a user has multiple roles and some of these roles are not included in this rule's role list, this data access rule will not apply to the user, meaning it does not restrict the query scope.
...
| indexes | array     | Y | Log index information; for authorized indexes outside the current workspace (which must be authorized by the workspace), use `workspaceUUID:indexUUID`, e.g., ["wksp_111:lgim_222", "wksp_333:lgim_444"] |
...
| extend         | dict     | Y | Extended fields storing the structured content of conditions for frontend display, e.g., {"device": [ "PC"], "session_has_replay": [1]} |
