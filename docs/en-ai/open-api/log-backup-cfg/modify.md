...
| accessCfg.externalId | string |  | External unique ID (external unique ID in AWS role authorization)<br>Can be empty: False <br> |
...
| accessCfg.region | string |  | Region (optional value, if not entered, it defaults to the corresponding region of the provider that matches the current site)<br>Can be empty: False <br> |
