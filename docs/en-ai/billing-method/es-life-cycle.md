...
| **Stage Name** | **Description** | **Writing** | **Querying** |
| --- | --- | --- | --- |
| hot | Hot data stage | Frequent writing | Frequent querying |
| warm | Warm data stage | No writing | Less frequent querying |
| cold | Cold data stage | No writing | Infrequent querying, slow queries |
| frozen | Frozen data stage | No writing | Rare querying, very slow queries |
| delete | Data deletion stage | No writing | Unqueryable |
...
## Practical Application in Guance
...
After modifying the retention policy, new indices will roll over, but the existing index data will not be deleted until it meets the deletion date condition. This means that the previous index data will continue to incur service charges.

![](../img/image.png)

### Extended Data Retention Time

After modifying the retention policy, new indices will roll over, but the existing index data retention time will not extend. New indices will use the new configuration for retention time.
...
- [<font color="coral"> :fontawesome-solid-arrow-right-long: &nbsp; **Implementing Hot-Warm-Cold Architecture with Index Lifecycle Management**</font>](https://www.elastic.co/cn/blog/implementing-hot-warm-cold-in-elasticsearch-with-index-lifecycle-management)
