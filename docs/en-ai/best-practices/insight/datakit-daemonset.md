...
Since [DataKit DaemonSet](/datakit/datakit-daemonset-deploy/) configuration management is very complex, this article will introduce best practices for configuration management.

This document will describe two different deployment methods and their configuration approaches.
...
### Configure Monitoring for Mysql and Pipeline Files
...
Note the YAML format; both `dataway_url` and `dkconfig` need to be changed.

The `values.yaml` can be used for future upgrades.
...
### Enable Git Deployment via Helm

Note: Enabling Git configuration management will make ConfigMap ineffective, and `default_enabled_inputs` will not affect it.

- Using password to manage Git

Modify `dataway_url`, `git_repos.git_url`.

```shell
helm add repo dataflux https://pubrepo.guance.com/chartrepo/datakit
helm repo update 
helm install my-datakit datakit/datakit --set dataway_url="https://openway.guance.com?token=<your-token>" \
...
Modify `dataway_url`, `git_repos.git_url`, `git_repos.git_key_path` (absolute path).
...
### Enable Git Deployment via YAML

First, download [datakit.yaml](https://<<< custom_key.static_domain >>>/datakit/datakit.yaml)

- Using password to manage Git

Modify `datakit.yaml` and add environment variables:
