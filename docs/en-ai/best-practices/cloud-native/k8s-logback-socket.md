# K8s Log Collection Using Logback Socket Best Practices

---

## Application Scenario Introduction

In addition to the commonly used file and stdout methods, Logback can also output logs via socket (TCP). The biggest advantage of using sockets for log reporting is reduced storage costs. Logs generated by applications are buffered in memory locally before being reported to the collection endpoint.

Similarly, DataKit supports Socket log collection. This article primarily introduces how to push logs from a Spring Boot application running on K8s to the Guance platform using the Logback Socket method for monitoring.

## Prerequisites

1. Create a [Guance account](https://www.guance.com/)
...
For Kubernetes, refer to the document <[Kubernetes Application RUM-APM-LOG Integrated Analysis](./k8s-rum-apm-log.md)> for DataKit installation.

#### 1. Configure Log Collection File

To receive logs, you need to enable log socket and open port 9541, then configure the Pipeline for parsing.
...
The `logback_socket_pipeline.p` file is used to parse the socket log format, making it easier to view and use on the Guance platform. Full content as follows:

??? quote "`logback_socket_pipeline.p`"

    ```bash
            #------------------------------------   Warning   -------------------------------------**
            # Do not modify this file. If updates are needed, copy it to another file, preferably with a prefix to avoid being overwritten after restarts.**
            #-----------------------------------------------------------------------------------        **
            # access log**
            json(_,msg,"message")**
            json(_,class,"class")**
...
??? quote "Logback Socket Configuration"
    ...
    ```xml
    <!-- Logs are serialized into JSON format; DataKit supports text-format logs that can be pushed directly via socket -->
...
Full content as follows: (Note: Adjust according to actual application needs.)
...
Copy the JAR file to the current directory and build the image

```bash
docker build -t registry.cn-shenzhen.aliyuncs.com/lr_715377484/springboot-logback-socket-appender-demo:v1 .
```

Push to `docker-hub`, in this example, it is pushed to Alibaba Cloud hub repository.
...
1. Write the `springboot-logback-socket-appender-demo-deployment.yaml` file, and modify the following parameters:

- DATAKIT_SOCKET_PORT: DataKit log socket port.
- dd-java-agent is the Datadog Java-agent used for tracing. If not needed, related configurations can be removed.
...
<[Kubernetes Application RUM-APM-LOG Integrated Analysis](./k8s-rum-apm-log.md)>

<[Guance Log Collection and Analysis Best Practices](./logs.md)>
