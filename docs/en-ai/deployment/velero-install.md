...
???+ warning "Precautions"
     ...
     This article describes the backup of Kubernetes configurations (YAML files) using Velero, which does not include data from PVC volumes.

## Introduction {#intro}

Velero is an open-source tool that securely backs up and restores Kubernetes cluster resources and persistent volumes, performs disaster recovery, and facilitates cluster migration.

- **Disaster Recovery**
  Velero reduces recovery time in cases of infrastructure loss, data corruption, and/or service interruptions.
...
- **Backup Clusters**
  Backs up entire clusters or parts of clusters based on namespace resources or label selectors, including Kubernetes resources and volumes.
...
## Offline Package List {#download-list}

| Name | Download Link |
| :------------------: | :---------------------------------------------: |
| Velero-cli | [Amd Download](https://static.guance.com/dataflux/package/velero-v1.13.0-linux-amd64.tar.gz)<br>[Arm Download](https://static.guance.com/dataflux/package/velero-v1.13.0-linux-arm64.tar.gz) |
| Velero Image | [Amd Download](https://static.guance.com/dataflux/package/velero-amd64.tar.gz)<br/>[Arm Download](https://static.guance.com/dataflux/package/velero-arm64.tar.gz) |
...
=== "S3"
    ...
    Velero requires an object storage bucket to store backups, ideally a unique bucket for each Kubernetes cluster (see [FAQ](https://velero.io/docs/faq/) for more details). Create an S3 bucket, replacing placeholders appropriately:
    ...
    ```
    > If you will be using Velero to back up multiple clusters with multiple S3 buckets, it's best to create a unique username for each cluster rather than using the default Velero.
    ...
    4. Create a specific Velero credential file (`credentials-velero`) in a local directory:
...
=== "OSS"
    ...
    Velero requires an object storage bucket to store backups, ideally unique to each Kubernetes cluster. Create an OSS bucket, replacing placeholders appropriately:
    ...
    > If you will be using Velero to back up multiple clusters with multiple OSS buckets, it's best to create a unique username for each cluster rather than using the default Velero.
    
    2. Attach a policy to grant `velero` necessary permissions:
    
    > Note: For security reasons, it's best to revoke delete permissions after completing backup or restore tasks.
    ...
    Refer to the [Creating AK](https://help.aliyun.com/zh/ram/user-guide/create-an-accesskey-pair?spm=a2c4g.11186623.0.0.77714c5cX1UXZe) guide in Alibaba Cloud documentation.

    4. Create a specific Velero credential file (`credentials-velero`) in your installation directory:
...
=== "S3"
    ...
    > For overseas nodes, modify `s3Url` to `https://s3.$BUCKET.amazonaws.com`
...
Backup daily at midnight (excluding PVC data), retain for 7 days
...
Velero can restore resources to a different namespace from where they were backed up. Use the `--namespace-mappings` flag for this.
velero restore create RESTORE_NAME --from-backup BACKUP_NAME --namespace-mappings old-ns-1:new-ns-1,old-ns-2:new-ns-2
For example, restoring resources from the `test-velero` namespace to `test-velero-1`:
