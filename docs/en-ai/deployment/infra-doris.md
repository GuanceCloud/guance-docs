...
* Doris FE: Mainly responsible for user request handling, query parsing and planning, metadata management, and node management.
* Doris BE: Mainly responsible for data storage and execution of query plans.
* guance-insert: Receives line protocol format data, accumulates locally (usually using tmpfs due to lower space usage), and writes in batches to Doris BE.
...
### Server Description
Typically, the following three groups of machines are used. For small clusters, they can be co-located. The server OS is preferably Ubuntu.

* doris-fe: CPU to memory ratio 1:2, one 20GB+ data disk for metadata storage, the first machine can deploy Doris Manager.
* doris-be: CPU to memory ratio 1:8, disk configuration should be optimized for maximum disk throughput (number of disks).
...
* Resources cannot be preempted by other machines (CPU steal should not be too high).

### Network Description
* Doris Manager is typically deployed on the doris-fe-01 server, which must be able to SSH communicate with all Doris machines and access port 8004.
* guancedb-logs machines need to install supervisor via system package management tools (APT).
* MySQL protocol access to FE machine port 9030 is required, usually installing mysql-client on the Doris Manager machine.
* guancedb-logs machines need to access: doris-fe machine ports 8030 and 9030; doris-be machine ports 8040 and 9060; the first guancedb-logs machine port 8428.
...
* Doris is deployed **on hosts**. Regardless of network conditions, the installation package must be placed in the specified location first.

| Category           | Description                                           |
|--------------|----------------------------------------------| 
| **Host Deployment Components**  | be + fe + manager + guancedb-logs            | | 
| **Deployment Prerequisites**   | 1. Provide root password and support passwordless SSH login as root<br/>2. CPU architecture supports AVX2 instruction set |
...
???+ warning "Host Deployment Instructions"
    ...
    There is only one guancedb-logs machine, providing an address for Guance services as host IP:8480/8481;
    
    If there are multiple guancedb-logs machines:
    
    a. If ELB capability is available, use ELB to listen on multiple guancedb-logs 8480/8481 ports, and the service address will be ELB IP:8480/8481;
    
    b. If ELB capability is unavailable, create a service within the business cluster. The service addresses are:
...
After decompressing the downloaded package, it contains the SelectDB + manager installation package. Place this on the fe-01 machine, in the directory specified in `inventory/doris-manager.vars.yaml`.
```shell
https://static.guance.com/guancedb/selectdb-latest.tar.gz
```

After decompressing the downloaded package, it contains the GuanceDB installation package. Place this on all guancedb-logs machines, in the directory specified in `inventory/guancedb-logs-doris.vars.yaml`.
```shell
https://static.guance.com/guancedb/guancedb-cluster-linux-amd64-latest.tar.gz
```

After decompressing the downloaded package, it contains the VictoriaMetrics + vmutils installation package. Place this on all guancedb-logs machines, in the directory specified in `inventory/guancedb-logs-doris.vars.yaml`.
```shell
...
???+ warning "Deployment Checkpoints"
     ...
     Verify that the configurations of be and fe machines are consistent;
...
# hosts.name:    Mark the server role, can be xxx-doris-be-01, where xxx can be poc or prd
# hosts.port:    SSH port, usually 22
# hosts.host:    SSH target machine IP
# hosts.user:    SSH target machine user, usually root
...
clusters:
  - name: xxx
    vars:
      ...
      # Cold storage settings can be omitted if not used
      oss_endpoint:
      oss_bucket:
      # Can be omitted
      oss_region:
      # When using cloud vendor object storage, usually not filled; when using self-built object storage or endpoint is IP, fill in "path"
      addressing_style:
```
...
clusters:
  - name: xxx
    vars:
      ...
      be_data_disk_num:
      # Size of each BE data disk in GB
      be_data_disk_gb:
      # Retention time for FE logs
      fe_log_retention: 3d
...
```shell
  - name: xxx
    vars:
      # Version number is not used, leave blank
      version: ""
      # Directory where the installation package is located; e.g., /root/packages
      local_dir:
```
...
clusters:
  - name: xxx
    vars:
      # zyadmin user password for the operating system, recommend using a strong password for security
      os_zyadmin_password:
      # doris user password for the operating system, recommend using a strong password for security
      os_doris_password:
      ...
      # Object storage access key, can be omitted if cold storage is not used
      oss_ak:
      # Object storage secret key, can be omitted if cold storage is not used
      oss_sk:
      # URLs for self-monitoring data reporting, usually only one is needed, can be omitted
      dataway_urls: []
...
Initialize the machines, then verify disk mounting.
```shell
...
???+ warning "Deployment Checkpoints"
     ...
     Verify swap is permanently disabled on servers;
    
     Verify the `vm.max_map_count` parameter is set to 2000000.
...
```shell
If this path exists, it indicates cgroup v1 is currently active
ls /sys/fs/cgroup/cpu/

If this path exists, it indicates cgroup v2 is currently active
...
- BE data storage directories: Each data disk corresponds to one input box, e.g., /data1/doris/data, /data2/doris/data, etc.
...
???+ warning "Deployment Checkpoints"
     ...
     Verify that `storage_path` is correctly configured in the configuration files;
    
     Verify that `priority_networks` is correctly configured in the configuration files.

Modify BE configuration: In the Doris Manager cluster page, click the "..." button in the top right corner -> "Parameter Configuration" -> Select all BE nodes -> Click "Edit Configuration" in the top right corner -> Paste the generated `be.conf` -> Check "Confirm..." -> Click "OK"

![](img/be-conf-1.png)
![](img/be-conf-3.png)

Modify FE configuration: In the Doris Manager cluster page, click the "..." button in the top right corner -> "Parameter Configuration" -> Select all FE nodes -> Click "Edit Configuration" in the top right corner -> Paste the generated `fe.conf` -> Check "Confirm..." -> Click "OK"
...
This step configures the S3 bucket. You can verify if files can be uploaded to the bucket as follows:

Switch to the root user on the FE machine and log into the cluster.
```shell
mysql -uroot -h127.0.0.1 -P 9030
```
Execute the following SQL and check if files prefixed with `result_` are generated in the `default_resource` folder in the S3 bucket. If no files are generated, check the permissions.
```shell
...
???+ warning "Deployment Checkpoints"
     ...
     Verify that the S3 storage can be used and that the configuration cannot be changed after setup;
