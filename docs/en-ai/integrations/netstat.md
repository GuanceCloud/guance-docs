...
NetStat Metrics display, including Tcp connection count, waiting connections, pending requests, Udp Socket connections, etc.
...
=== "Host Deployment"
    ...
    Navigate to the `conf.d/host` directory under the DataKit installation directory, copy `netstat.conf.sample`, and rename it to `netstat.conf`. An example is as follows:
    ...
    ``` toml
    # (3) Server has multiple network interfaces, only care about a few interfaces
    [[inputs.netstat.addr_ports]]
      ports = ["1.1.1.1:80","2.2.2.2:80"]
    ```

    ``` toml
    # (4) Server has multiple network interfaces, require displaying each interface separately, this configuration will override the ports setting
    [[inputs.netstat.addr_ports]]
...
=== "Kubernetes"
    ...
    Also supports modifying configuration parameters via environment variables (requires adding it to ENV_DEFAULT_ENABLED_INPUTS as a default collector):
    ...
    - **ENV_INPUT_NETSTAT_ADDR_PORTS**
        ...
        Group ports and add different tags for easier statistics
...
By default, all collected data will append a global tag named `host` (the tag value is the hostname where DataKit resides), and you can also specify other tags through `[inputs.netstat.tags]` in the configuration:
...
Metrics not grouped by port number: `netstat`, Metrics grouped by port number: `netstat_port`.
...
|`tcp_syn_recv`|SYN_RECV : The number of TCP states waiting for confirmation of connection acknowledgment after both sender and receiver have sent/received connection requests.|int|count|
...
|`tcp_time_wait`|TIME_WAIT : The number of TCP states waiting sufficient time to ensure the remote TCP host received acknowledgment of its request for connection termination.|int|count|
