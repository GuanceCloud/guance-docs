...
Link your JAVA application to Guance:

- Collect custom Metrics data from the application;
- Gather trace data from the application;
...
???+ info "About MDC"
    ...
    Mapping Diagnostic Context (MDC) is a tool used to distinguish interleaved log outputs from different sources. — [log4j MDC documentation](http://logging.apache.org/log4j/1.2/apidocs/org/apache/log4j/MDC.html)
<!-- markdownlint-enable -->
...
- ***trace_flags*** The current trace flags, formatted according to W3C trace flag format (same as `Span.current().getSpanContext().getTraceFlags().asHex()`).
...
Different APM tools configure log `pattern` differently. Taking `logback.xml` as an example:
<!-- markdownlint-disable MD046 -->
=== "OpenTelemetry"
