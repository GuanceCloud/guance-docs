...
The ClickHouse collector can collect various metrics exposed by the ClickHouse server instance, such as statement execution counts and memory storage, IO interactions, and more. It then sends these metrics to Guance to help you monitor and analyze any abnormal situations with ClickHouse.
...
- In the `config.xml` configuration file of the ClickHouse Server, find the following code segment, uncomment it, and set the port number for exposing metrics (choose any unique port). After modifying, restart the server (if in a cluster, perform this operation on each machine).
...
- `metrics`: Flag indicating whether to expose metrics from ClickHouse's `system.metrics` table.
- `events`: Flag indicating whether to expose events from ClickHouse's `system.events` table.
- `asynchronous_metrics`: Flag indicating whether to expose asynchronous metrics from ClickHouse's `system.asynchronous_metrics` table.

For more details, refer to the [official ClickHouse documentation](https://ClickHouse.com/docs/en/operations/server-configuration-parameters/settings/#server_configuration_parameters-prometheus){:target="_blank"}
...
=== "Host Installation"
    ...
    Navigate to the `conf.d/clickhousev1` directory under the DataKit installation directory, copy `clickhousev1.conf.sample`, and rename it to `clickhousev1.conf`. Example configuration is as follows:
    ...
    [[inputs.clickhousev1]]
      ...
      ## If collected data exceeds the limit, it will be dropped.
      ## Default is 32MB.
      # max_file_size = 0
      ...
      ## Supports regex. Multiple supported, conditions met if one matches.
      ## Collects all if empty.
      # metric_name_filter = ["cpu"]
    
      ## Metrics name blacklist.
      ## If a word exists in both blacklist and whitelist, blacklist takes priority.
      ## Supports regex. Multiple supported, conditions met if one matches.
      ## Collects all if empty.
      ...
      ## Matched tags will be dropped, but items will still be sent.
      # tags_ignore = ["xxxx"]
...
=== "Kubernetes"
    ...
    Currently, the collector can be enabled via [ConfigMap method](../datakit/datakit-daemonset-deploy.md#configmap-setting).
<!-- markdownlint-enable -->

## Metrics {#metric}

By default, all collected data appends global election tags and can specify other tags through `[inputs.clickhousev1.tags]`:
...
| ---- |---- | :---:    | :----: |
|`AsynchronousHeavyMetricsCalculationTimeSpent`|Time in seconds spent calculating heavy tables-related asynchronous metrics. This is the overhead of asynchronous metrics.|float|s|
|`AsynchronousHeavyMetricsUpdateInterval`|Update interval for heavy (tables-related) metrics.|float|s|
|`AsynchronousMetricsCalculationTimeSpent`|Time in seconds spent calculating asynchronous metrics. This is the overhead of asynchronous metrics.|float|s|
|`AsynchronousMetricsUpdateInterval`|Update interval for asynchronous metrics.|float|s|
|`BlockActiveTime`|Time in seconds the block device had IO requests queued. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. Source: `/sys/block`.|float|s|
|`BlockDiscardBytes`|Number of discarded bytes on the block device. Relevant for SSDs. Discard operations are not used by ClickHouse but can be used by other processes on the system. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. Source: `/sys/block`.|float|B|
|`BlockDiscardMerges`|Number of discard operations requested from the block device and merged together by the OS IO scheduler. Relevant for SSDs. Discard operations are not used by ClickHouse but can be used by other processes on the system. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. Source: `/sys/block`.|float|count|
|`BlockDiscardOps`|Number of discard operations requested from the block device. Relevant for SSDs. Discard operations are not used by ClickHouse but can be used by other processes on the system. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. Source: `/sys/block`.|float|count|
|`BlockDiscardTime`|Time in seconds spent on discard operations requested from the block device, summed across all operations. Relevant for SSDs. Discard operations are not used by ClickHouse but can be used by other processes on the system. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. Source: `/sys/block`.|float|s|
|`BlockInFlightOps`|Counts the number of I/O requests issued to the device driver but not yet completed. Does not include IO requests in the queue but not yet issued to the device driver. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. Source: `/sys/block`.|float|count|
|`BlockQueueTime`|Counts the number of milliseconds that IO requests have waited on this block device. If multiple IO requests are waiting, this value increases as the product of the number of milliseconds times the number of requests waiting. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. Source: `/sys/block`.|float|ms|
|`BlockReadBytes`|Number of bytes read from the block device. It can be lower than the number of bytes read from the filesystem due to the usage of the OS page cache, which saves IO. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. Source: `/sys/block`.|float|B|
|`BlockReadMerges`|Number of read operations requested from the block device and merged together by the OS IO scheduler. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. Source: `/sys/block`.|float|count|
|`BlockReadOps`|Number of read operations requested from the block device. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. Source: `/sys/block`.|float|count|
|`BlockReadTime`|Time in seconds spent on read operations requested from the block device, summed across all operations. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. Source: `/sys/block`.|float|s|
|`BlockWriteBytes`|Number of bytes written to the block device. It can be lower than the number of bytes written to the filesystem due to the usage of the OS page cache, which saves IO. A write to the block device may happen later than the corresponding write to the filesystem due to write-through caching. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. Source: `/sys/block`.|float|B|
|`BlockWriteMerges`|Number of write operations requested from the block device and merged together by the OS IO scheduler. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. Source: `/sys/block`.|float|count|
|`BlockWriteOps`|Number of write operations requested from the block device. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. Source: `/sys/block`.|float|count|
|`BlockWriteTime`|Time in seconds spent on write operations requested from the block device, summed across all operations. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. Source: `/sys/block`.|float|s|
|`CPUFrequencyMHz`|Current frequency of the CPU, in MHz. Most modern CPUs dynamically adjust the frequency for power saving and Turbo Boosting.|float|MHz|
...
|`DiskAvailable`|Available bytes on the disk (virtual filesystem). Remote `filesystems` can show large values like 16 EiB.|float|B|
|`DiskTotal`|Total size in bytes of the disk (virtual filesystem). Remote `filesystems` can show large values like 16 EiB.|float|B|
|`DiskUnreserved`|Available bytes on the disk (virtual filesystem) without reservations for merges, fetches, and moves. Remote `filesystems` can show large values like 16 EiB.|float|B|
|`DiskUsed`|Used bytes on the disk (virtual filesystem). Remote `filesystems` do not always provide this information.|float|B|
|`FilesystemCacheBytes`|Total bytes in the `cache` virtual filesystem. This cache is held on disk.|float|B|
|`FilesystemCacheFiles`|Total number of cached file segments in the `cache` virtual filesystem. This cache is held on disk.|float|count|
|`FilesystemLogsPathAvailableBytes`|Available bytes on the volume where ClickHouse logs path is mounted. If this value approaches zero, tune the log rotation in the configuration file.|float|B|
|`FilesystemLogsPathAvailableINodes`|Number of available `inodes` on the volume where ClickHouse logs path is mounted.|float|count|
|`FilesystemLogsPathTotalBytes`|Size of the volume where ClickHouse logs path is mounted, in bytes. It's recommended to have at least 10 GB for logs.|float|B|
|`FilesystemLogsPathTotalINodes`|Total number of `inodes` on the volume where ClickHouse logs path is mounted.|float|count|
|`FilesystemLogsPathUsedBytes`|Used bytes on the volume where ClickHouse logs path is mounted.|float|B|
|`FilesystemLogsPathUsedINodes`|Number of used `inodes` on the volume where ClickHouse logs path is mounted.|float|count|
|`FilesystemMainPathAvailableBytes`|Available bytes on the volume where the main ClickHouse path is mounted.|float|B|
|`FilesystemMainPathAvailableINodes`|Number of available `inodes` on the volume where the main ClickHouse path is mounted. If it is close to zero, it indicates a misconfiguration, and you will get 'no space left on device' even when the disk is not full.|float|count|
|`FilesystemMainPathTotalBytes`|Size of the volume where the main ClickHouse path is mounted, in bytes.|float|B|
|`FilesystemMainPathTotalINodes`|Total number of `inodes` on the volume where the main ClickHouse path is mounted. If it is less than 25 million, it indicates a misconfiguration.|float|count|
|`FilesystemMainPathUsedBytes`|Used bytes on the volume where the main ClickHouse path is mounted.|float|B|
|`FilesystemMainPathUsedINodes`|Number of used `inodes` on the volume where the main ClickHouse path is mounted. This value mostly corresponds to the number of files.|float|count|
|`HTTPThreads`|Number of threads in the HTTP interface server (without TLS).|float|count|
|`InterserverThreads`|Number of threads in the replicas communication protocol server (without TLS).|float|count|
|`Jitter`|Difference in time the thread for calculating asynchronous metrics was scheduled to wake up and the actual wake-up time. A proxy-indicator of overall system latency and responsiveness.|float|-|
|`LoadAverage`|Whole system load, averaged with exponential smoothing over 1 minute. The load represents the number of threads across all processes (the scheduling entities of the OS kernel) that are currently running on CPU or waiting for IO, or ready to run but not being scheduled at this point in time. This number includes all processes, not only `clickhouse-server`. The number can be greater than the number of CPU cores if the system is overloaded, and many processes are ready to run but waiting for CPU or IO.|float|count|
|`MMapCacheCells`|Number of files opened with `mmap` (mapped in memory). Used for queries with the setting `local_filesystem_read_method` set to `mmap`. Files opened with `mmap` are kept in the cache to avoid costly TLB flushes.|float|count|
...
|`MemoryCode`|Amount of virtual memory mapped for the pages of machine code of the server process, in bytes.|float|B|
|`MemoryDataAndStack`|Amount of virtual memory mapped for stack use and allocated memory, in bytes. It is unspecified whether it includes per-thread stacks and most of the allocated memory, which is allocated with the `mmap` system call. This metric exists only for completeness reasons. Recommend using the `MemoryResident` metric for monitoring.|float|B|
|`MemoryResident`|Amount of physical memory used by the server process, in bytes.|float|B|
|`MemoryShared`|Amount of memory used by the server process that is also shared by other processes, in bytes. ClickHouse does not use shared memory, but some memory can be labeled by OS as shared for its own reasons. This metric does not make much sense to watch and exists only for completeness reasons.|float|B|
|`MemoryVirtual`|Size of the virtual address space allocated by the server process, in bytes. The size of the virtual address space is usually much greater than the physical memory consumption and should not be used as an estimate for memory consumption. Large values of this metric are normal and make technical sense.|float|B|
|`MySQLThreads`|Number of threads in the MySQL compatibility protocol server.|float|count|
|`NetworkReceiveBytes`|Number of bytes received via the network interface. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|B|
|`NetworkReceiveDrop`|Number of bytes dropped while receiving via the network interface. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|B|
|`NetworkReceiveErrors`|Number of times an error occurred while receiving via the network interface. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|count|
|`NetworkReceivePackets`|Number of network packets received via the network interface. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|count|
|`NetworkSendBytes`|Number of bytes sent via the network interface. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|B|
|`NetworkSendDrop`|Number of times a packet was dropped while sending via the network interface. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|count|
|`NetworkSendErrors`|Number of times an error (e.g., TCP retransmit) occurred while sending via the network interface. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|count|
|`NetworkSendPackets`|Number of network packets sent via the network interface. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|count|
|`NumberOfDatabases`|Total number of databases on the server.|float|count|
|`NumberOfDetachedByUserParts`|Total number of parts detached from MergeTree tables by users with the `ALTER TABLE DETACH` query (as opposed to unexpected, broken, or ignored parts). The server does not care about detached parts and they can be removed.|float|count|
|`NumberOfDetachedParts`|Total number of parts detached from MergeTree tables. A part can be detached by a user with the `ALTER TABLE DETACH` query or by the server itself if the part is broken, unexpected, or unneeded. The server does not care about detached parts and they can be removed.|float|count|
|`NumberOfTables`|Total number of tables summed across the databases on the server, excluding databases that cannot contain MergeTree tables. Excluded database engines generate the set of tables on the fly, like `Lazy`, `MySQL`, `PostgreSQL`, `SQLite`.|float|count|
|`OSContextSwitches`|Number of context switches that the system underwent on the host machine. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|count|
|`OSGuestNiceTime`|Ratio of time spent running a virtual CPU for guest operating systems under the control of the Linux kernel, when a guest was set to a higher priority (see `man procfs`). This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. This metric is irrelevant for ClickHouse but still exists for completeness. The value for a single CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSGuestNiceTimeCPU`|Ratio of time spent running a virtual CPU for guest operating systems under the control of the Linux kernel, when a guest was set to a higher priority (see `man procfs`). This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. This metric is irrelevant for ClickHouse but still exists for completeness. The value for a single CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSGuestNiceTimeNormalized`|Similar to `OSGuestNiceTime` but divided by the number of CPU cores to measure in the [0..1] interval regardless of the number of cores. This allows averaging the values of this metric across multiple servers in a cluster even if the number of cores is non-uniform, getting the average resource utilization metric.|float|rate|
|`OSGuestTime`|Ratio of time spent running a virtual CPU for guest operating systems under the control of the Linux kernel (see `man procfs`). This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. This metric is irrelevant for ClickHouse but still exists for completeness. The value for a single CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSGuestTimeCPU`|Ratio of time spent running a virtual CPU for guest operating systems under the control of the Linux kernel (see `man procfs`). This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. This metric is irrelevant for ClickHouse but still exists for completeness. The value for a single CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSGuestTimeNormalized`|Similar to `OSGuestTime` but divided by the number of CPU cores to measure in the [0..1] interval regardless of the number of cores. This allows averaging the values of this metric across multiple servers in a cluster even if the number of cores is non-uniform, getting the average resource utilization metric.|float|rate|
|`OSIOWaitTime`|Ratio of time the CPU core was not running code but when the OS kernel did not run any other process on this CPU as the processes were waiting for IO. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. The value for a single CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSIOWaitTimeCPU`|Ratio of time the CPU core was not running code but when the OS kernel did not run any other process on this CPU as the processes were waiting for IO. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. The value for a single CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSIOWaitTimeNormalized`|Similar to `OSIOWaitTime` but divided by the number of CPU cores to measure in the [0..1] interval regardless of the number of cores. This allows averaging the values of this metric across multiple servers in a cluster even if the number of cores is non-uniform, getting the average resource utilization metric.|float|rate|
|`OSIdleTime`|Ratio of time the CPU core was idle (not even ready to run a process waiting for IO) from the OS kernel standpoint. This does not include the time when the CPU was under-utilized due to internal reasons within the CPU (memory loads, `pipeline stalls`, branch `mispredictions`, running another SMT core). This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. The value for a single CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSIdleTimeCPU`|Ratio of time the CPU core was idle (not even ready to run a process waiting for IO) from the OS kernel standpoint. This does not include the time when the CPU was under-utilized due to internal reasons within the CPU (memory loads, `pipeline stalls`, branch `mispredictions`, running another SMT core). This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. The value for a single CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSIdleTimeNormalized`|Similar to `OSIdleTime` but divided by the number of CPU cores to measure in the [0..1] interval regardless of the number of cores. This allows averaging the values of this metric across multiple servers in a cluster even if the number of cores is non-uniform, getting the average resource utilization metric.|float|rate|
|`OSInterrupts`|Number of interrupts on the host machine. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|count|
|`OSIrqTime`|Ratio of time spent handling hardware interrupt requests on the CPU. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. A high number of this metric may indicate hardware misconfiguration or very high network load. The value for a single CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSIrqTimeCPU`|Ratio of time spent handling hardware interrupt requests on the CPU. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. A high number of this metric may indicate hardware misconfiguration or very high network load. The value for a single CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSIrqTimeNormalized`|Similar to `OSIrqTime` but divided by the number of CPU cores to measure in the [0..1] interval regardless of the number of cores. This allows averaging the values of this metric across multiple servers in a cluster even if the number of cores is non-uniform, getting the average resource utilization metric.|float|rate|
|`OSMemoryAvailable`|Amount of memory available to be used by programs, in bytes. This is very similar to the `OSMemoryFreePlusCached` metric. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|B|
|`OSMemoryBuffers`|Amount of memory used by OS kernel buffers, in bytes. This should typically be small, and large values may indicate a misconfiguration of the OS. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|B|
|`OSMemoryCached`|Amount of memory used by the OS page cache, in bytes. Typically, almost all available memory is used by the OS page cache - high values of this metric are normal and expected. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|B|
|`OSMemoryFreePlusCached`|Amount of free memory plus OS page cache memory on the host system, in bytes. This memory is available to be used by programs. The value should be very similar to `OSMemoryAvailable`. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|B|
|`OSMemoryFreeWithoutCached`|Amount of free memory on the host system, in bytes. This does not include memory used by the OS page cache memory, in bytes. The page cache memory is also available for program usage, so the value of this metric can be confusing. See the `OSMemoryAvailable` metric instead. For convenience, we also provide the `OSMemoryFreePlusCached` metric, which should be somewhat similar to `OSMemoryAvailable`. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|B|
|`OSMemorySwapCached`|Amount of memory in swap that was also loaded into RAM. Swap should be disabled on production systems. If the value of this metric is large, it indicates a misconfiguration. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|B|
|`OSMemoryTotal`|Total amount of memory on the host system, in bytes.|float|B|
|`OSNiceTime`|Ratio of time the CPU core was running `userspace` code with higher priority. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. The value for a single CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSNiceTimeCPU`|Ratio of time the CPU core was running `userspace` code with higher priority. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. The value for a single CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSNiceTimeNormalized`|Similar to `OSNiceTime` but divided by the number of CPU cores to measure in the [0..1] interval regardless of the number of cores. This allows averaging the values of this metric across multiple servers in a cluster even if the number of cores is non-uniform, getting the average resource utilization metric.|float|rate|
|`OSOpenFiles`|Total number of opened files on the host machine. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|count|
|`OSProcessesBlocked`|Number of threads blocked waiting for I/O to complete (`man procfs`). This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|count|
|`OSProcessesCreated`|Number of processes created. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|count|
|`OSProcessesRunning`|Number of runnable (running or ready to run) threads by the operating system. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`.|float|count|
|`OSSoftIrqTime`|Ratio of time spent handling software interrupt requests on the CPU. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. A high number of this metric may indicate inefficient software running on the system. The value for a single CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSSoftIrqTimeCPU`|Ratio of time spent handling software interrupt requests on the CPU. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. A high number of this metric may indicate inefficient software running on the system. The value for a single CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSSoftIrqTimeNormalized`|Similar to `OSSoftIrqTime` but divided by the number of CPU cores to measure in the [0..1] interval regardless of the number of cores. This allows averaging the values of this metric across multiple servers in a cluster even if the number of cores is non-uniform, getting the average resource utilization metric.|float|rate|
|`OSStealTime`|Ratio of time spent in other operating systems by the CPU when running in a virtualized environment. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. Not every virtualized environment presents this metric, and most of them do not. The value for a single CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSStealTimeCPU`|Ratio of time spent in other operating systems by the CPU when running in a virtualized environment. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. Not every virtualized environment presents this metric, and most of them do not. The value for a single CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSStealTimeNormalized`|Similar to `OSStealTime` but divided by the number of CPU cores to measure in the [0..1] interval regardless of the number of cores. This allows averaging the values of this metric across multiple servers in a cluster even if the number of cores is non-uniform, getting the average resource utilization metric.|float|rate|
|`OSSystemTime`|Ratio of time the CPU core was running OS kernel (system) code. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. The value for a single CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSSystemTimeCPU`|Ratio of time the CPU core was running OS kernel (system) code. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. The value for a single| CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSSystemTimeNormalized`|Similar to `OSSystemTime` but divided by the number of CPU cores to measure in the [0..1] interval regardless of the number of cores. This allows averaging the values of this metric across multiple servers in a cluster even if the number of cores is non-uniform, getting the average resource utilization metric.|float|rate|
|`OSThreadsRunnable`|Total number of 'runnable' threads, as seen by the OS kernel scheduler.|float|count|
|`OSThreadsTotal`|Total number of threads, as seen by the OS kernel scheduler.|float|count|
|`OSUptime`|Uptime of the host server (the machine where ClickHouse is running), in seconds.|float|s|
|`OSUserTime`|Ratio of time the CPU core was running `userspace` code. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. This includes also the time when the CPU was under-utilized due to internal reasons within the CPU (memory loads, `pipeline stalls`, branch `mispredictions`, running another SMT core). The value for a single CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSUserTimeCPU`|Ratio of time the CPU core was running `userspace` code. This is a system-wide metric, including all processes on the host machine, not just `clickhouse-server`. This includes also the time when the CPU was under-utilized due to internal reasons within the CPU (memory loads, `pipeline stalls`, branch `mispredictions`, running another SMT core). The value for a single CPU core will be in the interval [0..1]. The value for all CPU cores is calculated as a sum across them [0..num cores].|float|rate|
|`OSUserTimeNormalized`|Similar to `OSUserTime` but divided by the number of CPU cores to measure in the [0..1] interval regardless of the number of cores. This allows averaging the values of this metric across multiple servers in a cluster even if the number of cores is non-uniform, getting the average resource utilization metric.|float|rate|
|`PostgreSQLThreads`|Number of threads in the server of the PostgreSQL compatibility protocol.|float|count|
|`PrometheusThreads`|Number of threads in the Prometheus endpoint server. Note: Prometheus endpoints can also be used via the usual HTTP/HTTPS ports.|float|count|
|`ReplicasMaxAbsoluteDelay`|Maximum difference in seconds between the most recent replicated part and the most recent data part still to be replicated, across Replicated tables. A very high value indicates a replica with no data.|float|s|
...
|`TCPThreads`|Number of threads in the TCP protocol server (without TLS).|float|count|
|`Temperature`|Temperature of the corresponding device in ℃. A sensor can return an unrealistic value. Source: `/sys/class/thermal`|float|C|
...
|`TotalPartsOfMergeTreeTables`|Total amount of data parts in all tables of MergeTree family. Numbers larger than 10,000 will negatively affect the server startup time and may indicate an unreasonable choice of the partition key.|float|count|
|`TotalRowsOfMergeTreeTables`|Total amount of rows (records) stored in all tables of MergeTree family.|float|count|
|`UncompressedCacheBytes`|Total size of uncompressed cache in bytes. Uncompressed cache does not usually improve performance and should be mostly avoided.|float|B|
...
|`Uptime`|Server uptime in seconds. It includes the time spent for server initialization before accepting connections.|float|s|
...
|`BackgroundBufferFlushSchedulePoolSize`|Limit on the number of tasks in BackgroundBufferFlushSchedulePool|float|count|
...
|`BackgroundCommonPoolSize`|Limit on the number of tasks in an associated background pool|float|count|
|`BackgroundCommonPoolTask`|Number of active tasks in an associated background pool|float|count|
|`BackgroundDistributedSchedulePoolSize`|Limit on the number of tasks in BackgroundDistributedSchedulePool|float|count|
|`BackgroundDistributedSchedulePoolTask`|Number of active tasks in BackgroundDistributedSchedulePool. This pool is used for distributed sends done in the background.|float|count|
|`BackgroundFetchesPoolSize`|Limit on the number of simultaneous fetches in an associated background pool|float|count|
|`BackgroundFetchesPoolTask`|Number of active fetches in an associated background pool|float|count|
|`BackgroundMergesAndMutationsPoolSize`|Limit on the number of active merges and mutations in an associated background pool|float|count|
|`BackgroundMergesAndMutationsPoolTask`|Number of active merges and mutations in an associated background pool|float|count|
|`BackgroundMessageBrokerSchedulePoolSize`|Limit on the number of tasks in BackgroundProcessingPool for message streaming|float|count|
...
|`BackgroundMovePoolSize`|Limit on the number of tasks in BackgroundProcessingPool for moves|float|count|
...
|`BackgroundSchedulePoolSize`|Limit on the number of tasks in BackgroundSchedulePool. This pool is used for periodic ReplicatedMergeTree tasks, like cleaning old data parts, altering data parts, replica re-initialization, etc.|float|count|
...
|`BackupsThreadsActive`|Number of threads in the thread pool for BACKUP running a task.|float|count|
|`BrokenDistributedFilesToInsert`|Number of files for asynchronous insertion into Distributed tables that have been marked as broken. This metric starts from 0 on start. Number of files for every shard is summed.|float|count|
...
|`ContextLockWait`|Number of threads waiting for lock in Context. This is a global lock.|float|count|
...
|`DelayedInserts`|Number of INSERT queries that are throttled due to a high number of active data parts for a partition in a MergeTree table.|float|count|
|`DestroyAggregatesThreads`|Number of threads in the thread pool for destroying aggregate states.|float|count|
|`DestroyAggregatesThreadsActive`|Number of threads in the thread pool for destroying aggregate states running a task.|float|count|
|`DictCacheRequests`|Number of requests in flight to data sources of dictionaries of cache type.|float|count|
...
|`EphemeralNode`|Number of ephemeral nodes held in ZooKeeper.|float|count|
...
|`GlobalThread`|Number of threads in the global thread pool.|float|count|
|`GlobalThreadActive`|Number of threads in the global thread pool running a task.|float|count|
...
|`IOPrefetchThreads`|Number of threads in the IO `prefetch` thread pool.|float|count|
...
|`KafkaConsumersInUse`|Number of consumers currently used by direct or background reads|float|count|
...
|`MarksLoaderThreads`|Number of threads in the thread pool for loading marks.|float|count|
...
|`MergeTreeAllRangesAnnouncementsSent`|The current number of announcements being sent in flight from the remote server to the initiator server about the set of data parts (for MergeTree tables). Measured on the remote server side.|float|count|
...
|`NetworkReceive`|Number of threads receiving data from the network. Only ClickHouse-related network interaction is included, not by 3rd party libraries.|float|count|
|`NetworkSend`|Number of threads sending data to the network. Only ClickHouse-related network interaction is included, not by 3rd party libraries.|float|count|
...
|`PartsDeleteOnDestroy`|Part was moved to another disk and should be deleted in its own destructor.|float|count|
...
|`PendingAsyncInsert`|Number of asynchronous inserts waiting for flush.|float|count|
...
|`QueryPreempted`|Number of queries that are stopped and waiting due to the 'priority' setting.|float|count|
...
|`Read`|Number of read (`read`, `pread`, `io_getevents`, etc.) `syscalls` in flight|float|count|
...
|`RemoteRead`|Number of read with remote reader in flight|float|count|
...
|`SendExternalTables`|Number of connections sending data for external tables to remote servers. External tables are used to implement GLOBAL IN and GLOBAL JOIN operators with distributed `subqueries`.|float|count|
|`SendScalars`|Number of connections sending data for scalars to remote servers.|float|count|
...
|`StorageHiveThreads`|Number of threads in the StorageHive thread pool.|float|count|
...
|`TCPConnection`|Number of connections to TCP server (clients with native interface), also including server-server distributed query connections|float|count|
...
|`TablesToDropQueueSize`|Number of dropped tables waiting for background data removal.|float|count|
...
|`Write`|Number of write (`write`/`pwrite`/`io_getevents`, etc.) `syscalls` in flight|float|count|
|`ZooKeeperRequest`|Number of requests to ZooKeeper in flight.|float|count|
...
|`AggregationHashTablesInitializedAsTwoLevel`|How many hash tables were initialized as two-level for aggregation.|float|count|
...
|`AsynchronousReadWaitMicroseconds`|Time spent waiting for asynchronous reads.|float|ms|
|`AsynchronousRemoteReadWaitMicroseconds`|Time spent waiting for asynchronous remote reads.|float|ms|
...
|`CannotRemoveEphemeralNode`|Number of times an error happened while trying to remove an ephemeral node. This is not an issue, because our implementation of the ZooKeeper library guarantees that the session will expire and the node will be removed.|float|count|
...
|`CompileExpressionsMicroseconds`|Total time spent compiling expressions to LLVM code.|float|ms|
...
|`CompressedReadBufferBlocks`|Number of compressed blocks (the blocks of data that are compressed independently of each other) read from compressed sources (files, network).|float|count|
...
|`ContextLock`|Number of times the lock of Context was acquired or tried to acquire. This is a global lock.|float|count|
|`CreatedHTTPConnections`|Total amount of created HTTP connections (counter increases every time a connection is created).|float|count|
...
|`CreatedReadBufferDirectIOFailed`|Number of times a read buffer with O_DIRECT was attempted to be created for reading data (while choosing among other read methods), but the OS did not allow it (due to lack of filesystem support or other reasons) and we fell back to the ordinary reading method.|float|count|
...
|`CreatedReadBufferMMapFailed`|Number of times a read buffer with `mmap` was attempted to be created for reading data (while choosing among other read methods), but the OS did not allow it (due to lack of filesystem support or other reasons) and we fell back to the ordinary reading method.|float|count|
|`CreatedReadBufferOrdinary`|Number of times an ordinary read buffer was created for reading data (while choosing among other read methods).|float|count|
|`DNSError`|Total count of errors in DNS resolution|float|count|
|`DataAfterMergeDiffersFromReplica`|Number of times data after merge is not byte-identical to the data on another replica. There could be several reasons.|float|count|
|`DataAfterMutationDiffersFromReplica`|Number of times data after mutation is not byte-identical to the data on another replica. In addition to the reasons described in 'DataAfterMergeDiffersFromReplica', it is also possible due to non-deterministic mutation.|float|count|
|`DelayedInserts`|Number of times the INSERT of a block to a MergeTree table was throttled due to a high number of active data parts for partition.|float|count|
|`DelayedInsertsMilliseconds`|Total number of milliseconds spent while the INSERT of a block to a MergeTree table was throttled due to a high number of active data parts for partition.|float|ms|
...
|`DictCacheKeysRequestedMiss`|Number of keys requested from the data source for dictionaries of 'cache' types but not found in the data source.|float||`DictCacheLockReadNs`|Number of nanoseconds spent waiting for read lock to look up the data for dictionaries of 'cache' types.|float|ms|
|`DictCacheLockWriteNs`|Number of nanoseconds spent waiting for write lock to update the data for dictionaries of 'cache' types.|float|ms|
|`DictCacheRequestTimeNs`|Number of nanoseconds spent querying external data sources for dictionaries of 'cache' types.|float|ms|
|`DictCacheRequests`|Number of bulk requests to external data sources for dictionaries of 'cache' types.|float|count|
...
|`DiskReadElapsedMicroseconds`|Total time spent waiting for read syscall. This includes reads from page cache.|float|ms|
...
|`DiskS3WriteMicroseconds`|Time of POST, DELETE, PUT and PATCH requests to DiskS3 storage.|float|ms|
...
|`DiskWriteElapsedMicroseconds`|Total time spent waiting for write syscall. This includes writes to page cache.|float|ms|
...
|`DistributedDelayedInserts`|Number of times the INSERT of a block to a Distributed table was throttled due to a high number of pending bytes.|float|count|
|`DistributedDelayedInsertsMilliseconds`|Total number of milliseconds spent while the INSERT of a block to a Distributed table was throttled due to a high number of pending bytes.|float|ms|
|`DistributedRejectedInserts`|Number of times the INSERT of a block to a Distributed table was rejected with 'Too many bytes' exception due to a high number of pending bytes.|float|count|
...
|`ExternalProcessingCompressedBytesTotal`|Number of compressed bytes written by external processing (sorting/aggregating/joining).|float|B|
|`ExternalProcessingFilesTotal`|Number of files used by external processing (sorting/aggregating/joining).|float|count|
|`ExternalProcessingUncompressedBytesTotal`|Amount of data (uncompressed, before compression) written by external processing (sorting/aggregating/joining).|float|B|
...
|`FileSegmentCacheWriteMicroseconds`|Metric per file segment. Time spent writing data to cache.|float|ms|
|`FileSegmentPredownloadMicroseconds`|Metric per file segment. Time spent pre-downloading data to cache (pre-downloading - finishing file segment download after someone who failed to do that up to the point current thread was requested to do).|float|ms|
|`FileSegmentReadMicroseconds`|Metric per file segment. Time spent reading from file.|float|ms|
|`FileSegmentUsedBytes`|Metric per file segment. How many bytes were actually used from the current file segment.|float|B|
|`FileSegmentWaitReadBufferMicroseconds`|Metric per file segment. Time spent waiting for internal read buffer (includes cache waiting).|float|ms|
|`FileSegmentWriteMicroseconds`|Metric per file segment. Time spent writing cache.|float|ms|
...
|`InsertedBytes`|Number of bytes (uncompressed; for columns as they are stored in memory) INSERTed to all tables.|float|B|
...
|`KafkaProducerErrors`|Number of errors during producing messages to Kafka.|float|count|
...
|`KeeperSaveSnapshot`|Number of snapshots saved.|float|count|
|`KeeperSetRequest`|Number of set requests.|float|count|
|`KeeperSnapshotApplys`|Number of snapshot applications.|float|count|
|`KeeperSnapshotApplysFailed`|Number of failed snapshot applications.|float|count|
|`KeeperSnapshotCreations`|Number of snapshot creations.|float|count|
...
|`MemoryOvercommitWaitTimeMicroseconds`|Total time spent waiting for memory to be freed in `OvercommitTracker`.|float|ms|
|`Merge`|Number of launched background merges.|float|count|
|`MergeTreeAllRangesAnnouncementsSent`|The number of announcements sent from the remote server to the initiator server about the set of data parts (for MergeTree tables). Measured on the remote server side.|float|count|
|`MergeTreeAllRangesAnnouncementsSentElapsedMicroseconds`|Time spent sending the announcement from the remote server to the initiator server about the set of data parts (for MergeTree tables). Measured on the remote server side.|float|ms|
...
|`MergeTreeDataProjectionWriterUncompressedBytes`|Uncompressed bytes (for columns as they are stored in memory) INSERTed to MergeTree tables projection.|float|B|
...
|`MergeTreeDataWriterUncompressedBytes`|Uncompressed bytes (for columns as they are stored in memory) INSERTed to MergeTree tables.|float|B|
...
|`MergedIntoInMemoryParts`|Number of parts merged into InMemory format.|float|count|
...
|`MergedUncompressedBytes`|Uncompressed bytes (for columns as they are stored in memory) that were read for background merges. This is the number before merge.|float|B|
...
|`NetworkSendBytes`|Total number of bytes sent to network. Only ClickHouse-related network interaction is included, not by 3rd party libraries.|float|B|
...
|`OpenedFileCacheMisses`|Number of times a file has not been found in the opened file cache, so we had to open it again.|float|count|
|`OtherQueryTimeMicroseconds`|Total time of queries that are not SELECT or INSERT.|float|ms|
|`OverflowAny`|Number of times approximate GROUP BY was in effect: when aggregation was performed only on top of the first 'max_rows_to_group_by' unique keys and other keys were ignored due to 'group_by_overflow_mode' = 'any'.|float|count|
...
|`OverflowThrow`|Number of times data processing was canceled by query complexity limitation with setting '*_overflow_mode' = 'throw' and an exception was thrown.|float|count|
|`PerfAlignmentFaults`|Number of alignment faults. These happen when unaligned memory accesses occur; the kernel can handle these but it reduces performance. This happens only on some architectures (never on x86).|float|count|
...
|`PerfBranchMisses`|Mis-predicted branch instructions.|float|count|
|`PerfBusCycles`|Bus cycles, which can be different from total cycles.|float|count|
|`PerfCacheMisses`|Cache misses. Usually this indicates Last Level Cache misses; this is intended to be used in conjunction with the `PERFCOUNTHWCACHEREFERENCES` event to calculate cache miss rates.|float|count|
...
|`PerfLocalMemoryMisses`|Local NUMA node memory read misses.|float|B|
...
|`QueryMaskingRulesMatch`|Number of times query masking rules were successfully matched.|float|count|
...
|`ReadBufferSeekCancelConnection`|Number of seeks leading to a new connection (S3, HTTP).|float|count|
...
|`RejectedInserts`|Number of times the INSERT of a block to a MergeTree table was rejected with 'Too many parts' exception due to a high number of active data parts for partition.|float|count|
|`RemoteFSBuffers`|Number of buffers created for asynchronous reading from remote filesystem.|float|count|
|`RemoteFSCancelledPrefetches`|Number of canceled prefetches (because of seek).|float|count|
...
|`RemoteFSSeeksWithReset`|Number of seeks leading to a new connection.|float|count|
|`RemoteFSUnprefetchedBytes`|Number of bytes from unprefetched buffer.|float|B|
|`RemoteFSUnprefetchedReads`|Number of reads from unprefetched buffer.|float|count|
...
|`ReplicaPartialShutdown`|How many times a Replicated table had to deinitialize its state due to session expiration in ZooKeeper. The state is reinitialized every time ZooKeeper is available again.|float|count|
|`ReplicatedDataLoss`|Number of times a data part that we wanted does not exist on any replica (even on replicas that are offline right now). That data parts are definitely lost. This is normal due to asynchronous replication (if quorum inserts were not enabled), when the replica on which the data part was written failed and when it became online after fail it does not contain that data part.|float|count|
|`ReplicatedPartChecks`|Number of times we had to perform advanced search for a data part on replicas or clarify the need for an existing data part.|float|count|
|`ReplicatedPartChecksFailed`|Number of times the advanced search for a data part on replicas did not give a result or when an unexpected part has been found and moved away.|float|count|
|`ReplicatedPartFailedFetches`|Number of times a data part was failed to download from a replica of a ReplicatedMergeTree table.|float|count|
|`ReplicatedPartFetches`|Number of times a data part was downloaded from a replica of a ReplicatedMergeTree table.|float|count|
|`ReplicatedPartFetchesOfMerged`|Number of times we prefer to download an already merged part from a replica of a ReplicatedMergeTree table instead of performing a merge ourselves (usually we prefer doing a merge ourselves to save network traffic). This happens when we do not have all source parts to perform a merge or when the data part is old enough. | float | count |
...
| `ScalarSubqueriesCacheMiss` | Number of times a read from a scalar subquery was not cached and had to be calculated completely. | float | count |
| `ScalarSubqueriesGlobalCacheHit` | Number of times a read from a scalar subquery was done using the global cache. | float | count |
| `ScalarSubqueriesLocalCacheHit` | Number of times a read from a scalar subquery was done using the local cache. | float | count |
...
| `SelectedBytes` | Number of bytes (uncompressed; for columns as they are stored in memory) SELECTed from all tables. | float | B |
...
| `SlowRead` | Number of reads from a file that were slow. This indicates system overload. Thresholds are controlled by read_backoff_* settings. | float | count |
...
| `StorageBufferErrorOnFlush` | Number of times a buffer in the 'Buffer' table has not been able to flush due to error writing to the destination table. | float | count |
| `StorageBufferFlush` | Number of times a buffer in a 'Buffer' table was flushed. | float | count |
| `StorageBufferLayerLockReadersWaitMilliseconds` | Time waiting for Buffer layer during reading. | float | ms |
| `StorageBufferLayerLockWritersWaitMilliseconds` | Time waiting for free Buffer layer to write to (can be used to tune Buffer layers). | float | ms |
| `StorageBufferPassedAllMinThresholds` | Number of times criteria on min thresholds have been reached to flush a buffer in a 'Buffer' table. | float | count |
| `StorageBufferPassedBytesFlushThreshold` | Number of times background-only flush threshold on bytes has been reached to flush a buffer in a 'Buffer' table. This is an expert-only metric. If you read this and you are not an expert, stop reading. | float | count |
| `StorageBufferPassedBytesMaxThreshold` | Number of times criteria on max bytes threshold have been reached to flush a buffer in a 'Buffer' table. | float | count |
| `StorageBufferPassedRowsFlushThreshold` | Number of times background-only flush threshold on rows has been reached to flush a buffer in a 'Buffer' table. This is an expert-only metric. If you read this and you are not an expert, stop reading. | float | count |
| `StorageBufferPassedRowsMaxThreshold` | Number of times criteria on max rows threshold have been reached to flush a buffer in a 'Buffer' table. | float | count |
| `StorageBufferPassedTimeFlushThreshold` | Number of times background-only flush threshold on time has been reached to flush a buffer in a 'Buffer' table. This is an expert-only metric. If you read this and you are not an expert, stop reading. | float | count |
| `StorageBufferPassedTimeMaxThreshold` | Number of times criteria on max time threshold have been reached to flush a buffer in a 'Buffer' table. | float | count |
...
| `SynchronousRemoteReadWaitMicroseconds` | Time spent waiting for synchronous remote reads. | float | ms |
| `SystemTimeMicroseconds` | Total time spent in processing (queries and other tasks) threads executing CPU instructions in OS kernel space. This includes time CPU pipeline was stalled due to cache misses, branch mispredictions, hyper-threading, etc. | float | ms |
...
| `ThreadPoolReaderPageCacheMiss` | Number of times the read inside ThreadPoolReader was not done from page cache and was handed off to thread pool. | float | count |
| `ThreadPoolReaderPageCacheMissBytes` | Number of bytes read inside ThreadPoolReader when read was not done from page cache and was handed off to thread pool. | float | B |
...
| `UserTimeMicroseconds` | Total time spent in processing (queries and other tasks) threads executing CPU instructions in user space. This includes time CPU pipeline was stalled due to cache misses, branch mispredictions, hyper-threading, etc. | float | ms |
...
| `WaitPrefetchTaskMicroseconds` | Time spent waiting for prefetched reader. | float | ms |
...
| `WriteBufferFromFileDescriptorWriteFailed` | Number of times the write (`write`/`pwrite`) to a file descriptor failed. | float | count |
...
| `ZooKeeperBytesSent` | Number of bytes sent over network while communicating with ZooKeeper. | float | B |
...
| `ZooKeeperClose` | Number of times connection with ZooKeeper has been closed voluntarily. | float | count |
...
| `ZooKeeperWaitMicroseconds` | Number of microseconds spent waiting for responses from ZooKeeper after creating a request, summed across all requesting threads. | float | ms |
...
This concludes the translation of the ClickHouse metrics documentation into English. If you need further assistance or have any specific sections you would like to focus on, feel free to ask!