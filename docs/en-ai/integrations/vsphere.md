...
Create a user `datakit` in the vCenter management interface and assign it `read-only` permissions, applying them to the resources that need monitoring. If you need to monitor all sub-objects, check the `Propagate to children` option.
...
=== "Host Installation"
    ...
    Enter the `conf.d/vmware` directory under the DataKit installation directory, copy `vsphere.conf.sample` and rename it to `vsphere.conf`. Example configuration is as follows:
...
By default, all collected data will append a global tag named `host` (tag value is the hostname where DataKit resides), additional tags can be specified in the configuration through `[inputs.vsphere.tags]`:
...
???+ attention
    ...
    Not all the following metrics are collected; refer to the [Data Collection Level](https://docs.vmware.com/cn/VMware-vSphere/7.0/com.vmware.vsphere.monitoring.doc/GUID-25800DE4-68E5-41CC-82D9-8811E27924BC.html){:target="_blank"} for detailed information.
