...
Currently, DataKit supports one method for collecting C/C++ profiling data, which is [Pyroscope](https://pyroscope.io/){:target="_blank"}.
...
Data flow of profiling data: "Pyroscope Agent collects profiling data -> DataKit -> Guance".

### Prerequisites {#pyroscope-requirement}

- According to the official Pyroscope documentation on [eBPF Profiling](https://pyroscope.io/docs/ebpf/#prerequisites-for-profiling-with-ebpf){:target="_blank"}, Linux kernel version >= 4.9 is required (due to the event [BPF_PROG_TYPE_PERF_EVENT](https://lkml.org/lkml/2016/9/1/831){:target="_blank"}).
...
Here is an example for the Linux AMD64 platform:
...
Following the above method will obtain the Pyroscope binary file, which can be run directly or placed in the [PATH](http://www.linfo.org/path_env_var.html){:target="_blank"}.
...
export PYROSCOPE_SERVER_ADDRESS='http://localhost:4040/' # URL configured for Pyroscope listening in DataKit profile.
...
After running the above profiling commands, Pyroscope Agent will start collecting the specified profiling data and report it to Guance. After a few minutes, you can view the corresponding data in the Guance workspace under [APM -> Profile](https://console.guance.com/tracing/profile){:target="_blank"}.