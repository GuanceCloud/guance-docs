...
=== "Host Installation"
    ...
    Navigate to the `conf.d/pinpoint` directory under the DataKit installation directory, copy `pinpoint.conf.sample`, and rename it to `pinpoint.conf`. An example is shown below:
    ...
    The listening address configuration for the DataKit Pinpoint Agent is:
...
=== "Kubernetes"
    ...
    It also supports modifying configuration parameters via environment variables (requires adding to ENV_DEFAULT_ENABLED_INPUTS as default collectors):
    ...
    - **ENV_INPUT_PINPOINT_TAGS**
        ...
        Custom tags. If there are same-named tags in the configuration file, they will overwrite them.
...
???+ warning "Limitations of the Pinpoint Agent in DataKit"

    - Currently only supports gRPC protocol
    - Multiple services (Agent/Metadata/Stat/Span) use the same port
    - Differences between Pinpoint traces and DataKit traces, see [below](pinpoint.md#opentracing-vs-pinpoint)
...
DataKit tracing data follows the OpenTracing protocol. In DataKit, a trace is linked through a simple parent-child structure (child span stores the parent span ID) and each span corresponds to a function call.
...
Pinpoint APM tracing data is more complex:

- The parent span generates the child span ID.
- Child spans also store the parent span ID.
- Uses span events instead of spans in OpenTracing.
- One span represents one service response process.
...
In version `DataKit 1.19.0`, after optimization, the `source` was changed to `PinPointV2`. The new version reorganized the relationships between `SpanChunk` and `Span`, `Event` and `Span`, and `Span` and `Span`. It also addressed the time alignment issue of `startElapsed` and `endElapsed` in `Event`.

Key points:

- Cache the `serviceType` service table and write it to a file to prevent data loss on DataKit restart.
- If `parentSpanId` in `Span` is not -1, cache it. If `parentSpanId` is -1, retrieve the `Span` from the cache using `nextSpanId` in `spanEvent` and append it to the trace.
- Cache all `event`s in `SpanChunk` until the main `Span` is received, then retrieve all cached events and append them to the trace.
- Accumulate the current `Event`'s `startElapsed` as the start time for the next `Event`.
- Determine the parent-child relationship of the current `Event` based on the `Depth` field.
- Replace SQL queries with the current 'resource' name.
...
- [Official Pinpoint Documentation](https://pinpoint-apm.gitbook.io/pinpoint/){:target="_blank"}
- [Pinpoint Version Documentation Library](https://pinpoint-apm.github.io/pinpoint/index.html){:target="_blank"}
- [Official Pinpoint Repository](https://github.com/pinpoint-apm){:target="_blank"}
- [Online Pinpoint Instance](http://125.209.240.10:10123/main){:target="_blank"}