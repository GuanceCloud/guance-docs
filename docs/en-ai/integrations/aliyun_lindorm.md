...
tags: 
  - Aliyun
summary: 'Use the cloud synchronization script package series in the Script Market to synchronize cloud monitoring and cloud asset data to Guance.'
__int_icon: 'icon/aliyun_lindorm'
dashboard:
  - desc: 'Aliyun Lindorm built-in views'
    path: 'dashboard/en/aliyun_lindorm/'

monitor:
  - desc: 'Aliyun Lindorm monitors'
...
Use the cloud synchronization script package series in the Script Market to synchronize cloud monitoring and cloud asset data to Guance.
...
To synchronize **Lindorm** cloud resource monitoring data, install the corresponding collection script: 「Guance Integration (Aliyun-**Lindorm** Collection)」(ID: `guance_aliyun_lindorm`)

After clicking 【Install】, enter the required parameters: Aliyun AK, Aliyun account name.

Click 【Deploy Startup Script】and the system will automatically create a `Startup` script set and configure the corresponding startup scripts.

Once enabled, you can view the corresponding automatic trigger configuration under 「Management / Automatic Trigger Configuration」. Click 【Execute】to run it immediately without waiting for the scheduled time. After a short while, you can check the execution task records and corresponding logs.
...
## Metrics {#metric}
After configuring Aliyun cloud monitoring, the default metric set is as follows. You can collect more metrics through configuration. [Details of Aliyun Cloud Monitoring Metrics](https://cms.console.aliyun.com/metric-meta/acs_lindorm/lindorm){:target="_blank"}
...
| `storage_used_bytes`         | Storage space used       | userId,instanceId,host | Average,Maximum,Minimum | bytes   |    60 s    |
...
| `hot_storage_used_bytes`     | Hot storage used       | userId,instanceId,host | Average,Maximum         | bytes   |    60 s    |
...
| `cold_storage_used_bytes`    | Cold storage used       | userId,instanceId,host | Average,Maximum,Minimum | bytes   |    60 s    |
| `cold_storage_total_bytes`   | Total cold storage capacity         | userId,instanceId,host | Average,Maximum,Minimum | bytes   |    60 s    |
| `cold_storage_token_percent` | Cold read token usage ratio   | userId,instanceId,host | Average,Maximum,Minimum | %       |    60 s    |
...
| `table_cold_storage_used_bytes` | Wide table cold storage used | userId,instanceId,host | Average,Maximum,Minimum | bytes | 60 s |
| `table_hot_storage_used_bytes` | Wide table hot storage used | userId,instanceId,host | Average,Maximum,Minimum | Byte | 60 s |
...
| `scan_rt_p99` | P99 delay for Scan operations | userId,instanceId,host | Average,Maximum,Minimum | milliseconds | 60 s |
| `thrift_call_num_ops` | Thrift request volume | userId,instanceId | Average | count | 60 s |
| `thrift_call_mean` | Average duration of Thrift requests | userId,instanceId | Average | milliseconds | 60 s |
| `thrift_call_time_in_queue_ops` | Number of Thrift requests in queue | userId,instanceId | Average | count | 60 s |
| `thrift_call_time_in_queue` | Queue wait time for Thrift requests | userId,instanceId | Average | ms | 60 s |
...
| `above_memstore_count` | Times exceeding `memstore` limit | userId,instanceId,host | Average,Maximum,Minimum | frequency | 60 s |
| `lql_connection` | SQL connection count | userId,instanceId,host | Average,Maximum | count | 60 s |
| `lql_select_ops` | Select request volume | userId,instanceId,host | Average,Maximum | countSecond | 60 s |
| `lql_upsert_avg_rt` | Average duration of `upsert` requests | userId,instanceId,host | Average,Maximum | milliseconds | 60 s |
| `lql_upsert_ops` | `upsert` request volume | userId,instanceId,host | Average,Maximum | countSecond | 60 s |
| `lql_select_p99_rt` | P99 duration for select requests | userId,instanceId,host | Average,Maximum | milliseconds | 60 s |
| `lql_select_avg_rt` | Average duration of select requests | userId,instanceId,host | Average,Maximum | milliseconds | 60 s |
| `lql_upsert_p99_rt` | P99 duration for `upsert` requests | userId,instanceId,host | Average,Maximum | milliseconds | 60 s |
| `lql_delete_ops` | Delete request volume | userId,instanceId,host | Average,Maximum | countSecond | 60 s |
| `lql_delete_avg_rt` | Average duration of delete requests | userId,instanceId,host | Average,Maximum | milliseconds | 60 s |
| `lql_delete_p99_rt` | P99 duration for delete requests | userId,instanceId,host | Average,Maximum | milliseconds | 60 s |
| `regions_per_ldserver` | Number of Regions managed by RegionServer | userId,instanceId,host | Average,Maximum,Minimum | count | 60 s |
| `handler_queue_size` | Length of HandlerQueue | userId,instanceId,host | Average,Maximum,Minimum | count | 60 s |
...
| `tsdb_hot_storage_used_bytes` | Time series hot storage used | userId,instanceId,host | Average,Maximum,Minimum | bytes | 60 s |
| `tsdb_cold_storage_used_bytes` | Time series cold storage used | userId,instanceId,host | Average,Maximum,Minimum | bytes | 60 s |
| `tsdb_datapoints_added` | Number of data points | userId,instanceId,host | Average,Maximum,Minimum | count | 60 s |
| `search_cold_storage_used_bytes` | Search cold storage used | userId,instanceId,host | Average,Maximum,Minimum | bytes | 60 s |
| `search_hot_storage_used_bytes` | Search hot storage used | userId,instanceId,host | Average,Maximum,Minimum | bytes | 60 s |
...
## Objects {#object}
The collected Aliyun `Lindorm` object data structure can be viewed from 「Infrastructure - Custom」.
