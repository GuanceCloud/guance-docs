...
在 Datakit 安装目录 `/usr/local/datakit/conf.d` ，打开配置文件 `datakit.conf` ，开启 dca 即 `enable = true`  ，监听端口默认开启即 `listen = "0.0.0.0:9531"` ，使用命令 `datakit --restart` 重启，即可完成配置。
...
目前 DCA Web 端支持 Docker 镜像安装，下载 DCA 镜像后运行即可使用。
