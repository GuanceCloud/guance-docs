...
![image.png](../imgs/huawei-obs-4.png)
...
3、 创建 ID 为 OBS Object 的脚本

![image.png](../imgs/huawei-obs-5.png)

4、 添加代码

- 需要修改账号配置 `AccessKey ID` 、 `Secret access key` 、 `Account Name` 、`Region`
...
import guance_huaweicloud_obs__main as huawei_obs
...
def run():
    ...
    collectors = [
        huawei_obs.DataCollector(account, common_huaweiyun_configs),
    ]

    Runner(collectors).run()
```

5、 创建 ID 为 OBS Metrics 的脚本

![image.png](../imgs/huawei-obs-6.png)
...
2、 自动触发配置：在「执行函数」中添加两个脚本，执行频率为 **5 分钟 _/5 _ \* \* \***

OBS Metrics
![image.png](../imgs/huawei-obs-7.png)

OBS Object
![image.png](../imgs/huawei-obs-8.png)
