...
在 Kubernetes 集群，推荐使用 Daemonset 部署 DataKit，通过证书的方式访问 etcd，下面的部署中，[https://172.16.0.229:2379/metrics](https://172.16.0.229:2379/metrics) 是 etcd 的一个节点。

1、 下载 datakit.yaml

登录『[观测云](https://console.guance.com/)』，点击『集成』模块，再点击左上角『DataKit』，选择『Kubernetes』，下载 datakit.yaml，然后按指引完成部署的配置。生产环境

2、 开启 etcd 采集器

在 Kubernetes 集群，开通采集器需要使用 ConfigMap 定义配置，然后挂载到 DataKit 相应目录。采集使用了 https 协议，需要用到证书，下面是 Kubeadmin 部署的 Kubernetes 集群使用证书的配置方式。 
