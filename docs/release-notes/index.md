...
#### 优化查看器正选、反选、模糊匹配三种筛选模式
 观测云查看器支持按照“字段:值”的方式进行精确筛选、模糊筛选（wildcard）和反选，支持对筛选的“字段:值”进行编辑修改。更多详情可参考文档 [查看器搜索说明](../others/explorer-search.md) 。

![](img/12.explorer_search.png)

#### 优化查看器快捷筛选

在观测云查看器快捷筛选，默认全选，支持在选择字段筛选内容时进行“重置”，并通过“All”和“only”进行“反选”，若在搜索框同时包括正选和反选，则对应的快捷筛选标签置灰不可选择。更多详情可参考文档 [快捷筛选](../others/explorer-search.md#quick-filter) 。

![](img/12.explorer_search_11.png)

#### 新增查看器显示列多种快捷操作

在观测云查看器，通过「显示列」，您可以向查看器列表添加、删除显示列；当鼠标放在添加的显示列上时，支持对添加的显示列进行升降序、向左移动列、向右移动列、移除列等操作。若显示列的内容显示不完整，支持把鼠标放在显示列右侧分割线上「双击分割线」，即可展开该列的内容。
...
- 快照开启时间筛选，将当前页面所选时间范围转换成“绝对时间”进行保存
- 快照关闭时间筛选，跟随系统默认时间，分享出去的快照可以切换时间控件


![](img/11.snapshot_1.1.png)

快照保存后，可在查看器历史快照列表进行查看，选择并打开历史快照，点击右上家「返回查看器」即可回到默认查看器。
...
观测云支持在日志查看情详情页的「网路」，选择查看主机、Pod 和 Deployment 视图的数据连接情况。 
...
在日志、进程、链路、主机、Pod 查看器详情关联网络，支持点击时间控件选择回放 48 小时网络数据。
