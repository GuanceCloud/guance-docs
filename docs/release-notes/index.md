...
在场景新建查看器时，支持选择日志、应用性能、用户访问、安全巡检、Profile 这 5 种数据类型。保存后不可更改。当选择用户访问类型时，需要先选定应用。

![](img/3.changelog_1.png)

#### 优化内置视图绑定功能

内置视图绑定功能支持 value 的模糊匹配

![](img/3.changelog_2.png)
...
- 查看器搜索、查看器快捷筛选、图表事件关联、日志检测等监控器支持 wildcard 左*匹配
- 基础设施分析维度优化，在主机详情页主机最后上报时间
- 新增工作空间成员查询数据 30 分钟内连续超过 3 次以上慢查询，则限制该成员在该空间的查询权限 30 分钟
- 新增缓存用户上一次通过单点登录的账号信息
