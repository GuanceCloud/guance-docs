...
观测云商业版工作空间支持绑定外部 SLS 的索引数据，已默认开通 SLS 存储的工作空间支持绑定其他已授权开通 SLS 权限的索引数据，绑定完成后，您可以在观测云工作空间对来自 SLS 的日志数据进行查询和分析。更多详情可参考文档 [日志索引](../logs/multi-index.md#sls) 。
