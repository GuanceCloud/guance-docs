...
所有的商业版工作空间均支持绑定索引； SLS 工作空间除支持绑定阿里云一键注册商业版时获取的 AccessKey ID 与 AccessKey Secret 外，还支持通过日志绑定索引的功能，切换绑定同一阿里云账号下、或者其他阿里云账号下的已授权开通 SLS 权限的 AK/SK 。更多详情可参考文档 [日志索引](../logs/multi-index.md#sls) 。



##### 新增字段映射功能

日志索引支持对绑定的外部索引进行字段映射。在「日志」-「索引」页面，「绑定索引」及「其他索引」界面处增加原字段 date 映射为 time，默认填充 __docid 和 message 两个字段。更多详情可参考文档 [日志索引](../logs/multi-index.md#sls) 。
...
视图变量默认值新增多选，调整隐藏操作和排序操作的位置；默认值是当前视图变量在仪表板中的初始变量，配置时选择的多个默认值支持在仪表板直接筛选展示对应的图表数据。更多详情可参考文档 [视图变量](../scene/view-variable.md) 。
