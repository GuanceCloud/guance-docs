...
#### 新增「基础设施」查看器「指标列」提示信息

- 主机的CPU使用率、MEM使用率、CPU单核负载等指标数据“非实时更新，5分钟统计一次，可能存在误差”；
- 容器/Pod/进程的CPU使用率、MEM使用率等指标数据“非实时更新，5分钟统计一次，可能存在误差”。
...
在日志查看器，您可以切换到手动输入模式，自定义输入筛选条件和搜索条件，如使用 match、and/or、空格等进行组合查询。

#### 优化应用性能监测

- 链路详情页火焰图、span 列表、服务调用关系图显示和交互优化
- 链路详情页-关联 profile 代码热点显示和交互优化
- 快捷筛选处新增 HTTP 相关字段
- Profile 详情页火焰图、维度列表交互优化
...
![](img/10.changelog_2.png)



#### 监控器事件通知功能优化

满足触发条件时发送的事件通知内容，支持输入markdown 格式文本信息。但需要注意，不同告警通知对象支持的 markdown 语法不同，例如：企业微信不支持无序列表。

![](img/10.changelog_3.png)

#### 笔记的文本框 Markdown 格式优化

笔记的文本框顶部增加工具栏，增加预览模式。

![](img/10.changelog_4.png)

#### 静默管理支持配置周期性静默

静默管理新增周期性静默功能，在静默时间中可选择 【仅一次】、【重复】。
...
- 日志多索引支持跳转查看
- 快捷筛选值 TOP 5 支持查看占比数量
- 免费注册流程删除同步开通观测云企业账号
- 优化全部查看器中time 和 create_time 字段的时间格式，默认显示以【2022/09/15 20:53:40】的形式展示。
