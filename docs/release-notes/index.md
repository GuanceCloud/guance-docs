...
同期对比功能不再与锁定时间联动。按钮默认显示且为关闭状态，开启同期对比后，对比维度支持4个选项：小时（与一小时前对比）、日（与一天前对比）、周（与一周前对比）、月（与一个月前对比）。更多详情可参考文档 [同期对比](../scene/visual-chart/time-comparison.md) 。

#### 优化 sls 工作空间索引列表

仅 SLS 存储方式的工作空间有「绑定索引」功能。仅当工作空间有绑定的索引时，显示「其他索引」部分。

观测云支持用户注册 SLS 工作空间，默认创建 guance 前缀的 project 以及 logstore，由于部分用户过去就在使用 sls 日志服务，并且需要在观测云中查看这些非 guance 前缀的 logstore，所以观测云新增「绑定索引」功能。更多详情可参考文档 [日志索引](../logs/multi-index.md) 。
