...
通过“添加筛选条件”以匹配时序图查询对象与预设异常事件，从而达到时序数据跟事件关联显示的目的。这个功能可以帮助用户在查看趋势的同时，感知数据波动期间是否有相关事件产生，以帮助用户从另一个视角定位问题。更多详情可参考文档 [时序图](../scene/visual-chart/timeseries-chart.md) 。

![](img/5.changelog_1.1.png)
...
用户访问监测生成指标支持用户通过“应用名称”选择相关应用作为数据源。

#### 优化智能巡检

- 数据结构升级，应用性能检测事件改为一个服务一条异常事件
- 智能巡检事件详情页面显示调整，图表支持分组、Markdown文本、折线图、Servicemap、饼图、表格图组件
- 智能巡检未恢复列表显示调整
...
在「监控」，调整「分组管理」页面为「告警策略管理」，通过新建告警策略创建监控器分组及其告警设置。
...
基础设施存活检测用于监控基础设施的运行状态。更多详情可参考 [新增基础设施存活检测](../monitoring/monitor/infrastructure-detection.md) 。
...
新增【进程异常检测】用于监控工作空间内的进程数据，支持对进程数据的一个或多个字段类型设置触发告警。更多详情可参考 [进程异常检测](../monitoring/monitor/processes-detection.md) 。
...
- 只读成员不支持查看成员管理页面
- 支持普通成员和 SSO 成员筛选过滤，SSO 成员仅显示当前 IDP 对应成员
...
- PAAS 版本管理后台无法修改工作空间的 RP 策略 bug 解决
