...
DCA 是 DataKit 的 Web 端管理工具，旨在方便管理已经安装和配置的采集器，支持查看采集器运行情况、采集器配置管理、Pipeline管理、黑名单管理以及采集器文档帮助等功能。更多详情可参考文档 [DCA](../dca/index.md) 。
...
- 新增【存在】【不存在】筛选逻辑 - 新增字段数据类型提示，匹配模式说明
- 新增针对数字类型的运算方式
...
时间控件新增手动输入时间范围、绝对时间查询历史、优化时间控件 UI。通过输入或选择时间范围，快速筛选查看对应的数据内容。更多查看器时间控件详情可参考文档 [时间控件说明](../getting-started/necessary-for-beginners/explorer-search.md#time) 。
...
在观测云工作空间，点击左下角账号，选择「功能菜单管理」，即可对菜单进行调整。更多详情可参考文档 [功能菜单管理](../management/customized-menu.md) 。
...
#### 新增图表查询中对分组后的查询做别名
支持对分组后的查询做别名,每个指标只能有一个别名。在添加别名后，图例的名称也随之变化，方便更直观的区分相关指标。（目前支持本功能的图表有：时序图、饼图、柱状图、散点图、气泡图、漏斗图。）
...
#### 新增时序图、饼图图例样式
饼图目前支持底部、右侧、环绕、隐藏图例，时序图目前支持底部、右侧和隐藏图例。默认状态下时序图和饼图都隐藏图例。

![](img/15.changelog_3.png)

#### 优化对象历史数据保存及关联主机、容器、pod 查看

观测云为所有上报到观测云工作空间的对象（主机、容器、pod）存储 48 小时历史数据，方便用户回溯过去两天内某个时刻对象的数据表现情况。通过观测云「日志」、「链路」、「安全巡检」查看器，您可以在「属性视图」中回溯日志、链路、巡检数据产生时主机/容器/pod对象的真实情况，查看相关日志、链路、巡检数据产生前10分钟内最新的一条对象数据。
